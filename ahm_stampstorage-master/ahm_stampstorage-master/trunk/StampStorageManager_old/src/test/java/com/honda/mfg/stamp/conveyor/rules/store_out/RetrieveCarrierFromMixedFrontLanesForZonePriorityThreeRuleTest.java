    ...
    public void successfullyRetrieveCarriersWithHighVolumeParts() {
        ...
        StorageStateContext context = getContext();
                StoreOutRule storeOutRule =
                        new RetrieveCarrierFromRowOfZonePriorityAndRowCondition(context, null, LaneCondition.MIXED_FRONT, StoragePriority.Priority.THREE);
...
//    }
    ...
    List<StorageRow> getMixedFrontLanes() {
        ...
        for (int i = 0; i < bAreaLane3.getCapacity().intValue() - 1; i++) {
            bAreaLane3.store(getEmptyCarrier(OM.EMPTY, OM.OLD_PRODUCTION_RUN_NUMBER, OM.OLD_PRODUCTION_RUN_TIMESTAMP, bAreaLane3.getStop(), bAreaLane3.getStop()));
        }
