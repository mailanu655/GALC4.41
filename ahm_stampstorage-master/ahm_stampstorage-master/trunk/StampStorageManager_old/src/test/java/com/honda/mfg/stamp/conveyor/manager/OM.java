...
public class OM {
    ...
    public static StorageStateContext storageState(StorageRow laneImplOne, StorageRow laneImplTwo, Long backOrderedPart) {
    ...
    public static StorageState storageStateMe(StorageRow laneImplOne, StorageRow laneImplTwo, Long backOrderedPart) {
        List<StorageRow> lanes = new ArrayList<StorageRow>();
        lanes.add(laneImplOne);
        lanes.add(laneImplTwo);
        return storageStateMe(lanes, backOrderedPart);
    }

    public static StorageStateContext storageState(List<StorageRow> lanes, Long backOrderedPart) {
        return new StorageStateContextMock(storageStateMe(lanes, backOrderedPart));
    }
     public static StorageState storageStateMe(List<StorageRow> lanes, Long backOrderedPart) {
        Set<Long> backOrderParts = new HashSet<Long>();
        if (backOrderedPart != null) {
            backOrderParts.add(backOrderedPart);
        }
        return new StorageStateImpl(lanes, backOrderParts);
    }
    ...
    public static Carrier carrier(Long dieNumber, PartProductionVolume volume) {
        return new Carrier(index++, new Die(dieNumber, volume), QUANTITY, NEW_PRODUCTION_RUN_TIMESTAMP, PRODUCTION_RUN_NUMBER);
    }
