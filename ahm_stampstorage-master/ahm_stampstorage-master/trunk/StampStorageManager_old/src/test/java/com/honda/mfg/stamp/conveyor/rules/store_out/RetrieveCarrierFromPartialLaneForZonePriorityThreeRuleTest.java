    ...
    public void successfullyRetrieveCarriersWithHighVolumeParts() {
        ...
        StorageStateContext context = getContext();
                       StoreOutRule storeOutRule =
                        new RetrieveCarrierFromRowOfZonePriorityAndRowCondition(context, null, LaneCondition.PARTIAL, StoragePriority.Priority.THREE);
    ...
    private List<StorageRow> getLanes() {
       //same part oldest production run no and sorted in ascending by carrier count
        ...
        for (int i = 0; i < bAreaLane4.getCapacity().intValue() - 1; i++) {
            bAreaLane4.store(getEmptyCarrier(OM.EMPTY, OM.OTHER_PRODUCTION_RUN_NUMBER, OM.OLD_PRODUCTION_RUN_TIMESTAMP, bAreaLane4.getStop(), bAreaLane4.getStop()));
        }
