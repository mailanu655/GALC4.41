...
public class RetrieveCarrierFromMixedBlockedLanesForZonePriorityOneRuleTest {
    ...
    public void successfullyRetrieveCarriersWithHighVolumeParts() {
        ...
        StoreOutRule storeOutRule =
                        new RetrieveCarrierFromRowOfZonePriorityAndRowCondition(context, null, LaneCondition.MIXED_BLOCK, StoragePriority.Priority.ONE);
        ...
        //make sure that the carrier released is the LOW volume part that was at the head of the row
    ...
    public void successfullyRetrieveCarriersWithMediumVolumeParts() {
        ...
        Carrier carrier = sRow.release();
        //make sure that the carrier released is the HIGH volume part that was at the head of the row
        assertEquals(OM.PART_C, carrier.getDie().getId());
...
//    }
    ...
    List<StorageRow> getMixedFrontLanes() {
        ...
        lane3: high volume, old production run + 1 new in the end
        lane4: med volume, full lane, new production run
        
        C-High (capacity:20)
        ------
        lane1: empty, full lane
        lane2: high volume old production run + 1 low volume old at the end
        lane3: Single high volume new followed by all medium new till the end
        lane4: med volume, full lane, new production run
        ...
        lane1: empty, full lane
        lane2: high volume old production run + 1 low volume old at the end
        lane3: low volume, full lane, old production run
        lane4: Single low volume new followed by med volume old till the end
        
        B Area (capacity:31)
        ------
        lane1: Single high volume new followed by medium new till the end
        lane2: Single low volume new followed by high volume old till the end
        lane3: high volume, old production run + 1 old low vol in the end
       ...
       */
        ...
        for (int i = 0; i < aAreaLane3.getCapacity() - 1; i++) {
            aAreaLane3.store(getHighVolumeCarrier(OM.PART_A, OM.OLD_PRODUCTION_RUN_NUMBER, OM.OLD_PRODUCTION_RUN_TIMESTAMP, aAreaLane3.getStop(), aAreaLane3.getStop()));
        }
        ...
        for (int i = 0; i < aAreaLane4.getCapacity() - 1; i++) {
            aAreaLane4.store(getMediumVolumeCarrier(OM.PART_B, OM.PRODUCTION_RUN_NUMBER, OM.NEW_PRODUCTION_RUN_TIMESTAMP, aAreaLane4.getStop(), aAreaLane4.getStop()));
        }
        ...
        for (int i = 0; i < cHighLane2.getCapacity().intValue() - 1; i++) {
        	cHighLane2.store(getHighVolumeCarrier(OM.PART_A, OM.OLD_PRODUCTION_RUN_NUMBER, OM.OLD_PRODUCTION_RUN_TIMESTAMP, cHighLane2.getStop(), cHighLane2.getStop()));
        }
        ...
        cLowLane2.store(getLowVolumeCarrier(OM.PART_C, OM.PRODUCTION_RUN_NUMBER, OM.NEW_PRODUCTION_RUN_TIMESTAMP, cLowLane2.getStop(), cLowLane2.getStop()));
        ...
        cLowLane4.store(getLowVolumeCarrier(OM.PART_C, OM.PRODUCTION_RUN_NUMBER, OM.NEW_PRODUCTION_RUN_TIMESTAMP, cLowLane4.getStop(), cLowLane4.getStop()));
        for (int i = 0; i < cLowLane4.getCapacity()-1 ; i++) {
            cLowLane4.store(getMediumVolumeCarrier(OM.PART_B, OM.OLD_PRODUCTION_RUN_NUMBER, OM.OLD_PRODUCTION_RUN_TIMESTAMP, cLowLane4.getStop(), cLowLane4.getStop()));
        }

          //high volume capacity
        bAreaLane1.store(getHighVolumeCarrier(OM.PART_A, OM.PRODUCTION_RUN_NUMBER, OM.NEW_PRODUCTION_RUN_TIMESTAMP, bAreaLane1.getStop(), bAreaLane1.getStop()));
        ...
        bAreaLane2.store(getLowVolumeCarrier(OM.PART_C, OM.OLD_PRODUCTION_RUN_NUMBER, OM.OLD_PRODUCTION_RUN_TIMESTAMP, bAreaLane2.getStop(), bAreaLane2.getStop()));
        ...
        sAreaLane2.store(getLowVolumeCarrier(OM.PART_C, OM.PRODUCTION_RUN_NUMBER, OM.NEW_PRODUCTION_RUN_TIMESTAMP, sAreaLane2.getStop(), sAreaLane2.getStop()));
        for (int i = 0; i < bAreaLane2.getCapacity()-1 ; i++) {
