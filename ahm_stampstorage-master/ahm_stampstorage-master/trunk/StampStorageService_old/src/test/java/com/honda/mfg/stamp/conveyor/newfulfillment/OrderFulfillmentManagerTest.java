...
public class OrderFulfillmentManagerTest extends AbstractTestBase {
    ...
    public void successfullyRunOrderFulfillmentManager() {
        ...
        OrderFulfillmentManager manager2 = new OrderFulfillmentManager(orderMgr2, storage, helper, releaseManager, "fulfillmentCarrierReleaseCount", "recirculationCarrierReleaseCount");
    ...
    public void successfullyRunOrderFulfillmentManagerWhenStorageStateIsStale() {
        ...
        when(fulfillmentHelper.getActiveOrder((OrderMgr) any())).thenReturn(new WeldOrder()).thenReturn(null).thenThrow(new RuntimeException());
        OrderFulfillmentManager manager = new OrderFulfillmentManager((OrderMgr) any(), storage, fulfillmentHelper, releaseManager, "fulfillmentCarrierReleaseCount", "recirculationCarrierReleaseCount");
    ...
    public void successfullyReceiveStaleDataMessage() {
        ...
        ReleaseManager releaseManager = mock(ReleaseManager.class);
        NewFulfillmentHelper fulfillmentHelper = mock(NewFulfillmentHelper.class);
        when(fulfillmentHelper.getActiveOrder((OrderMgr) any())).thenReturn(weldOrder).thenThrow(new RuntimeException());
        OrderFulfillmentManager manager = new OrderFulfillmentManager((OrderMgr) any(), storage, fulfillmentHelper, releaseManager, "fulfillmentCarrierReleaseCount", "recirculationCarrierReleaseCount");
    ...
    //@Test
    public void successfullyDeliverCarriers() {
        WeldOrder weldOrder = new WeldOrder();
        weldOrder.setOrderStatus(OrderStatus.DeliveringCarriers);
        weldOrder.setDeliveryStatus(OrderStatus.Initialized);
        Storage storage = mock(Storage.class);
        StorageState storageState = mock(StorageState.class);
        when(storage.getStorageState()).thenReturn(storageState);
        ...
        when(fulfillmentHelper.getMinCycleCountForOrderForFulfillmentStatus(weldOrder, CarrierFulfillmentStatus.RETRIEVED)).thenReturn(0).thenReturn(0).thenReturn(1).thenReturn(1);
        when(fulfillmentHelper.getMinCycleCountForOrderForFulfillmentStatus(weldOrder, CarrierFulfillmentStatus.RELEASED)).thenReturn(1).thenReturn(0).thenReturn(1).thenReturn(0);
        OrderFulfillmentManager manager = new OrderFulfillmentManager((OrderMgr) any(), storage, fulfillmentHelper, releaseManager, "fulfillmentCarrierReleaseCount", "recirculationCarrierReleaseCount");
...
//        fulfillment2.setCurrentLocation(stop24);
//        fulfillment2.setDestination(stop24);
...
//        fulfillment4.persist();
        ...
        CarrierMes carrier2 = getCarrierMes(oneDayOld, new Integer(103), stop3, stop3, CarrierStatus.SHIPPABLE, new Integer(200), someDie1);
        CarrierMes carrier3 = getCarrierMes(oneDayOld, new Integer(103), stop4, stop4, CarrierStatus.SHIPPABLE, new Integer(202), someDie1);
