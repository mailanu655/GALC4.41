...
public class StorageStateUpdateServiceImpl implements StorageStateUpdateService {
    ...
    public StorageStateUpdateServiceImpl(Storage storage, ReleaseManager releaseManager, AlarmServiceHelper alarmServiceHelper) {
    ...
    public void saveCarrier(Carrier carrier) {
        ...
        if (carrier.getSubmitToReleaseManager() && carrierMes.getBuffer().intValue() == 0 && !carrierMes.getDestination().equals(carrier.getDestination().getId()) && !carrierMes.getCurrentLocation().equals(carrier.getDestination().getId())) {
            ...
            CarrierRelease.saveCarrierRelease(carrierNumber, carrier.getCurrentLocation(), carrier.getDestination(), carrier.getSource());
        } else {
...
//                count++;
    ...
    public void releaseCarriers(Stop rowStop, int count, Stop destination, String source) {
        ...
        if (rowStop.getStopArea().equals(StopArea.Q_WELD_LINE_1) || rowStop.getStopArea().equals(StopArea.Q_WELD_LINE_2)) {
            List<CarrierMes> carriers = CarrierMes.findAllCarriersWithCurrentLocation(rowStop.getId());
            for (CarrierMes carrier : carriers) {
                if (i < count) {
                    releaseManager.releaseCarrier(carrier.getCarrierNumber(), destination, StorageConfig.OHCV_APP_MANUAL_ORDER + "-" + source, true);
                } else {
                    break;
                }
                i++;
                            ...
                            try {
                                ...
                                if (carrierMes != null) {
                                    if (carrierMes.getCurrentLocation().equals(lane.getStop().getId())) {
                                        releaseManager.releaseCarrier(carrierNo, destination, StorageConfig.OHCV_APP_MANUAL_ORDER + "-" + source, true);
                                        storage.getStorageState().releaseCarrierIfExistsAtHeadOfLane(carrier);
                                    } else {
                                        LOG.info("carrier -" + carrierNo + " is not in the row " + lane.getRowName() + " yet ");
                                    }
                                }
                            } catch (Exception e) {
    ...
    public void releaseEmptyCarriersFromRows(StorageArea area, boolean releaseMgr, String source) {
        //PartProductionVolume volume = StorageConfig.getStorageAreaPartProductionVolumeMap().get(area);
        Stop emptyCarrierDeliveryStop = null;

        try {
            if (area.equals(StorageArea.B_AREA)) {
                List<Stop> stops = Stop.findAllStopsByTypeAndArea(StopType.EMPTY_CARRIER_DELIVERY, StopArea.B_AREA);
                ...
                if (lane != null) {
                    ...
                    while (i < count) {
                        if (!lane.isEmpty()) {
                            ...
                            CarrierMes carrierMes = CarrierMes.findCarrierByCarrierNumber(carrierNo);
                            if (carrierMes != null) {
                                if (carrierMes.getCurrentLocation().equals(lane.getStop().getId()) && carrierMes.getDieNumber().equals(Integer.valueOf(999))) {
                                    releaseManager.releaseCarrier(carrier.getCarrierNumber(), emptyCarrierDeliveryStop, StorageConfig.OHCV_APP_MANUAL_EMPTY + "-" + source, releaseMgr);
                                    storage.getStorageState().releaseCarrierIfExistsAtHeadOfLane(carrier);
                                } else {
                                    LOG.info("carrier -" + carrierNo + " is not in the row " + lane.getRowName() + " yet or is not an Empty Carrier ");
                                    break;
                                }
                            }
                        } else {
    ...
    public void reloadStorageState() {
        ...
        storage.getStorageState().setStale(true);
        storage.reloadStorageState();
        storage.getStorageState().setStale(false);
        ...
        try {
            if (!isQpcConnected()) {  //disconnected, or MesUninitialized
                ...
                if (alarmEvents == null || alarmEvents.size() == 0) {
                    ...
                    LOG.debug("StorageStateUpdateService#setConnectionState(): uncleared MES Unhealthy alarm found - not inserting");
                }
                ...
                if (alarmEvents.size() == 0) {
                    ...
                    LOG.debug("StorageStateUpdateService#setConnectionState(): uncleared MES Unhealthy alarm found - clearing and archiving");
