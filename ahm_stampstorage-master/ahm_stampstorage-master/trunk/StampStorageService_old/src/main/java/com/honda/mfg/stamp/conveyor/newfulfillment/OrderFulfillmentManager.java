...
public class OrderFulfillmentManager implements Fulfillment {
        ...
        try {
            ...
            if (order != null) {
                if (!storage.getStorageState().isStale()) {
                    ...
                    newFulfillmentHelper.saveWeldOrderComments(order, "Storage state stale");
                }
            } else {
                LOG.info("No Active Order found");
            }
        } catch (Exception e) {
            ...
            if (weldOrder.getOrderStatus().equals(OrderStatus.InProcess)) {
                if (allCarriersRetrieved(weldOrder)) {
                    ...
                    if (releaseCycle.intValue() == 0) {
                        ...
                        newFulfillmentHelper.saveWeldOrderComments(weldOrder, "waiting for carriers to queue");
                    }
                } else {
                    Integer releaseCycle = newFulfillmentHelper.getMinCycleCountForOrderForFulfillmentStatus(weldOrder, CarrierFulfillmentStatus.RELEASED);
                    ...
                    if (releaseCycle == 0 && cycle == 0 && retrievalCycle == 0) {
                        newFulfillmentHelper.saveWeldOrderComments(weldOrder, "Retrieving Carriers");
                        ...
                        LOG.info("Waiting for carriers to enter queuerows for Order-" + weldOrder.getId());
                        newFulfillmentHelper.saveWeldOrderComments(weldOrder, "waiting for carriers to enter queue rows");
                    }
                }
            }
        } else {
            newFulfillmentHelper.saveWeldOrderComments(weldOrder, "Another order selecting carriers");
        }
    }
        ...
        if (order.getOrderStatus().equals(OrderStatus.RetrievingCarriers)) {
           ...
           // if (retrievalCycle.intValue() == 0) {
                if (cycle.intValue() != 0) {
                    ...
                    newFulfillmentHelper.saveWeldOrderComments(order, "Releasing Carriers");
                    ...
                    newFulfillmentHelper.saveWeldOrderStatusAndComments(order, OrderStatus.InProcess, "No More cycles to Release");
                }
           /* } else {
                LOG.info("Waiting for carriers to release for Order-" + order.getId());
                newFulfillmentHelper.saveWeldOrderComments(order, "waiting for carriers to release");
            } */
        }
    }
        ...
        try {
            if (msg != null && msg.isStale()) {
                ...
                if (order != null && order.getOrderStatus().equals(OrderStatus.RetrievingCarriers)) {
                    ...
                    newFulfillmentHelper.saveWeldOrderStatusAndComments(order, OrderStatus.InProcess, "StorageState Stale");
                }
            }
        } catch (Exception e) {
    ...
    public void reOrderCarriers(ReloadStorageStateMessage msg) {
        ...
        if (order != null && order.getOrderStatus().equals(OrderStatus.RetrievingCarriers)) {
            newFulfillmentHelper.deleteAllUnReleasedOrderFulfillments(order);
            newFulfillmentHelper.saveWeldOrderStatusAndComments(order, OrderStatus.InProcess, msg.getCurrentLocation().getName() + " Out Of Order");
        }
        storage.reloadStorageState();
    }
}
