                        ...
                        if (newFulfillmentHelper.isSpaceAvailableToDeliver(orderMgr, deliveryCarrierReleaseCount)) {
                            if (newFulfillmentHelper.noOrderWithDeliveryStatus(OrderStatus.DeliveringCarriers)) {
                                ...
                                if (queuesHveReqCntOfCarriersWithCorrectDieToDeliver(orderMgr, order, deliveryCarrierReleaseCount)) {
                                    ...
                                    getCarriersToDeliverFromQueueRow(order, orderMgr.getLeftQueueStop(), order.getLeftDie(), order.getRemainingLeftQuantity(), deliveryCarrierReleaseCount);
                                    getCarriersToDeliverFromQueueRow(order, orderMgr.getRightQueueStop(), order.getRightDie(), order.getRemainingRightQuantity(), deliveryCarrierReleaseCount);
                                    ...
                                    if (selectedToDeliver.size() > 0 || readyToDeliver.size() > 0) {
                                        newFulfillmentHelper.saveWeldDeliveryStatusAndComments(order, OrderStatus.DeliveringCarriers, "Delivering Carriers");
                                    }
                                } else {
                                    LOG.info("Cannot Deliver Carriers from Queue ");

                                    newFulfillmentHelper.saveWeldDeliveryStatusAndComments(order, OrderStatus.InProcess, "Cannot deliver carriers now - Not enough Carriers in Queue to Deliver");
                                }
                            } else {
                                LOG.info("Another Order Delivering Carriers ");
                                newFulfillmentHelper.saveWeldDeliveryStatusAndComments(order, OrderStatus.InProcess, "Cannot deliver carriers now - Another Order Delivering Carriers ");
                            }
                        } else {
                            LOG.info("No Space Available to deliver Carriers ");

                            newFulfillmentHelper.saveWeldDeliveryStatusAndComments(order, OrderStatus.InProcess, "Cannot deliver carriers now - No Space Available to deliver Carriers");
                        }
...
/**Used when carriers are left over in the queue */
            ...
            for (int i = 0; i < queuedCarriers.size(); i++) {
                if (partQty < remainingQty) {
                    if (carrierHasDie(queuedCarriers.get(i), die)) {
                        ...
                        if (!isCarrierPartOfActiveOrder(order, queuedCarriers.get(i))) {
    ...
    private boolean isCarrierPartOfActiveOrder(WeldOrder order, CarrierMes carrierMes) {
        List<OrderFulfillment> fulfillments = newFulfillmentHelper.getAllOrderFulfillmentsByOrder(order);

        for (OrderFulfillment fulfillment : fulfillments) {
            if (fulfillment.getId().getCarrierNumber().equals(carrierMes.getCarrierNumber())) {
                return true;
            }
        }
        return false;
...
/**Check the quantity of the product on carriers with correct die */
    ...
    private void deliverCarriers(WeldOrder order) {
        ...
        if (readyToDeliver.size() == 0) {
            ...
            newFulfillmentHelper.saveWeldOrderComments(order, "waiting for carriers to be delivered");
        }
    }
    ...
    public void getCarriersToDeliverFromQueueRow(WeldOrder order, Stop queueStop, Die die, Integer remainingQty, int deliveryCycleSize) {
        List<CarrierMes> queuedCarriers = CarrierMes.findAllCarriersWithCurrentLocation(queueStop.getId());
        int partQty = 0;
        if (queuedCarriers.size() > 0) {
