...
public class StampServiceSocketConnection implements StampServiceSocketConnectionInterface, Runnable
{
    ...
    /**
     * boolean to control termination of run() loop
     * call {@link #setDone(boolean)} to end processing
     */
    private volatile boolean done = false;
	/**
	 * Manages socket connection on a separate thread; calls {@link MessageTransport}
	 * for reading and writing from/to data streams
	 * @param clientSocket
	 * @param props
	 */
	public StampServiceSocketConnection(Socket clientSocket, Properties props)
	{
		try
		{
			...
			socketTimeOut = Integer.valueOf(this.props.getProperty(SOCKET_TIMEOUT_PROPERTY_KEY, String.valueOf(socketTimeOut))).intValue();
			LOG.info("StampServiceSocketConnection.Init(): clientSocketTime out is: "+ socketTimeOut);
			maxNumSocketRetries = Integer.valueOf(this.props.getProperty(MAX_NUM_CLIENT_SOCKET_TIMEOUT_ON_A_READ, String.valueOf(maxNumSocketRetries))).intValue();
	...
	public void processOutput(ConnectionMessage msg)
		throws Exception
	{
		try
		{
	...
	/**
	 * this method called from {@link #run()} and initiates processing
	 * of data on the input stream.  Tries nto reconnect a specified number of times
	 * before terminating thread
		...
		throws Exception
	{
		...
		/**
		*added a conditional while loop so that upon recieving an exception
		*we  set a flag and send a ping and wait for a response from client.
		*If client does not respond within the so-timeout then close the socket.
		*/
		while(!done)
		{
			...
			catch(InterruptedIOException ioEx)	//SocketTimeoutException
			{
				/**
					...
					*/
					...
					LOG.error("StampServiceSocketConnection.ServiceClient():MaxNumClientSocketTimesOut: " + maxNumSocketRetries );//first time read times out
					if((numRetries < maxNumSocketRetries ) )//first time read times out
					{
							LOG.error("StampServiceSocketConnection.serviceClient():InterruptedIOException:" + numRetries +" Timeout" + ioEx.getMessage());
							done = false;
							LOG.error("StampServiceSocketConnection.serviceClient():InterruptedIOException: Send heart beat." + ioEx.getMessage());
							processOutput(new PingMessage());
					}
					else if(numRetries == maxNumSocketRetries)//second time read times out
					{
							LOG.error("StampServiceSocketConnection.ServiceClient():InterruptedIOException:" + numRetries +" timeout:" +ioEx.getMessage());
							LOG.error("StampServiceSocketConnection.ServiceClient():InterruptedIOException: Close Client Socket:" +ioEx.getMessage());
							done = true;
					}
			}
			catch(Exception e)
	...
	/**
	 * sets message sink in {@link com.honda.mfg.stamp.storage.service.utils.MessageTransport}
	 * @param messageSink
	 */
