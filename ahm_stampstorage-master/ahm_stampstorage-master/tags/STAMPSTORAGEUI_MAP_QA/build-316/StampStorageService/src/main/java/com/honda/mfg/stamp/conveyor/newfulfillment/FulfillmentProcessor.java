...
/**Carrier at Release check stop and update carrier fulfillment status */
    ...
    public void carrierStatusMessageListener(CarrierStatusMessage statusMessage) {
        ...
        if (!newFulfillmentHelper.isEmpty(statusMessage.getDie())) {
            WeldOrder order = OrderFulfillment.findFulfillingOrderByCarrierForFulfillment(carrierNumber, statusMessage.getDie());
            ...
            if (order != null) {
                ...
                if (!order.getOrderStatus().equals(OrderStatus.ManuallyCompleted) || !order.getDeliveryStatus().equals(OrderStatus.ManuallyCompleted)) {
                    ...
                    if (fulfillment != null) {
                        ...
                        if ((StopType.RELEASE_CHECK.equals(stop.getStopType()) || StopType.RECIRC_TO_ALL_ROWS.equals(stop.getStopType()))
                                && fulfillment.getCarrierFulfillmentStatus().equals(CarrierFulfillmentStatus.RETRIEVED)) {
                            LOG.info(carrierText + " Released ");

                            fulfillment.setCarrierFulfillmentStatus(CarrierFulfillmentStatus.RELEASED);
                        }

                        // 2013-06-13:VB: , removed references to ST13 and ST13-1, not needed for expansion
                        if (stop.getStopArea().equals(StopArea.Q_WELD_LINE_1) || stop.getStopArea().equals(StopArea.Q_WELD_LINE_2)) {
                            if (!fulfillment.isStatus(CarrierFulfillmentStatus.QUEUED) && !(fulfillment.isStatus(CarrierFulfillmentStatus.READY_TO_DELIVER)) && !(fulfillment.isStatus(CarrierFulfillmentStatus.DELIVERED)) && !(fulfillment.isStatus(CarrierFulfillmentStatus.SELECTED_TO_DELIVER)) ) {
    ...
    public void deliveryCarrierStatusMessageListener(CarrierStatusMessage statusMessage) {
        ...
        if (stop != null && !newFulfillmentHelper.isEmpty(statusMessage.getDie()) && stop.equals(inspectionStop)) {
            ...
            WeldOrder order = OrderFulfillment.findDeliveringOrderByCarrierForFulfillment(carrierNumber, statusMessage.getDie());
            Integer cycle = OrderFulfillment.getMaxCycleCountForOrderByCarrier(order, carrierNumber);
            if (order != null) {
                LOG.debug("carrier is part of Order - " + order.getId());
                if (!order.getDeliveryStatus().equals(OrderStatus.ManuallyCompleted)) {
                    ...
                    if (fulfillment != null) {
                        ...
                        if (carrierFulfillStatus.equals(CarrierFulfillmentStatus.READY_TO_DELIVER) || carrierFulfillStatus.equals(CarrierFulfillmentStatus.DELIVERED) || carrierFulfillStatus.equals(CarrierFulfillmentStatus.SELECTED_TO_DELIVER)) {
                            ...
                            if (!fulfillment.getDestination().equals(deliveryDestination) && destination.equals(inspectionStop)) {
                                if (statusMessage.getCarrierStatus().equals(CarrierStatus.SHIPPABLE)) {
                                    LOG.info(carrierText + " is Shippable updating destination to- " + deliveryDestination);
                                    ...
                                    DieType dieType  = (order.getLeftDie().equals(statusMessage.getDie()))?DieType.LEFT:DieType.RIGHT;
                                    newFulfillmentHelper.saveWeldOrderComments(order, carrierText + " is Not Shippable Waiting for Inspector to Update ", dieType,CommentCategory.DELIVERY);
                                }
                            } else {
