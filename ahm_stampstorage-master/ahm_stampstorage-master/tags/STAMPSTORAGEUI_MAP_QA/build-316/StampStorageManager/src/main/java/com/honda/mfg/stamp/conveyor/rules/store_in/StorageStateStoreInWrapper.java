...
public class StorageStateStoreInWrapper {
    ...
    public StorageRow getAppropriatePartialDifferentDieRowInStorageArea(Long die, StorageArea area){
		...
		List<StorageRow> candidateRows = getStorageState().queryForRows(
    			allOf(
    					RowMatchers.isLaneAvailable(),
    					RowMatchers.inStorageArea(area),
    					RowMatchers.isCurrentCapacityPartial(),
    					RowMatchers.isLaneWithSingleDieNumberNotOf(die)
    					)			
    			);
    	if(candidateRows!=null && candidateRows.size()>0){
    		if(candidateRows.size()==1)appropriateRow=candidateRows.get(0);
    		else{
