 ...
 * Date: 3/11/11
 */
public class RetrieveCarrierFromFullLanesForZonePriorityFourRuleTest {
    ...
    public void successfullyRetrieveCarriersWithHighVolumeParts() {
        ...
        StoreOutRule storeOutRule =
                new RetrieveCarrierFromRowOfZonePriorityAndRowCondition(context, null, LaneCondition.FULL, StoragePriority.Priority.FOUR);
    ...
    public void successfullyRetrieveEmptyCarriers() {
        ...
        Die part = new Die(999L, PartProductionVolume.EMPTY);
        //test
        StorageRow StorageRow = storeOutRule.processRule(part);
        ...
        assertNotNull(StorageRow);
        Carrier carrier = StorageRow.release();
        assertEquals(new Long(999L), carrier.getDie().getId());
    ...
    private List<StorageRow> getLanes() {
        ...
        aAreaLane3.store(getMediumVolumeCarrier(OM.PART_A, OM.PRODUCTION_RUN_NUMBER, OM.NEW_PRODUCTION_RUN_TIMESTAMP,aAreaLane3.getStop(),aAreaLane3.getStop()));
        for (int i = 0; i < aAreaLane4.getCapacity(); i++) {
        ...
        for (int i = 0; i < cHighLane3.getCapacity() ; i++) {
        ...
        //cHighLane4.store(getMediumVolumeCarrier(OM.PART_B, OM.PRODUCTION_RUN_NUMBER, OM.NEW_PRODUCTION_RUN_TIMESTAMP, cHighLane3.getStop(),cHighLane3.getStop()));
        for (int i = 0; i < cHighLane4.getCapacity() ; i++) {
