 ...
 * Date: 3/11/11
 */
public class RetrieveCarrierFromMixedFrontLanesForZonePriorityFourRuleTest {
    ...
    public void successfullyRetrieveCarriersWithHighVolumeParts() {
        ...
        StorageStateContext context = getContext();
                StoreOutRule storeOutRule =
                        new RetrieveCarrierFromRowOfZonePriorityAndRowCondition(context, null, LaneCondition.MIXED_FRONT, StoragePriority.Priority.FOUR);
    ...
    @Test
       public void successfullyRetrieveEmptyCarriers() {
           ...
           assertEquals(StorageRow.getCapacity(), OM.C_HIGH_CAPACITY);
...
//    }
    ...
    List<StorageRow> getMixedFrontLanes() {
        ...
        //low volume capacity
        for (int i = 0; i < aAreaLane1.getCapacity()-2; i++) {
            aAreaLane1.store(getEmptyCarrier(OM.EMPTY,999,OM.OLD_PRODUCTION_RUN_TIMESTAMP,aAreaLane1.getStop(),aAreaLane1.getStop()));
        }
        for (int i = 0; i < 2; i++) {
            aAreaLane1.store(getMediumVolumeCarrier(OM.PART_B,OM.OLD_PRODUCTION_RUN_NUMBER,OM.OLD_PRODUCTION_RUN_TIMESTAMP,aAreaLane1.getStop(),aAreaLane1.getStop()));
        }

        for (int i = 0; i < aAreaLane2.getCapacity(); i++) {
            aAreaLane2.store(getMediumVolumeCarrier(OM.PART_B, OM.OLD_PRODUCTION_RUN_NUMBER, OM.OLD_PRODUCTION_RUN_TIMESTAMP,aAreaLane2.getStop(),aAreaLane2.getStop()));
        }

        for (int i = 0; i < aAreaLane3.getCapacity() - 1; i++) {
            aAreaLane3.store(getMediumVolumeCarrier(OM.PART_B, OM.OLD_PRODUCTION_RUN_NUMBER, OM.OLD_PRODUCTION_RUN_TIMESTAMP,aAreaLane3.getStop(),aAreaLane3.getStop()));
        }
        aAreaLane3.store(getHighVolumeCarrier(OM.PART_A, OM.PRODUCTION_RUN_NUMBER, OM.NEW_PRODUCTION_RUN_TIMESTAMP,aAreaLane3.getStop(),aAreaLane3.getStop()));
        for (int i = 0; i < aAreaLane4.getCapacity()-3; i++) {
            aAreaLane4.store(getLowVolumeCarrier(OM.PART_C, OM.OLD_PRODUCTION_RUN_NUMBER, OM.OLD_PRODUCTION_RUN_TIMESTAMP,aAreaLane4.getStop(),aAreaLane4.getStop()));
        }
        for (int i = 0; i < 3; i++) {
            aAreaLane4.store(getLowVolumeCarrier(OM.PART_B, OM.OLD_PRODUCTION_RUN_NUMBER, OM.OLD_PRODUCTION_RUN_TIMESTAMP,aAreaLane4.getStop(),aAreaLane4.getStop()));
        }
        //medium volume capacity
        for (int i = 0; i < cHighLane1.getCapacity(); i++) {
            cHighLane1.store(getHighVolumeCarrier(OM.PART_A, OM.PRODUCTION_RUN_NUMBER, OM.NEW_PRODUCTION_RUN_TIMESTAMP, cHighLane1.getStop(),cHighLane1.getStop()));
        }
        ...
        for (int i = 0; i < cHighLane3.getCapacity()-1 ; i++) {
            cHighLane3.store(getHighVolumeCarrier(OM.PART_A, OM.OLD_PRODUCTION_RUN_NUMBER, OM.OLD_PRODUCTION_RUN_TIMESTAMP, cHighLane3.getStop(),cHighLane3.getStop()));
        }
        cHighLane3.store(getMediumVolumeCarrier(OM.PART_B, OM.PRODUCTION_RUN_NUMBER, OM.NEW_PRODUCTION_RUN_TIMESTAMP, cHighLane3.getStop(),cHighLane3.getStop()));
        for (int i = 0; i < cHighLane4.getCapacity()-2 ; i++) {
            cHighLane4.store(getEmptyCarrier(OM.EMPTY, OM.OTHER_PRODUCTION_RUN_NUMBER, OM.OLD_PRODUCTION_RUN_TIMESTAMP, cHighLane4.getStop(),cHighLane4.getStop()));
        }
        for (int i = 0; i < 2 ; i++) {
            cHighLane4.store(getMediumVolumeCarrier(OM.PART_B, OM.OTHER_PRODUCTION_RUN_NUMBER, OM.OLD_PRODUCTION_RUN_TIMESTAMP, cHighLane4.getStop(),cHighLane4.getStop()));
        }
        //high volume capacity
        for (int i = 0; i < cLowLane1.getCapacity(); i++) {
            cLowLane1.store(getHighVolumeCarrier(OM.PART_A, OM.PRODUCTION_RUN_NUMBER, OM.NEW_PRODUCTION_RUN_TIMESTAMP, cLowLane1.getStop(), cLowLane1.getStop()));
        }

        for (int i = 0; i < cLowLane2.getCapacity(); i++) {
            cLowLane2.store(getHighVolumeCarrier(OM.PART_A, OM.OLD_PRODUCTION_RUN_NUMBER, OM.OLD_PRODUCTION_RUN_TIMESTAMP,cLowLane2.getStop(), cLowLane2.getStop()));
        }
        ...
        cLowLane3.store(getMediumVolumeCarrier(OM.PART_B, OM.PRODUCTION_RUN_NUMBER, OM.NEW_PRODUCTION_RUN_TIMESTAMP,cLowLane3.getStop(), cLowLane3.getStop()));
        for (int i = 0; i < cLowLane4.getCapacity() - 1; i++) {
            cLowLane4.store(getHighVolumeCarrier(OM.PART_A, OM.OTHER_PRODUCTION_RUN_NUMBER, OM.OLD_PRODUCTION_RUN_TIMESTAMP,cLowLane4.getStop(), cLowLane4.getStop()));
        }

        //high volume capacity
        for (int i = 0; i < bAreaLane1.getCapacity().intValue(); i++) {
            bAreaLane1.store(getMediumVolumeCarrier(OM.PART_B, OM.PRODUCTION_RUN_NUMBER, OM.NEW_PRODUCTION_RUN_TIMESTAMP, bAreaLane1.getStop(), bAreaLane1.getStop()));
        }
