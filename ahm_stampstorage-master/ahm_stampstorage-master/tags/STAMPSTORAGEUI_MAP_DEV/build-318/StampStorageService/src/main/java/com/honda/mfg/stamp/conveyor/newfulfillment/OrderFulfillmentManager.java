...
public class OrderFulfillmentManager implements Fulfillment {
    ...
    public OrderFulfillmentManager(OrderMgr orderMgr, Storage storage, NewFulfillmentHelper newFulfillmentHelper, ReleaseManager releaseManager, String fulfillmentCycleSize, String recirculationCarrierReleaseCount) {
        ...
        try {
            ...
            if (order != null) {
                if (!storage.getStorageState().isStale()) {
                    ...
                    switch (orderStatus) {
                        case InProcess:
                            newFulfillmentHelper.saveWeldOrderComments(order, "None." , DieType.BOTH,CommentCategory.FULFILLMENT);
                    ...
                    newFulfillmentHelper.saveWeldOrderComments(order, "Storage state stale", DieType.BOTH,CommentCategory.FULFILLMENT);
                }
            } else {
    ...
    private void retrieveCarriers(WeldOrder weldOrder, String fulfillmentCycleSize, String recirculationCarrierReleaseCount) {
        if (newFulfillmentHelper.noOrderMgrWithOrderStatus(OrderStatus.RetrievingCarriers)) {
            if (weldOrder.getOrderStatus().equals(OrderStatus.InProcess)) {
                if (allCarriersRetrieved(weldOrder)) {
                    Integer releaseCycle = newFulfillmentHelper.getMinCycleCountForOrderForFulfillmentStatus(weldOrder, CarrierFulfillmentStatus.RELEASED);
                    Integer retrieveCycle = newFulfillmentHelper.getMinCycleCountForOrderForFulfillmentStatus(weldOrder, CarrierFulfillmentStatus.RETRIEVED);
                    if (releaseCycle.intValue() == 0 && retrieveCycle.intValue() == 0) {
                        LOG.info("No More carriers required to fulfill updating OrderStatus to AutoComplete for Order-" + weldOrder.getId());
                        ...
                        //newFulfillmentHelper.saveWeldOrderComments(weldOrder, "Waiting for carriers to queue", DieType.BOTH,CommentCategory.FULFILLMENT);
                    }
                } else {
                    Integer releaseCycle = newFulfillmentHelper.getMinCycleCountForOrderForFulfillmentStatus(weldOrder, CarrierFulfillmentStatus.RELEASED);
                    Integer cycle = newFulfillmentHelper.getMinCycleCountForOrderForFulfillmentStatus(weldOrder, CarrierFulfillmentStatus.SELECTED);
                    Integer retrievalCycle = newFulfillmentHelper.getMinCycleCountForOrderForFulfillmentStatus(weldOrder, CarrierFulfillmentStatus.RETRIEVED);
                    if (releaseCycle == 0 && cycle == 0 && retrievalCycle == 0) {
                       // newFulfillmentHelper.saveWeldOrderComments(weldOrder, "Retrieving Carriers",DieType.BOTH,CommentCategory.FULFILLMENT);
                        
                        RetrieveCycleManager retrieveCycleManager = new RetrieveCycleManager(storage, fulfillmentCycleSize, recirculationCarrierReleaseCount);
                       ...
                       // newFulfillmentHelper.saveWeldOrderComments(weldOrder, "Waiting for carriers to enter queue rows", DieType.BOTH, CommentCategory.FULFILLMENT);
                    }	
                }
            }
        } else {
           // newFulfillmentHelper.saveWeldOrderComments(weldOrder, "Another order selecting carriers", DieType.BOTH, CommentCategory.FULFILLMENT);
        }
    }
    ...
    private boolean allCarriersRetrieved(WeldOrder order) {
        ...
        if (orderFulfillments != null) {
            ...
            LOG.info(" left orderedQty - " + leftOrderedQty.intValue() + " right orderedQty - " + rightOrderedQty.intValue());
            if (leftOrderedQty.intValue() >= order.getLeftQuantity().intValue() && rightOrderedQty.intValue() >= order.getRightQuantity().intValue()) {
                flag = true;
            }
            //2014-04-02:VB:Note: We are not counting left over carriers queued for previous orders here;
            //Therefore the fulfillment will keep running until the delivery manager starts and makes
    ...
    /**
     * update the count of carriers already queued for this order,
     * including those in DELIVERED and CONSUMED status
     *
     * @param order
     */
    private WeldOrder updateQueuedQty(WeldOrder order) {
        ...
        if (orderFulfillments != null) {
            ...
            if (activeDeliveryOrder != null && activeDeliveryOrder.getId().equals(order.getId())) {
                int lQty = getProductQtyInQueueNotPartOfThisOrder(order, order.getLeftDie(), order.getOrderMgr().getLeftQueueStop());
                int rQty = getProductQtyInQueueNotPartOfThisOrder(order, order.getRightDie(), order.getOrderMgr().getRightQueueStop());
    ...
    private void releaseCarriers(WeldOrder order) {
    	...
    	//TODO redo this logic only keep the part that releases carriers.  ie  if (cycle.intValue() != 0)
    	
        if (order.getOrderStatus().equals(OrderStatus.RetrievingCarriers)) {
            Integer cycle = newFulfillmentHelper.getMinCycleCountForOrderForFulfillmentStatus(order, CarrierFulfillmentStatus.SELECTED);
            Integer retrievalCycle = newFulfillmentHelper.getMinCycleCountForOrderForFulfillmentStatus(order, CarrierFulfillmentStatus.RETRIEVED);
            /*
                * (selected)cycle - lowest cycle with SELECTED status
                * retrievalCycle - lowest cycle with RETRIEVED status
                * if(retrievalCycle != 0)  -->there is at least 1 carrier with RETRIEVED status
                *   do nothing/exit
                * else (implies no carriers with RETRIEVED status)
                *   if(there are SELECTED carriers)
                *     release carriers
                *   else  (implies no SELECTED or RETRIEVED carriers
                *     set the order to InProcess
                */
            ...
            if (cycle.intValue() != 0) {
                ...
                countLeft = newFulfillmentHelper.getAllOrderFulfillmentsByCarrierFulfillmentStatusAndDie(order, CarrierFulfillmentStatus.SELECTED, order.getLeftDie()).size();
                countRight = newFulfillmentHelper.getAllOrderFulfillmentsByCarrierFulfillmentStatusAndDie(order, CarrierFulfillmentStatus.SELECTED, order.getRightDie()).size();
               dieType = null;
                ...
                if (dieType!=null){
                    newFulfillmentHelper.saveWeldOrderComments(order, "Carriers Selected and Releasing from Rows", dieType, CommentCategory.FULFILLMENT);
                }
                ...
                if (retrievalCycle.intValue() == 0) {
                   ...
                   newFulfillmentHelper.saveWeldOrderStatusAndComments(order, OrderStatus.InProcess, "None.", DieType.BOTH);
                } else {
                    LOG.info("Waiting for carriers to release for Order-" + order.getId());
  
                    int countLeft, countRight;
                    countLeft = newFulfillmentHelper.getAllOrderFulfillmentsByCarrierFulfillmentStatusAndDie(order, CarrierFulfillmentStatus.RETRIEVED, order.getLeftDie()).size();
                    countRight = newFulfillmentHelper.getAllOrderFulfillmentsByCarrierFulfillmentStatusAndDie(order, CarrierFulfillmentStatus.RETRIEVED, order.getRightDie()).size();
                    countLeft = countLeft + newFulfillmentHelper.getAllOrderFulfillmentsByCarrierFulfillmentStatusAndDie(order, CarrierFulfillmentStatus.RELEASED, order.getLeftDie()).size();
                    countRight = countRight + newFulfillmentHelper.getAllOrderFulfillmentsByCarrierFulfillmentStatusAndDie(order, CarrierFulfillmentStatus.RELEASED, order.getRightDie()).size();
                    
                    if (countLeft>0 && countRight>0 ){
                    }else{
                    	if(countLeft>0){
                    		   newFulfillmentHelper.saveWeldOrderComments(order, "Carriers in route to queue.", DieType.LEFT, CommentCategory.FULFILLMENT);
                    	}
                    	if(countRight>0){
                   		   newFulfillmentHelper.saveWeldOrderComments(order, "Carriers in route to queue", DieType.RIGHT, CommentCategory.FULFILLMENT);
                    	}
                    }
                 }
            }
        ...
        try {
            if (msg != null && msg.isStale()) {
                ...
                if (order != null && order.getOrderStatus().equals(OrderStatus.RetrievingCarriers)) {
                    ...
                    newFulfillmentHelper.saveWeldOrderStatusAndComments(order, OrderStatus.InProcess, "StorageState Stale", DieType.BOTH);
    ...
    public void reOrderCarriers(ReloadStorageStateMessage msg) {
        ...
        if (order != null && order.getOrderStatus().equals(OrderStatus.RetrievingCarriers)) {
            newFulfillmentHelper.deleteAllUnReleasedOrderFulfillments(order);
            newFulfillmentHelper.saveWeldOrderStatusAndComments(order, OrderStatus.InProcess, msg.getCurrentLocation().getName() + " Out Of Order", DieType.BOTH);
        }
        storage.reloadStorageState();
    }
    ...
    Integer getProductQtyInQueueNotPartOfThisOrder(WeldOrder order, Die die, Stop leftQueue) {
        ...
        List<CarrierMes> carriers = CarrierMes.findAllCarriersWithCurrentLocationAndDestinationLocationAndDie(leftQueue.getId(), dieNumber);
        Integer qty = 0;
