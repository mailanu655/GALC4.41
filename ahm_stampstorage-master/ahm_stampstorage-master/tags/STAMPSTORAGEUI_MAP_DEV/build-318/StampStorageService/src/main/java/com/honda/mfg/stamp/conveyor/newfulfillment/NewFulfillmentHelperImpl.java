...
public class NewFulfillmentHelperImpl extends AbstractHelperImpl implements NewFulfillmentHelper {
    ...
    @Override 
    public synchronized void saveWeldDeliveryStatusAndComments(WeldOrder order, OrderStatus orderStatus, String comments, DieType dieType) {
        WeldOrder activeOrder = WeldOrder.findWeldOrder(order.getId());
        if (!activeOrder.getDeliveryStatus().equals(OrderStatus.ManuallyCompleted)) {
            activeOrder.setDeliveryStatus(orderStatus);
    ...
    public boolean canClearBackOrder(Stop queueStop, Die die, String deliveryCycleSize) {
        ...
        if (count >= (deliverySize * 2)) {
             LOG.info("queue stop:"+ queueStop.getName()+"carrier count: "+count+" enough ( 2* deliveryCycleSize) to clear back Order ");
            return true;
        }

        LOG.info("queue stop:"+ queueStop.getName()+"carrier count: "+count+" Not Enough ( 2* deliveryCycleSize) to clear back Order ");
        return false;
    }
...
private synchronized void saveExtendedComments(WeldOrder order,String comments, DieType dieType, CommentCategory category){
    ...
    @Override
    public synchronized void saveWeldOrderComments(WeldOrder order, String comments, DieType dieType, CommentCategory category) {
        WeldOrder activeOrder = WeldOrder.findWeldOrder(order.getId());
        activeOrder.setComments(comments);
    ...
    @Override
    public synchronized void saveWeldOrderStatusAndComments(WeldOrder order, OrderStatus orderStatus, String comments, DieType dieType) {
        WeldOrder activeOrder = WeldOrder.findWeldOrder(order.getId());
        if (!activeOrder.getOrderStatus().equals(OrderStatus.ManuallyCompleted)) {
            activeOrder.setOrderStatus(orderStatus);
            activeOrder.setComments(comments);
    ...
    public Stop getCarrierDestinationForRecirculation(StorageRow lane, Die die) {
        ...
        if (!isEmpty(die) && lane.getStorageArea().equals(StorageArea.C_HIGH) || lane.getStorageArea().equals(StorageArea.C_LOW) ){
