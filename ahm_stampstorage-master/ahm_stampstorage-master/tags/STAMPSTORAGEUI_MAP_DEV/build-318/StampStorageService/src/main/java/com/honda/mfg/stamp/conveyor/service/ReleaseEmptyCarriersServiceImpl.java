...
public class ReleaseEmptyCarriersServiceImpl implements ReleaseEmptyCarriersService, Runnable {
    ...
    public ReleaseEmptyCarriersServiceImpl(EmptyCarriersServiceHelper orderEmptyCarriersServiceHelper, ReleaseManager releaseManager, String emptyAreaUnderFilledQuantity, String releaseCarrierCount) {
    ...
    public void releaseEmptyCarriersFromOldWeldLineToEmptyStorageArea(int carrierCount) {
        try {
            List<CarrierMes> carriers = orderEmptyCarriersServiceHelper.getCarriersToMoveToEmptyStorageArea(carrierCount);
            if (carriers.size() == 0) {
            ...
            for (CarrierMes carrierMes : carriers) {
                ...
                if (!carrierMes.getDestination().equals(emptyCarrierDeliveryStop.getId()) && orderEmptyCarriersServiceHelper.getEmptyDie().getId().equals(dieId)) {
                    LOG.debug("Moving empty carrier out of Old Weld Line to empty Storage Area");
                    releaseManager.releaseCarrier(carrierMes.getCarrierNumber(), emptyCarrierDeliveryStop, StorageConfig.OHCV_APP_AUTO_EMPTY, true);
                }
            }
        } catch (Exception e) {
