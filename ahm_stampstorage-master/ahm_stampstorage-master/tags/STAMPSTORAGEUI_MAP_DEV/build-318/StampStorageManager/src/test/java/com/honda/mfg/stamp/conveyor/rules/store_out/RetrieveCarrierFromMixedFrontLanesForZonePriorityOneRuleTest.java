    ...
    public void successfullyRetrieveCarriersWithHighVolumeParts() {
        ...
        StorageStateContext context = getContext();
                StoreOutRule storeOutRule =
                        new RetrieveCarrierFromRowOfZonePriorityAndRowCondition(context, null, LaneCondition.MIXED_FRONT, StoragePriority.Priority.ONE);
...
//    }
    ...
    List<StorageRow> getMixedFrontLanes() {
        ...
        C-High (capacity:20)
        ------
        lane1: low volume old, half lane + med vol new second half
        ...
        lane2: med volume old first half lane + low vol new second half
        ...
        S Area (capacity:87)
        ------
        lane1: high vol old first half lane + med old second half
       ...
       */
        ...
        for (int i = 0; i < aAreaLane4.getCapacity() - 3; i++) {
            aAreaLane4.store(getEmptyCarrier(OM.EMPTY, OM.PRODUCTION_RUN_NUMBER, OM.NEW_PRODUCTION_RUN_TIMESTAMP, aAreaLane4.getStop(), aAreaLane4.getStop()));
        }
        ...
        for (int i = 0; i < cHighLane1.getCapacity()/2; i++) {
            cHighLane1.store(getLowVolumeCarrier(OM.PART_C, OM.OLD_PRODUCTION_RUN_NUMBER, OM.OLD_PRODUCTION_RUN_TIMESTAMP, cHighLane1.getStop(), cHighLane1.getStop()));
        }
        ...
        for (int i = 1+(cLowLane2.getCapacity()/2); i < cLowLane2.getCapacity(); i++) {
        	cLowLane2.store(getLowVolumeCarrier(OM.PART_A, OM.PRODUCTION_RUN_NUMBER, OM.NEW_PRODUCTION_RUN_TIMESTAMP, cLowLane2.getStop(), cLowLane2.getStop()));
        }
        ...
        for (int i = 0; i < bAreaLane1.getCapacity()/2; i++) {
			bAreaLane1.store(getEmptyCarrier(OM.EMPTY, OM.PRODUCTION_RUN_NUMBER,
					OM.NEW_PRODUCTION_RUN_TIMESTAMP, bAreaLane1.getStop(),
					bAreaLane1.getStop()));
        }
