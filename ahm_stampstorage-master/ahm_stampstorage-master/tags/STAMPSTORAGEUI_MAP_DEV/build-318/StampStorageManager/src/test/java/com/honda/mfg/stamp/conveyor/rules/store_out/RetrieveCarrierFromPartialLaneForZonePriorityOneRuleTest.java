...
public class RetrieveCarrierFromPartialLaneForZonePriorityOneRuleTest {
    ...
    public void successfullyRetrieveCarriersWithHighVolumeParts() {
        ...
        StorageStateContext context = getContext();
                       StoreOutRule storeOutRule =
                        new RetrieveCarrierFromRowOfZonePriorityAndRowCondition(context, null, LaneCondition.PARTIAL, StoragePriority.Priority.ONE);
    ...
    private List<StorageRow> getLanes() {
       //same part oldest production run no and sorted in ascending by carrier count
        ...
        lane4: all low volume, new production run - last slot unfilled
        
        C-High (capacity:20)
        ------
        lane1: empty carriers 1/3 lane
        lane2: med volume, full lane, old production run
        lane3: low volume old - 1/2 lane
        lane4: all med volume, new production run, last slot unfilled

        C-Low (capacity:30)
        ------
        lane1: empties, last 2 slots unfilled
        lane2: med vol old 1/3 lane
        lane3: low volume, old production run 1/2 lane
        ...
        lane3: med volume, old production run 1/3 lane
        lane4: high volume, other production run, last slot unfilled
       ...
       */
        ...
        for (int i = 0; i < cHighLane3.getCapacity()/2; i++) {
            cHighLane3.store(getLowVolumeCarrier(OM.PART_C, OM.OLD_PRODUCTION_RUN_NUMBER, OM.OLD_PRODUCTION_RUN_TIMESTAMP, cHighLane3.getStop(), cHighLane3.getStop()));
        }
        ...
        for (int i = 0; i < bAreaLane1.getCapacity().intValue()-2; i++) {
            bAreaLane1.store(getEmptyCarrier(OM.EMPTY, OM.PRODUCTION_RUN_NUMBER, OM.NEW_PRODUCTION_RUN_TIMESTAMP, bAreaLane1.getStop(), bAreaLane1.getStop()));
        }
        ...
        for (int i = 0; i < bAreaLane3.getCapacity()/3; i++) {
            bAreaLane3.store(getMediumVolumeCarrier(OM.PART_B, OM.OLD_PRODUCTION_RUN_NUMBER, OM.OLD_PRODUCTION_RUN_TIMESTAMP, bAreaLane3.getStop(), bAreaLane3.getStop()));
        }
