...
public class RetrieveCarrierFromMixedFrontLanesForZonePriorityFiveRuleTest {
    ...
    public void successfullyRetrieveCarriersWithHighVolumeParts() {
        ...
        StoreOutRule storeOutRule =
                        new RetrieveCarrierFromRowOfZonePriorityAndRowCondition(context, null, LaneCondition.MIXED_FRONT, StoragePriority.Priority.FIVE);
...
//    }
    ...
    List<StorageRow> getMixedFrontLanes() {
        ...
        for (int i = 0; i < aAreaLane1.getCapacity().intValue()-3; i++) {
            aAreaLane1.store(getHighVolumeCarrier(OM.PART_A, OM.OLD_PRODUCTION_RUN_NUMBER, OM.OLD_PRODUCTION_RUN_TIMESTAMP, aAreaLane1.getStop(), aAreaLane1.getStop()));
        }
         for (int i = 0; i < 3; i++) {
            aAreaLane1.store(getLowVolumeCarrier(OM.PART_C, OM.OLD_PRODUCTION_RUN_NUMBER, OM.OLD_PRODUCTION_RUN_TIMESTAMP, aAreaLane1.getStop(), aAreaLane1.getStop()));
         }

        for (int i = 0; i < aAreaLane2.getCapacity().intValue(); i++) {
            aAreaLane2.store(getLowVolumeCarrier(OM.PART_C, OM.OLD_PRODUCTION_RUN_NUMBER, OM.OLD_PRODUCTION_RUN_TIMESTAMP, aAreaLane2.getStop(), aAreaLane2.getStop()));
        }
        ...
        aAreaLane3.store(getLowVolumeCarrier(OM.PART_C, OM.PRODUCTION_RUN_NUMBER, OM.NEW_PRODUCTION_RUN_TIMESTAMP, aAreaLane3.getStop(), aAreaLane3.getStop()));
        for (int i = 0; i < aAreaLane4.getCapacity().intValue() - 1; i++) {
            aAreaLane4.store(getLowVolumeCarrier(OM.PART_C, OM.OTHER_PRODUCTION_RUN_NUMBER, OM.OLD_PRODUCTION_RUN_TIMESTAMP, aAreaLane4.getStop(), aAreaLane4.getStop()));
        }
        ...
        cLowLane3.store(getMediumVolumeCarrier(OM.PART_A, OM.PRODUCTION_RUN_NUMBER, OM.NEW_PRODUCTION_RUN_TIMESTAMP, cLowLane3.getStop(), cLowLane3.getStop()));
        for (int i = 0; i < cLowLane4.getCapacity().intValue() - 1; i++) {
            cLowLane4.store(getHighVolumeCarrier(OM.PART_B, OM.OTHER_PRODUCTION_RUN_NUMBER, OM.OLD_PRODUCTION_RUN_TIMESTAMP, cLowLane4.getStop(), cLowLane4.getStop()));
        }
        ...
        for (int i = 0; i < 3; i++) {
            sAreaLane1.store(getHighVolumeCarrier(OM.PART_A, OM.OLD_PRODUCTION_RUN_NUMBER, OM.OLD_PRODUCTION_RUN_TIMESTAMP, sAreaLane1.getStop(), sAreaLane1.getStop()));
        }
        ...
        for (int i = 0; i < 4; i++) {
            sAreaLane2.store(getMediumVolumeCarrier(OM.PART_B, OM.OLD_PRODUCTION_RUN_NUMBER, OM.OLD_PRODUCTION_RUN_TIMESTAMP, sAreaLane2.getStop(), sAreaLane2.getStop()));
        }
