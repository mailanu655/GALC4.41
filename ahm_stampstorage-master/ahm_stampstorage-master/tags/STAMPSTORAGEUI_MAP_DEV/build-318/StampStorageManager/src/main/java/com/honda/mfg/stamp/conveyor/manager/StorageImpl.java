...
public class StorageImpl implements Storage {
    ...
    public StorageImpl(StoreInManager storeInManager, StoreOutManager storeOutManager,
                       EmptyManagerImpl emptyManager, StorageStateContext storageStateContext) {
    ...
    public void store(Carrier carrier) {
        ...
        if (!die.getActive()) {
        ...
        } else if ((carrier.getDie().getId().equals(999L)) && (type.equals(StopType.STORE_IN_ALL_LANES) || type.equals(StopType.REWORK))) {
            ...
            switch (type) {
                ...
                case RECIRC_TO_ALL_ROWS:
                    if ((area.equals(StopArea.STORE_IN_ROUTE) || area.equals(StopArea.STORE_OUT_ROUTE)) && storageStateContext.spaceAvailable(stop)) {
                        try {
    ...
    public void sendCarrierUpdateMessage(Carrier carrier) {
        ...
        //if carrier exists in storage state and is at head on the lane  then release carrier from lane
        if (!storageStateContext.getStorageState().isStale()) {
            ...
            if (carrierMes != null) {
                ...
                Long dieId = carrier.getDie() == null ? Long.valueOf(carrierMes.getDieNumber().toString()) : carrier.getDie().getId();
                Integer quantity = carrier.getQuantity() == null ? carrierMes.getQuantity() : carrier.getQuantity();
                CarrierStatus status = carrier.getCarrierStatus() == null ? CarrierStatus.findByType(carrierMes.getStatus()) : carrier.getCarrierStatus();

                if (!quantity.equals(carrierMes.getQuantity()) || !status.equals(CarrierStatus.findByType(carrierMes.getStatus())) || !(dieId.equals(Long.valueOf(carrierMes.getDieNumber().toString())))) {
