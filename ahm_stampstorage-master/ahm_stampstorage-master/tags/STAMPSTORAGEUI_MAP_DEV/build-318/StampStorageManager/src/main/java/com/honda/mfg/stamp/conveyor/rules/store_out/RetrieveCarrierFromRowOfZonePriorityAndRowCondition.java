...
public class RetrieveCarrierFromRowOfZonePriorityAndRowCondition extends StoreOutRuleBase {
       private static final Logger LOG = LoggerFactory.getLogger(RetrieveCarrierFromRowOfZonePriorityAndRowCondition.class);
    StoragePriority.Priority priority;
    LaneCondition laneCondition ;

    public RetrieveCarrierFromRowOfZonePriorityAndRowCondition(StorageStateContext storageStateContext, StoreOutRule nextRule, LaneCondition laneCondition, StoragePriority.Priority priority) {
    ...
    public StorageRow computeLane(Die die) {
       ...
       StorageArea area = StoragePriority.getStorageAreaByPriorityForVolume(die.getPartProductionVolume(), priority);
         List<StorageRow> rows = getStorageStateStoreOutWrapper().getStorageRowsWithOldestProductionRunPartCarriersForStorageArea(die,area);
       ...
       LOG.info("Computed Appropriate Row for Die " + ((die==null)?"null":die.getId()) + " as " + ((appropriateRow==null)?"null":appropriateRow.getRowName()) +" " + ((laneCondition==null)?"null":laneCondition.toString()));
