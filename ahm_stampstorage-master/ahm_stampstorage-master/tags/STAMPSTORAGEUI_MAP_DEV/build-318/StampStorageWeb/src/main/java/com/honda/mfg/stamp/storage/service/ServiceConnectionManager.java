...
private ApplicationContext appContext;
	...
	public void updateServiceRoles() throws IllegalBlockingModeException, SocketTimeoutException, UnknownHostException, IllegalArgumentException, SecurityException, IOException{
		ServiceRole thisRole = null;
		List serviceRoleList = ServiceRole.findServiceRolesByCurrentActiveNot(false).getResultList();
		if(serviceRoleList.size() > 0) {thisRole = (ServiceRole)serviceRoleList.get(0);}
		if(thisRole!=null){
			...
			serviceSocketFactory.setHostName(serviceRole.getIp());
			serviceSocketFactory.setPortNumber(serviceRole.getPort());	
			if(serviceSocketFactory.getCurrentSocket()!=null &&! serviceSocketFactory.getCurrentSocket().isClosed() && serviceSocketFactory.getCurrentSocket().isConnected()){
