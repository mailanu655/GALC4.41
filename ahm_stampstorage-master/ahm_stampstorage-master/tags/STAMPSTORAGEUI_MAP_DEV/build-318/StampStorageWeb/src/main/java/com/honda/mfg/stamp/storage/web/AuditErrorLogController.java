...
public class AuditErrorLogController {
   ...
   private static final int COOKIE_MAX_AGE = 900;
    ...
    public String create(@Valid AuditErrorLog auditErrorLog, BindingResult bindingResult, Model uiModel, HttpServletRequest httpServletRequest) {
    ...
    public String update(@Valid AuditErrorLog auditErrorLog,
    ...
    public String findAuditErrorLogByCriteria(
        ...
        if (nodeId.length() > 0) finderCriteria.setNodeId(nodeId);
        if (source.length() > 0) finderCriteria.setSource(source);
        if (severity.length() > 0) finderCriteria.setSeverity(Integer.parseInt(severity));
        if (messageText.length() > 0) finderCriteria.setMessageText(messageText);
        if (beginTimestamp.length() > 0) finderCriteria.setBeginTimestamp(Timestamp.valueOf(beginTimestamp));
        if (endTimestamp.length() > 0) finderCriteria.setEndTimestamp(Timestamp.valueOf(endTimestamp));
	...
	private AuditErrorLogFinderCriteria getSavedAuditErrorLogFinderCriteria(HttpServletRequest httpServletRequest) {
        ...
        if (nodeId != null || source != null || severity != null || messageText != null || beginTimestamp != null || endTimestamp != null) {
	...
	private void saveAuditErrorLogFinderCriteria(AuditErrorLogFinderCriteria finderCriteria, HttpServletResponse httpServletResponse) {
        ...
        if (finderCriteria != null) {
            ...
            String beginTimestamp = finderCriteria.getBeginTimestamp() != null ? finderCriteria.getBeginTimestamp().toString() : "";
