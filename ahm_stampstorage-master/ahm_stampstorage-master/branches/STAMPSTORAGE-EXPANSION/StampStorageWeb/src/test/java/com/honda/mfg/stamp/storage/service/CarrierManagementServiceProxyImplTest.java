...
/**
 * @author VCC44349 2013-01-30 The tests in this class have the purpose of
 *         ensuring that a ServiceRequestMessage is sent through the device
 *         wrapper to the service component. Calls are made to the
 *         CarrierManagementServiceProxy which creates the service request and
 *         wraps it in a JsonServiceWrapperMessage before publishing it, which
 *         is then picked up over EventBus by ServiceSendMessageProcessor.
 *         {@link ServiceSendMessageProcessor}
 *         {@link com.honda.mfg.stamp.storage.service.CarrierManagementServiceProxy}
 *         {@link ServiceRequestMessage} {@link JsonServiceWrapperMessage}
 */
@Configurable
...
public class CarrierManagementServiceProxyImplTest {
	...
	public void before() throws EventServiceExistsException {
		...
		if (e != null && !(e instanceof SwingEventService)) {
			...
			EventServiceLocator.setEventService(EventServiceLocator.SERVICE_NAME_EVENT_BUS, e);
		}
	}
	...
	public void testCarrierUpdateRequestMessage() {
		...
		JsonServiceWrapperMessage jsonWrapperIn = new JsonServiceWrapperMessage(carrierUpdateReqMsg,
		...
		Message thisMessage = (Message) jSonParser.parse(request, JsonServiceWrapperMessage.class); // CarrierUpdateRequestMessage.class);
	...
	public void testUpdateLaneCarrierListRequestMessage() {
		...
		ServiceRequestMessage updateLaneListReqMsg = new UpdateLaneCarrierListRequestMessage(cList, 1201L,
				CarrierUpdateOperations.ROW_RESEQUENCE);
		JsonServiceWrapperMessage jsonWrapperIn = new JsonServiceWrapperMessage(updateLaneListReqMsg,
	...
	public void testLaneUpdateRequestMessage() {
		...
		Carrier c = getCarrier(7);
		JSonResponseParser jSonParser = new JSonResponseParser();
		ServiceRequestMessage laneUpdateReqMsg = new LaneUpdateRequestMessage(7,
				CarrierUpdateOperations.REMOVE_FROM_ROW);
		JsonServiceWrapperMessage jsonWrapperIn = new JsonServiceWrapperMessage(laneUpdateReqMsg,
		...
		MessageRequest msgRequest = new ConnectionRequest(mesMsg, ConnectionMessageSeparators.DEFAULT.getSeparator());
		String request = msgRequest.getMessageRequest();
		Message thisMessage = (Message) jSonParser.parse(request, JsonServiceWrapperMessage.class); // LaneUpdateRequestMessage.class);
		JsonServiceWrapperMessage jsonWrapperOut = (JsonServiceWrapperMessage) thisMessage;
		ServiceRequestMessage serviceRequest = jsonWrapperOut.getContent();
	...
	public void testLaneUpdateRequestMessage_AddCarrier() {
		...
		Carrier c = getCarrier(7);
		JSonResponseParser jSonParser = new JSonResponseParser();
		ServiceRequestMessage laneUpdateReqMsg = new LaneUpdateRequestMessage(7, 3, 1201L,
	...
	public void testReleaseCarrierRequestMessage() {
		...
		loadStopTable();
		JSonResponseParser jSonParser = new JSonResponseParser();
		ServiceRequestMessage releaseCarriersReq = new ReleaseCarriersRequestMessage(1201L, 4, 5200L,
				CarrierUpdateOperations.RELEASE_CARRIERS);
		JsonServiceWrapperMessage jsonWrapperIn = new JsonServiceWrapperMessage(releaseCarriersReq,
	...
	public void testBulkStatusUpdateRequestMessage() {
		...
		ServiceRequestMessage bulkReq = new BulkStatusUpdateRequestMessage(cList, CarrierStatus.ON_HOLD, "XXX",
				CarrierUpdateOperations.GROUP_HOLD);
		JsonServiceWrapperMessage jsonWrapperIn = new JsonServiceWrapperMessage(bulkReq,
	...
	public void testEmptyCarrierReleaseRequestMessage() {
		...
		JSonResponseParser jSonParser = new JSonResponseParser();
		ServiceRequestMessage releaseEmptyFromRowsReq = new EmptyCarrierReleaseRequest(StorageArea.C_HIGH, true,
				CarrierUpdateOperations.RELEASE_EMPTIES_FROM_ROW);
		JsonServiceWrapperMessage jsonWrapperIn = new JsonServiceWrapperMessage(releaseEmptyFromRowsReq,
		...
		MessageRequest msgRequest = new ConnectionRequest(mesMsg, ConnectionMessageSeparators.DEFAULT.getSeparator());
		String request = msgRequest.getMessageRequest();
		Message thisMessage = (Message) jSonParser.parse(request, JsonServiceWrapperMessage.class); // EmptyCarrierReleaseRequest.class);
		JsonServiceWrapperMessage jsonWrapperOut = (JsonServiceWrapperMessage) thisMessage;
		ServiceRequestMessage serviceRequest = jsonWrapperOut.getContent();
