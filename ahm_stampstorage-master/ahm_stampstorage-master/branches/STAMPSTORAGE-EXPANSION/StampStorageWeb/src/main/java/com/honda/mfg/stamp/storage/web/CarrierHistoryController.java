...
public class CarrierHistoryController {
	...
	public String create(@Valid CarrierHistory carrierHistory, BindingResult bindingResult, Model uiModel,
		...
		return "redirect:/carrierhistories/"
				+ encodeUrlPathSegment(carrierHistory.getId().toString(), httpServletRequest);
	}
	...
	public String update(@Valid CarrierHistory carrierHistory, BindingResult bindingResult, Model uiModel,
			HttpServletRequest httpServletRequest) throws UnsupportedEncodingException {
		if (bindingResult.hasErrors()) {
			uiModel.addAttribute("carrierHistory", carrierHistory);
	...
	public String findCarrierHistoryByCriteria(@RequestParam("carrierNumber") Integer carrierNumber,
		...
		List<CarrierHistory> carrierHistoryList = CarrierHistory.findCarrierHistoryByCarrierNumber(finderCriteria, 1,
				50);
	...
	private CarrierHistoryFinderCriteria getSavedCarrierHistoryFinderCriteria(HttpServletRequest httpServletRequest) {
		...
		if (carrierNumber != null || carrierStatus != null || currentLocation != null || destination != null
				|| die != null || press != null || productionRunNumber != null) {
			finderCriteria = new CarrierHistoryFinderCriteria();
			finderCriteria.setCarrierNumber(carrierNumber);
	...
	private void saveCarrierHistoryFinderCriteria(CarrierHistoryFinderCriteria finderCriteria,
