...
public class CarrierController {
	...
	private static final String MAINT_BIT_FILTER = "maintBitFilter";
	...
	// public String create(@Valid Carrier carrier, BindingResult bindingResult,
	// Model uiModel,
	...
	// encodeUrlPathSegment(carrier.getId().toString(), httpServletRequest);
	// }

	@RequestMapping(params = "form", method = RequestMethod.GET)
	public String createForm(Model uiModel) {
		Carrier c = new Carrier();
		// TODO:NEW Carrier Creation
		List<ParmSetting> parms = ParmSetting.findAllParmSettings();
		BitInfo bi = carrierManagementService.setBitInfo(parms);
	...
	public String show(@PathVariable("id") Long id, Model uiModel) {
		...
		uiModel.addAttribute("carrier", carrier);
		attachBitInfo(uiModel);
		uiModel.addAttribute("itemId", id);
		return "carriers/show";
	}

	private List<Carrier> findMovingCarriers() {
	...
	private void attachBitInfo(Model uiModel) {
		List<ParmSetting> parms = ParmSetting.findAllParmSettings();
		BitInfo bi = carrierManagementService.setBitInfo(parms);
		uiModel.addAttribute("bitInfo", bi);
	}
	...
	public String movingCarriers(@RequestParam(value = "page", required = false) Integer page,
	...
	public String list(@RequestParam(value = "page", required = false) Integer page,
		...
		if (page != null || size != null) {
			...
			if (finderCriteria == null) {
				carrierMeslist = CarrierMes.findCarrierEntries(page == null ? 0 : (page.intValue() - 1) * sizeNo,
						sizeNo);
				float nrOfPages = (float) CarrierMes.countCarriers() / sizeNo;
				uiModel.addAttribute("maxPages",
						(int) ((nrOfPages > (int) nrOfPages || nrOfPages == 0.0) ? nrOfPages + 1 : nrOfPages));
			} else {
				carrierMeslist = CarrierMes
						.findCarriersByCarrierNumberAndDieAndCurrentLocationAndCarrierStatusAndPressAndProductionRunNo(
								finderCriteria, page, sizeNo);
			...
			if (finderCriteria == null) {
				...
				carrierMeslist = CarrierMes
						.findCarriersByCarrierNumberAndDieAndCurrentLocationAndCarrierStatusAndPressAndProductionRunNo(
								finderCriteria, page, size);
			}
		}
		for (CarrierMes carrierMes : carrierMeslist) {
			Carrier c = carrierManagementService.getCarrier(carrierMes);
			carrierList.add(c);
		}
		uiModel.addAttribute("openfind", true);
		uiModel.addAttribute("carriers", carrierList);
		...
		if (finderCriteria != null) {
			...
			uiModel.addAttribute("finderBits", finderCriteria.getBitInfo().showBitsToString());
	...
	public String update(@Valid Carrier carrier, BindingResult bindingResult,
			@RequestParam(value = "originalReferrer", required = false) String originalReferrer, Model uiModel,
			HttpServletRequest httpServletRequest) {
		uiModel.asMap().clear();
		...
		if (carrier.getDie() == null) {
			...
			List defects = Defect.findDefectsByCarrierNumberAndProductionRunNo(carrier.getCarrierNumber(),
					carrier.getProductionRunNo());
		...
		if (carrier.getBuffer() != null && carrier.getBuffer() > 0) {
			...
			for (CarrierMes mesCarrier : carrierList) {
				if (!carrier.getCarrierNumber().equals(mesCarrier.getCarrierNumber())
						&& mesCarrier.getBuffer().intValue() == 1) {
	...
	public String updateForm(@PathVariable("id") Long id, HttpServletRequest httpServletRequest, Model uiModel) {
		CarrierMes carrierMes = CarrierMes.findCarrier(id);
		...
		if (carrierMes != null) {
			carrier = carrierManagementService.getCarrier(carrierMes);
			CarrierRelease release = CarrierRelease
					.findCarrierRelease(Long.parseLong(carrier.getCarrierNumber().toString()));
			if (release != null) {
				...
				} else if (release.getDestination().getId().equals(1300L)
						|| release.getDestination().getId().equals(1301L)) {
				...
				if (carrier.getDestination().getId().equals(704L)) {
					recirculateMsg = " Carrier is recirculating";
				} else if (carrier.getDestination().getId().equals(1300L)
						|| carrier.getDestination().getId().equals(1301L)) {
					WeldOrder order = OrderFulfillment.findOrderByCarrier(carrier);
					if (order != null) {
						partOfOrderMsg = "Carrier Part of Active Order for-" + order.getOrderMgr().getLineName();
					}
	...
	public String updateFormForCurrentLocationModify(@PathVariable("id") Long id,
			@PathVariable("currentlocation") Long location, HttpServletRequest httpServletRequest, Model uiModel) {
		CarrierMes carrierMes = CarrierMes.findCarrier(id);
		Carrier carrier = null;
		String imgUrl = "";
		String imgLabel = "";
	...
	public String reworkForm(@RequestParam("location") Long location,
			@RequestParam("workStationName") String workStationName, Model uiModel) {
	...
	public String reworkUpdate(@Valid Carrier carrier, BindingResult bindingResult,
			@RequestParam("location") Long location, @RequestParam("workStationName") String workStationName,
	...
	public String findReworkCarrierByCarrierNumber(Model uiModel) {
		addDateTimeFormatPatterns(uiModel);
	...
	public String inspectionUpdate(Carrier carrier, BindingResult bindingResult,
			@RequestParam("workStationName") String workStationName, Model uiModel,
			HttpServletRequest httpServletRequest) {
		uiModel.asMap().clear();
		...
		if (carrierMes != null) {
			...
			// MG set Maint Bits to avoid "Null to zero issue"
			newCarrier.setMaintenanceBits(carrierMes.getMaintenanceBits());
	...
	public String inspectionForm(@RequestParam("location") Long location,
			@RequestParam("workStationName") String workStationName, Model uiModel) {
		...
		if (carrier != null) {
			...
			defects = Defect.findDefectsByCarrierNumberAndProductionRunNo(carrier.getCarrierNumber(),
					carrier.getProductionRunNo());
	...
	public String pressActivityForm(@RequestParam("press") String press,
		...
		if (pressActivity != null) {
			if (pressActivity.getProdRunNumber() != null) {
				...
				List<CarrierMes> carrierMesList = CarrierMes
						.findAllCarriersWithProductionRunNumber(pressActivity.getProdRunNumber(), page, size);

				for (CarrierMes carrierMes : carrierMesList) {
					if (pressActivity.getDieNumber().intValue() == carrierMes.getDieNumber().intValue()) {
						carrierList.add(carrierManagementService.getCarrier(carrierMes));
					}
				}
			}
			...
			float nrOfPages = (float) CarrierMes
					.findCountOfCarriersWithProductionRunNumber(pressActivity.getProdRunNumber()) / 50;
			if (nrOfPages > 20) {
				uiModel.addAttribute("maxPages", 20);
			} else {
				uiModel.addAttribute("maxPages",
	...
	public Collection<DEFECT_TYPE> populateDefectTypes() {
	...
	public static Carrier getCarrierForLocation(Long location) {
	...
	@RequestMapping(params = { "find=ByCarrierNumberAndDieAndCurrentLocationAndCarrierStatusAndPressAndProductionRunNo",
			"form" }, method = RequestMethod.GET)
	public String findCarrierByCriteriaForm(Model uiModel) {
		addDateTimeFormatPatterns(uiModel);
		List<Carrier> carrierList = new ArrayList<Carrier>();
	...
	public String findCarrierByCriteria(@RequestParam("carrierNumber") Integer carrierNumber,
			@RequestParam("die") Die die, @RequestParam("productionRunNumber") Integer productionRunNumber,
			@RequestParam("currentLocation") Stop currentLocation,
			@RequestParam("carrierStatus") CarrierStatus carrierStatus, @RequestParam("press") Press press,
			@RequestParam("destination") Stop destination, @RequestParam("maintRequired") Integer maintRequired,
			@RequestParam(value = "showBit1", required = false) Integer showBit1,
			@RequestParam(value = "showBit2", required = false) Integer showBit2,
			@RequestParam(value = "showBit3", required = false) Integer showBit3,
			@RequestParam(value = "showBit4", required = false) Integer showBit4,
			@RequestParam(value = "showBit5", required = false) Integer showBit5,
			@RequestParam(value = "showBit6", required = false) Integer showBit6,
			@RequestParam(value = "showBit7", required = false) Integer showBit7,
			@RequestParam(value = "showBit8", required = false) Integer showBit8,
			@RequestParam(value = "showBit9", required = false) Integer showBit9,
			@RequestParam(value = "showBit10", required = false) Integer showBit10,
			@RequestParam(value = "showBit11", required = false) Integer showBit11,
			@RequestParam(value = "showBit12", required = false) Integer showBit12,
			@RequestParam(value = "showBit13", required = false) Integer showBit13,
			@RequestParam(value = "showBit14", required = false) Integer showBit14,
			@RequestParam(value = "showBit15", required = false) Integer showBit15,
			HttpServletRequest httpServletRequest, HttpServletResponse httpServletResponse, Model uiModel) {
		BitInfo finderBits = new BitInfo((maintRequired == null || maintRequired == 3) ? null : (maintRequired != 0),
				(showBit1 == null || showBit1 == 3) ? null : (showBit1 != 0),
				(showBit2 == null || showBit2 == 3) ? null : (showBit2 != 0),
				(showBit3 == null || showBit3 == 3) ? null : (showBit3 != 0),
				(showBit4 == null || showBit4 == 3) ? null : (showBit4 != 0),
				(showBit5 == null || showBit5 == 3) ? null : (showBit5 != 0),
				(showBit6 == null || showBit6 == 3) ? null : (showBit6 != 0),
				(showBit7 == null || showBit7 == 3) ? null : (showBit7 != 0),
				(showBit8 == null || showBit8 == 3) ? null : (showBit8 != 0),
				(showBit9 == null || showBit9 == 3) ? null : (showBit9 != 0),
				(showBit10 == null || showBit10 == 3) ? null : (showBit10 != 0),
				(showBit11 == null || showBit11 == 3) ? null : (showBit11 != 0),
				(showBit12 == null || showBit12 == 3) ? null : (showBit12 != 0),
				(showBit13 == null || showBit13 == 3) ? null : (showBit13 != 0),
				(showBit14 == null || showBit14 == 3) ? null : (showBit14 != 0),
				(showBit15 == null || showBit15 == 3) ? null : (showBit15 != 0));
		LOG.info("Bit Info Criteria from Form " + finderBits);
		CarrierFinderCriteria finderCriteria = new CarrierFinderCriteria();
		finderCriteria.setBitInfo(finderBits);
		...
		uiModel.addAttribute("finderBits", finderBits.showBitsToString());
		boolean mesHealthy = !carrierManagementService.isDisconnected();
		uiModel.addAttribute("meshealthy", mesHealthy);
		uiModel.addAttribute("alarmevent", carrierManagementService.getAlarmEventToDisplay());
	...
	private CarrierFinderCriteria getSavedCarrierFinderCriteria(HttpServletRequest httpServletRequest) {
		...
		for (Cookie cookie : cookies) {
			...
			if (name.equals(MAINT_BIT_FILTER) && (value != null && value.length() > 0)) {
				LOG.info("Bit Info retrieved from Cookie " + value);
				bitInfo = new BitInfo();
				bitInfo.parseShowBits(value);
				LOG.info("Bit Info retrieved from Cookie and Parsed " + bitInfo);
			}
		}
		if (carrierNumber != null || carrierStatus != null || currentLocation != null || destination != null
				|| die != null || press != null || productionRunNumber != null || !allMaintBitsNull(bitInfo)) {
			...
			finderCriteria.setProductionRunNo(productionRunNumber);
			if (bitInfo != null) {
				finderCriteria.setBitInfo(bitInfo);
			}
			LOG.info("Bit Info from Cookie set in criteria " + bitInfo);
		}
		return finderCriteria;
	}

	private boolean allMaintBitsNull(BitInfo bitInfo) {
		if (bitInfo == null)
			return true;
		if (bitInfo.getShowBit1() != null)
			return false;
		if (bitInfo.getShowBit2() != null)
			return false;
		if (bitInfo.getShowBit3() != null)
			return false;
		if (bitInfo.getShowBit4() != null)
			return false;
		if (bitInfo.getShowBit5() != null)
			return false;
		if (bitInfo.getShowBit6() != null)
			return false;
		if (bitInfo.getShowBit7() != null)
			return false;
		if (bitInfo.getShowBit8() != null)
			return false;
		if (bitInfo.getShowBit9() != null)
			return false;
		if (bitInfo.getShowBit10() != null)
			return false;
		if (bitInfo.getShowBit11() != null)
			return false;
		if (bitInfo.getShowBit12() != null)
			return false;
		if (bitInfo.getShowBit13() != null)
			return false;
		if (bitInfo.getShowBit14() != null)
			return false;
		if (bitInfo.getShowBit15() != null)
			return false;
		if (bitInfo.getMaintRequired() != null)
			return false;
		return true;

	}

	private void saveCarrierFinderCriteria(CarrierFinderCriteria finderCriteria,
		...
		if (finderCriteria != null) {
			...
			LOG.info("BitInfo From Criterial Before putting into Cookie: " + finderCriteria.getBitInfo());
			BitInfo bitInfo = finderCriteria.getBitInfo() == null ? new BitInfo() : finderCriteria.getBitInfo();
			...
			Cookie maintBitCookie = new Cookie(MAINT_BIT_FILTER, bitInfo.showBitsToString());
			maintBitCookie.setMaxAge(COOKIE_MAX_AGE);
			LOG.info("Setting Cookie with BitInfo " + bitInfo);
			httpServletResponse.addCookie(maintBitCookie);
			...
			LOG.info("Finder Criteria NULL can't build cookie");
		}
	}
}
