...
import com.honda.mfg.stamp.conveyor.messages.ClearAlarmRequestMessage;
...
import com.honda.mfg.stamp.conveyor.messages.StorageStateRefreshRequestMessage;
import com.honda.mfg.stamp.conveyor.messages.StorageStateResetRequestMessage;
import com.honda.mfg.stamp.conveyor.messages.UpdateLaneCarrierListRequestMessage;

/**
 * This class "marshals" the update request into an update message and publishes
 * on event bus; the subscriber is ServiceSendMessageProcessor User: vcc44349
 * Date: 01/09/2013
 */
public class CarrierManagementServiceProxyImpl implements CarrierManagementServiceProxy {
	...
	public void saveCarrier(Carrier carrier) {
		...
		JsonServiceWrapperMessage jsonWrapper = new JsonServiceWrapperMessage(carrierUpdateReqMsg,
				JsonServiceMessageTypes.CarrierUpdateReqeustMessage);
	...
	public void sendBulkCarrierStatusUpdate(List<Carrier> carriers, CarrierStatus status, String user) {
		...
		ServiceRequestMessage bulkReq = new BulkStatusUpdateRequestMessage(numbers, status, user,
				CarrierUpdateOperations.GROUP_HOLD);
		JsonServiceWrapperMessage jsonWrapper = new JsonServiceWrapperMessage(bulkReq,
				JsonServiceMessageTypes.BulkStatusUpdateRequest);
		EventBus.publish(jsonWrapper);
	}
	...
	public void recalculateCarrierDestination(Carrier carrier) {
		...
		ServiceRequestMessage carrierUpdateReqMsg = new CarrierUpdateRequestMessage(c,
	...
	public void removeCarrierFromRow(Integer carrierNum) {
		...
		ServiceRequestMessage laneUpdateReqMsg = new LaneUpdateRequestMessage(carrierNum,
				CarrierUpdateOperations.REMOVE_FROM_ROW);
		JsonServiceWrapperMessage jsonWrapper = new JsonServiceWrapperMessage(laneUpdateReqMsg,
				JsonServiceMessageTypes.LaneUpdateRequestMessage);
		EventBus.publish(jsonWrapper);
	}
	...
	public void addCarrierToRow(Integer carrierNumber, Integer position, Stop laneStop) {
		...
		if (laneStop != null) {
			sb.append(laneStop.getName()).append(", position: ");
		}
		if (position != null) {
			sb.append(position);
		}
		...
		if (laneStop != null) {
			laneStopId = laneStop.getId();
		}
		ServiceRequestMessage laneUpdateReqMsg = new LaneUpdateRequestMessage(carrierNumber, position, laneStopId,
	...
	public void saveCarriersInToRow(List<Integer> carrierNumberList, Stop laneStop) {
		...
		ServiceRequestMessage laneUpdateReqMsg = new UpdateLaneCarrierListRequestMessage(carrierNumberList, laneStopId,
				CarrierUpdateOperations.ROW_RESEQUENCE);
		JsonServiceWrapperMessage jsonWrapper = new JsonServiceWrapperMessage(laneUpdateReqMsg,
				JsonServiceMessageTypes.UpdateLaneCarrierListMessage);
	...
	public void releaseEmptyCarriersFromRows(StorageArea area, boolean releaseMgr, String source) {
		...
		EmptyCarrierReleaseRequest releaseEmptyFromRowsReq = new EmptyCarrierReleaseRequest(area, releaseMgr,
				CarrierUpdateOperations.RELEASE_EMPTIES_FROM_ROW);
		releaseEmptyFromRowsReq.setSource(source);
		JsonServiceWrapperMessage jsonWrapper = new JsonServiceWrapperMessage(releaseEmptyFromRowsReq,
				JsonServiceMessageTypes.ReleaseEmptiesFromRowRequest);
		EventBus.publish(jsonWrapper);
	}
	...
	public void releaseCarriers(StorageRow lane, Integer releaseCount, Stop destination, String source) {
		...
		ReleaseCarriersRequestMessage releaseCarriersReq = new ReleaseCarriersRequestMessage(laneStopId, releaseCount,
				destStopId, CarrierUpdateOperations.RELEASE_CARRIERS);
		releaseCarriersReq.setSource(source);
		JsonServiceWrapperMessage jsonWrapper = new JsonServiceWrapperMessage(releaseCarriersReq,
				JsonServiceMessageTypes.ReleaseCarriersRequest);
		EventBus.publish(jsonWrapper);
	}

	/*
	 * (non-Javadoc) unused method, not implementing
	 * 
	 * @deprecated
	 */
	@Override
	...
	public void updateRow(StorageRow row) {
		...
		LOG.info(sb.toString());
		ServiceRequestMessage laneUpdateReqMsg = new LaneUpdateRequestMessage(0, 0, row.getStop().getId(),
				CarrierUpdateOperations.UPDATE_ROW);
		JsonServiceWrapperMessage jsonWrapper = new JsonServiceWrapperMessage(laneUpdateReqMsg,
				JsonServiceMessageTypes.LaneUpdateRequestMessage);
	...
	public void refreshStorageState() {
		...
		LOG.info(sb.toString());
		ServiceRequestMessage storageStateRefreshReqMsg = new StorageStateRefreshRequestMessage(
				CarrierUpdateOperations.REFRESH_STORAGE_STATE);
		JsonServiceWrapperMessage jsonWrapper = new JsonServiceWrapperMessage(storageStateRefreshReqMsg,
				JsonServiceMessageTypes.StorageStateRefreshRequestMessage);
		EventBus.publish(jsonWrapper);
	}
	...
	public void resetStorageState() {
		...
		LOG.info(sb.toString());
		ServiceRequestMessage storageStateResetReqMsg = new StorageStateResetRequestMessage(
				CarrierUpdateOperations.RESET_STORAGE_STATE);
		JsonServiceWrapperMessage jsonWrapper = new JsonServiceWrapperMessage(storageStateResetReqMsg,
				JsonServiceMessageTypes.StorageStateResetRequestMessage);
		EventBus.publish(jsonWrapper);
	}
	...
	public void reorderCarriersInRow(Long stopId) {
		...
		LOG.info(sb.toString());
		ServiceRequestMessage laneUpdateReqMsg = new LaneUpdateRequestMessage(0, 0, stopId,
	...
	public void clearAlarm(Integer alarmNumber) {
		...
		LOG.info(sb.toString());
		ServiceRequestMessage clearAlarmRequestMessage = new ClearAlarmRequestMessage(
				CarrierUpdateOperations.CLEAR_ALARM, "", alarmNumber.toString());
		JsonServiceWrapperMessage jsonWrapper = new JsonServiceWrapperMessage(clearAlarmRequestMessage,
				JsonServiceMessageTypes.ClearAlarmRequestMessage);
		EventBus.publish(jsonWrapper);
	}
