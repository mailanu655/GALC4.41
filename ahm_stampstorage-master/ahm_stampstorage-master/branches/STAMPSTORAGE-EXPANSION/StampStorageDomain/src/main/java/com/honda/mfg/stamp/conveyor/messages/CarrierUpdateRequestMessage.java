...
/**
 * A class representing a carrier update request. For example:
 * 
 * <pre>
...
public class CarrierUpdateRequestMessage extends CarrierUpdateMessage implements ServiceRequestMessage {
	...
	/*
	 * constructs a carrier update request from Carrier sets target operation
	 *
	 * @param Carrier
	 * 
	...
	public Carrier getCarrier() {
		...
		Die die = null;
		if (!isEmptyString(dieNumber)) {
			die = Die.findDie(Long.parseLong(dieNumber));
		}
		carrier.setDie(die);

		Stop currentStop = null;
		if (!isEmptyString(currentLocation)) {
			currentStop = Stop.findStop(Long.valueOf(currentLocation));
		}
		carrier.setCurrentLocation(currentStop);

		Stop destinationStop = null;
		if (!isEmptyString(destination)) {
			destinationStop = Stop.findStop(Long.valueOf(destination));
		}
		carrier.setDestination(destinationStop);
		// MG Add Maint Bits
		if (!isEmptyString(maintenanceBits)) {
			carrier.setMaintenanceBits(Integer.parseInt(maintenanceBits));
		}
		if (!isEmptyString(status)) {
			carrier.setCarrierStatus(CarrierStatus.findByType(Integer.parseInt(status)));
		}
		if (!isEmptyString(originationLocation)) {
			carrier.setPress(Press.findByType(Integer.parseInt(originationLocation)));
		}
		if (!isEmptyString(productionRunNumber)) {
		...
		try {
			...
			Date prodRunTS = null;
			if (!isEmptyString(productionRunTimestamp)) {
				prodRunTS = new Date(formatter.parse(productionRunTimestamp).getTime());
			}
