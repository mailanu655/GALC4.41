...
public class CarrierRelease {
	...
	public static List<CarrierRelease> findCarrierReleaseEntries(int firstResult, int maxResults) {
		return entityManager().createQuery("SELECT o FROM CarrierRelease o", CarrierRelease.class)
	...
	public static List<CarrierRelease> findCarrierReleaseEntriesAtStop(Long stopNumber) {
		Query q = entityManager().createQuery(
				"SELECT o FROM CarrierRelease o WHERE o.id IN (SELECT x.carrierNumber FROM CarrierMes x WHERE x.currentLocation = :stopNumber)",
				CarrierRelease.class);
	...
	public static List<CarrierRelease> findCarrierReleaseEntriesInStorageArea(List<Long> stopNumbers) {
		Query q = entityManager().createQuery(
				"SELECT o FROM CarrierRelease o WHERE o.id IN (SELECT x.carrierNumber FROM CarrierMes x WHERE x.currentLocation IN (:stopNumbers) AND buffer = 1) order by o.requestTimestamp asc",
				CarrierRelease.class);
	...
	public static List<CarrierRelease> findCarrierReleaseEntriesInStorageAreaThatNeedDestinationUpdated(
			List<Long> stopNumbers) {
		Query q = entityManager().createQuery(
				"SELECT o FROM CarrierRelease o WHERE o.id IN (SELECT x.carrierNumber FROM CarrierMes x WHERE x.currentLocation IN (:stopNumbers) AND buffer = 1 and  x.currentLocation = x.destination) order by o.requestTimestamp asc",
				CarrierRelease.class);
		q.setParameter("stopNumbers", stopNumbers);
