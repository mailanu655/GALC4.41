...
public class OrderFulfillmentDataOnDemand {
	...
	private void setEmbeddedId(OrderFulfillment obj, int index) {
		...
		com.honda.mfg.stamp.conveyor.domain.OrderFulfillmentPk embeddedIdClass = new com.honda.mfg.stamp.conveyor.domain.OrderFulfillmentPk(
				weldOrder, carrierNumber, releaseCycle);
	...
	public void init() {
		...
		if (data == null)
			throw new IllegalStateException(
					"Find entries implementation for 'OrderFulfillment' illegally returned null");
		if (!data.isEmpty()) {
			return;
		}
