...
public class StopIntegrationTest {
	...
	public void testFindAllStops() {
		org.junit.Assert.assertNotNull("Data on demand for 'Stop' failed to initialize correctly", dod.getRandomStop());
		long count = Stop.countStops();
		org.junit.Assert.assertTrue("Too expensive to perform a find all test for 'Stop', as there are " + count
	...
	public void testFindStopEntries() {
		...
		org.junit.Assert.assertEquals("Find entries method for 'Stop' returned an incorrect number of entries", count,
				result.size());
	}
	...
	public void testFlush() {
		...
		obj.flush();
		org.junit.Assert.assertTrue("Version for 'Stop' failed to increment on flush directive",
	...
	public void testMerge() {
		...
		org.junit.Assert.assertEquals("Identifier of merged object not the same as identifier of original object",
				merged.getId(), id);
		org.junit.Assert.assertTrue("Version for 'Stop' failed to increment on merge and flush directive",
				(currentVersion != null && obj.getVersion() > currentVersion) || !modified);
	}
	...
	public void testPersist() {
		...
		obj.persist();
		obj.flush();
		// org.junit.Assert.assertNotNull("Expected 'Stop' identifier to no longer be
		// null", obj.getId());
	}
