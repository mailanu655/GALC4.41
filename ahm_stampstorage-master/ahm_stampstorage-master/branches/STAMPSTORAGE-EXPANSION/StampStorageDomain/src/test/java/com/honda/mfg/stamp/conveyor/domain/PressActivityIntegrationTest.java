...
public class PressActivityIntegrationTest {
	...
	public void testCountPressActivitys() {
		org.junit.Assert.assertNotNull("Data on demand for 'PressActivity' failed to initialize correctly",
				dod.getRandomPressActivity());
		long count = com.honda.mfg.stamp.conveyor.domain.PressActivity.countPressActivitys();
		org.junit.Assert.assertTrue("Counter for 'PressActivity' incorrectly reported there were no entries",
				count > 0);
	}
	...
	public void testFindPressActivity() {
		...
		org.junit.Assert.assertNotNull("Find method for 'PressActivity' illegally returned null for id '" + id + "'",
				obj);
		org.junit.Assert.assertEquals("Find method for 'PressActivity' returned the incorrect identifier", id,
				obj.getId());
	}
	...
	public void testFindAllPressActivitys() {
		org.junit.Assert.assertNotNull("Data on demand for 'PressActivity' failed to initialize correctly",
				dod.getRandomPressActivity());
		long count = com.honda.mfg.stamp.conveyor.domain.PressActivity.countPressActivitys();
		org.junit.Assert.assertTrue("Too expensive to perform a find all test for 'PressActivity', as there are "
				+ count
				+ " entries; set the findAllMaximum to exceed this value or set findAll=false on the integration test annotation to disable the test",
				count < 250);
		java.util.List<com.honda.mfg.stamp.conveyor.domain.PressActivity> result = com.honda.mfg.stamp.conveyor.domain.PressActivity
				.findAllPressActivitys();
	...
	public void testFindPressActivityEntries() {
		...
		if (count > 20)
			count = 20;
		java.util.List<com.honda.mfg.stamp.conveyor.domain.PressActivity> result = com.honda.mfg.stamp.conveyor.domain.PressActivity
				.findPressActivityEntries(0, (int) count);
		org.junit.Assert.assertNotNull("Find entries method for 'PressActivity' illegally returned null", result);
		org.junit.Assert.assertEquals("Find entries method for 'PressActivity' returned an incorrect number of entries",
				count, result.size());
	}
	...
	public void testFlush() {
		...
		obj.flush();
		org.junit.Assert.assertTrue("Version for 'PressActivity' failed to increment on flush directive",
	...
	public void testMerge() {
		...
		boolean modified = dod.modifyPressActivity(obj);
		java.lang.Integer currentVersion = obj.getVersion();
		com.honda.mfg.stamp.conveyor.domain.PressActivity merged = (com.honda.mfg.stamp.conveyor.domain.PressActivity) obj
				.merge();
		obj.flush();
		org.junit.Assert.assertEquals("Identifier of merged object not the same as identifier of original object",
				merged.getId(), id);
		org.junit.Assert.assertTrue("Version for 'PressActivity' failed to increment on merge and flush directive",
				(currentVersion != null && obj.getVersion() > currentVersion) || !modified);
	}
	...
	public void testPersist() {
		...
		org.junit.Assert.assertNotNull("Data on demand for 'PressActivity' failed to provide a new transient entity",
				obj);
	...
	public void testRemove() {
		...
		obj.remove();
		obj.flush();
		org.junit.Assert.assertNull("Failed to remove 'PressActivity' with identifier '" + id + "'",
				com.honda.mfg.stamp.conveyor.domain.PressActivity.findPressActivity(id));
	}
}
