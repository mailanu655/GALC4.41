...
public class ParmSettingIntegrationTest {
	...
	public void testCountParmSettings() {
		org.junit.Assert.assertNotNull("Data on demand for 'ParmSetting' failed to initialize correctly",
				dod.getRandomParmSetting());
	...
	public void testFindParmSetting() {
		...
		org.junit.Assert.assertNotNull("Find method for 'ParmSetting' illegally returned null for id '" + id + "'",
				obj);
		org.junit.Assert.assertEquals("Find method for 'ParmSetting' returned the incorrect identifier", id,
				obj.getId());
	}
	...
	public void testFindAllParmSettings() {
		org.junit.Assert.assertNotNull("Data on demand for 'ParmSetting' failed to initialize correctly",
				dod.getRandomParmSetting());
		long count = com.honda.mfg.stamp.conveyor.domain.ParmSetting.countParmSettings();
		org.junit.Assert.assertTrue("Too expensive to perform a find all test for 'ParmSetting', as there are " + count
				+ " entries; set the findAllMaximum to exceed this value or set findAll=false on the integration test annotation to disable the test",
				count < 250);
		java.util.List<com.honda.mfg.stamp.conveyor.domain.ParmSetting> result = com.honda.mfg.stamp.conveyor.domain.ParmSetting
				.findAllParmSettings();
	...
	public void testFindParmSettingEntries() {
		...
		if (count > 20)
			count = 20;
		java.util.List<com.honda.mfg.stamp.conveyor.domain.ParmSetting> result = com.honda.mfg.stamp.conveyor.domain.ParmSetting
				.findParmSettingEntries(0, (int) count);
		org.junit.Assert.assertNotNull("Find entries method for 'ParmSetting' illegally returned null", result);
		org.junit.Assert.assertEquals("Find entries method for 'ParmSetting' returned an incorrect number of entries",
				count, result.size());
	}
	...
	public void testFlush() {
		...
		obj.flush();
		org.junit.Assert.assertTrue("Version for 'ParmSetting' failed to increment on flush directive",
	...
	public void testMerge() {
		...
		boolean modified = dod.modifyParmSetting(obj);
		java.lang.Integer currentVersion = obj.getVersion();
		com.honda.mfg.stamp.conveyor.domain.ParmSetting merged = (com.honda.mfg.stamp.conveyor.domain.ParmSetting) obj
				.merge();
		obj.flush();
		org.junit.Assert.assertEquals("Identifier of merged object not the same as identifier of original object",
				merged.getId(), id);
		org.junit.Assert.assertTrue("Version for 'ParmSetting' failed to increment on merge and flush directive",
				(currentVersion != null && obj.getVersion() > currentVersion) || !modified);
	}
	...
	public void testPersist() {
		...
		org.junit.Assert.assertNotNull("Data on demand for 'ParmSetting' failed to provide a new transient entity",
				obj);
	...
	public void testRemove() {
		...
		obj.remove();
		obj.flush();
		org.junit.Assert.assertNull("Failed to remove 'ParmSetting' with identifier '" + id + "'",
				com.honda.mfg.stamp.conveyor.domain.ParmSetting.findParmSetting(id));
	}
}
