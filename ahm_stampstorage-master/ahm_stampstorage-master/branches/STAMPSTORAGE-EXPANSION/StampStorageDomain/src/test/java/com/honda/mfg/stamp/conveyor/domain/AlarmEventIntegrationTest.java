...
@Transactional
@Configurable
public class AlarmEventIntegrationTest {
	...
	@Autowired
	private AlarmEventDataOnDemand dod;

	@Test
	public void testCountCurrent_Alarms() {
		org.junit.Assert.assertNotNull("Data on demand for 'AlarmEvent' failed to initialize correctly",
				dod.getRandomCurrent_Alarm());
	...
	public void testFindCurrent_Alarm() {
		...
		org.junit.Assert.assertEquals("Find method for 'AlarmEvent' returned the incorrect identifier", id,
				obj.getId());
	}

	@Test
	public void testFindAllCurrent_Alarms() {
		org.junit.Assert.assertNotNull("Data on demand for 'AlarmEvent' failed to initialize correctly",
				dod.getRandomCurrent_Alarm());
		long count = AlarmEvent.countCurrent_Alarms();
		org.junit.Assert.assertTrue("Too expensive to perform a find all test for 'AlarmEvent', as there are " + count
	...
	@Test
	public void testFindCurrent_AlarmEntries() {
		org.junit.Assert.assertNotNull("Data on demand for 'AlarmEvent' failed to initialize correctly",
				dod.getRandomCurrent_Alarm());
		long count = AlarmEvent.countCurrent_Alarms();
		if (count > 20)
		...
		org.junit.Assert.assertEquals("Find entries method for 'AlarmEvent' returned an incorrect number of entries",
				count, result.size());
	}
	...
	public void testFlush() {
		...
		obj.flush();
		org.junit.Assert.assertTrue("Version for 'AlarmEvent' failed to increment on flush directive",
	...
	public void testMerge() {
		...
		org.junit.Assert.assertEquals("Identifier of merged object not the same as identifier of original object",
				merged.getId(), id);
		org.junit.Assert.assertTrue("Version for 'AlarmEvent' failed to increment on merge and flush directive",
				(currentVersion != null && obj.getVersion() > currentVersion) || !modified);
	}
	...
	public void testRemove() {
		...
		obj.remove();
		obj.flush();
		org.junit.Assert.assertNull("Failed to remove 'AlarmEvent' with identifier '" + id + "'",
				AlarmEvent.findCurrent_Alarm(id));
	}
}
