...
//        }
	...
	private Integer getOldestProductionRunNo(Long die) {
		...
		if (rows != null) {
			for (StorageRow storageRow : rows) {
				// if (!(StorageRow.getDieNumbersForAllCarriers().size() == 1 &&
	...
	private List<StorageRow> getRowsWithOldestProductionRunPartCarriers(Long die, List<StorageRow> rows) {
		List<StorageRow> rowsWithOldestProductionRunPartCarriers = getStorageState()
				.queryForRows(
						allOf(RowMatchers.hasCarriersWithDieNumber(die),
								RowMatchers
										.hasCarrierWithProdRunNoAndCarrierStatusNormal(getOldestProductionRunNo(die))),
						rows);
		return rowsWithOldestProductionRunPartCarriers;
	}

	public List<StorageRow> getStorageRowsWithOldestProductionRunPartCarriersForStorageArea(Die part,
			StorageArea area) {
		List<StorageRow> rows = getStorageRowsByArea(area);
		if (part.getPartProductionVolume().equals(PartProductionVolume.EMPTY)) {
			return rows;
		} else {
			List<StorageRow> rowsWithOldestProductionRunPartCarriers = getRowsWithOldestProductionRunPartCarriers(
					part.getId(), rows);
			return rowsWithOldestProductionRunPartCarriers;
		}
	}
	...
	public StorageRow getAppropriatePartialRowForDie(Long dieNumber, List<StorageRow> rows) {
		List<StorageRow> unsortedPartialRows = getStorageState().queryForRows(
				allOf(RowMatchers.isCurrentCapacityPartial(), RowMatchers.isLaneWithOnlySingleDieNumberOf(dieNumber)

				), rows);
		LOG.debug("querying partial Lanes ");
		List<StorageRow> partialLanesWithMinimumPartsOfDieNumber = unsortedPartialRows == null ? unsortedPartialRows
				: Ordering.from(RowComparators.getComparatorByCarriersCountOfDieNumberAtLaneOut(dieNumber))
						.sortedCopy(unsortedPartialRows);

		return partialLanesWithMinimumPartsOfDieNumber == null || partialLanesWithMinimumPartsOfDieNumber.size() == 0
				? null
				: partialLanesWithMinimumPartsOfDieNumber.get(0);
	}
	...
	public StorageRow getAppropriateMixedFrontRowForDie(Long dieNumber, List<StorageRow> rows) {
		List<StorageRow> unsortedMixedFrontRows = getStorageState().queryForRows(
				allOf(RowMatchers.isCurrentCapacityMixed(), RowMatchers.hasCarrierAtTheLaneOutWithDieNumber(dieNumber)),
				rows);
		LOG.debug("querying Mixed Front Lanes ");
		List<StorageRow> mixedRowsWithMinimumPartsOfDieAtLaneOut = unsortedMixedFrontRows == null
				? unsortedMixedFrontRows
				: Ordering.from(RowComparators.getComparatorByCarriersCountOfDieNumberAtLaneOut(dieNumber))
						.sortedCopy(unsortedMixedFrontRows);

		return mixedRowsWithMinimumPartsOfDieAtLaneOut == null || mixedRowsWithMinimumPartsOfDieAtLaneOut.size() == 0
				? null
				: mixedRowsWithMinimumPartsOfDieAtLaneOut.get(0);
	}
	...
	public StorageRow getAppropriateFullRowForDie(Long dieNumber, List<StorageRow> rows) {
		List<StorageRow> fullRows = getStorageState().queryForRows(
				allOf(RowMatchers.isCurrentCapacityFull(), RowMatchers.isLaneWithOnlySingleDieNumberOf(dieNumber)),
				rows);
	...
	public StorageRow getAppropriateMixedBlockedRowForDie(Long dieNumber, List<StorageRow> rows) {
		List<StorageRow> unsortedMixedBlockedRows = getStorageState().queryForRows(allOf(
				RowMatchers.isCurrentCapacityMixed(), RowMatchers.hasBlockedCarrierWithDieNumberAtTheLaneOut(dieNumber),
				RowMatchers.hasCarrierWithDieAndCarrierStatusShippable(dieNumber)), rows);
		LOG.info("querying Mixed blocked Lanes ");
		List<StorageRow> mixedRowsHavingDieWithMinimumBlockingCarriersAtLaneOut = unsortedMixedBlockedRows == null
				? unsortedMixedBlockedRows
		...
		return mixedRowsHavingDieWithMinimumBlockingCarriersAtLaneOut == null
				|| mixedRowsHavingDieWithMinimumBlockingCarriersAtLaneOut.size() == 0 ? null
						: mixedRowsHavingDieWithMinimumBlockingCarriersAtLaneOut.get(0);
	}
