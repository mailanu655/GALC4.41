...
public abstract class StoreOutRuleBase implements StoreOutRule {
	...
	public StoreOutRuleBase(StorageStateContext storageStateContext, StoreOutRule nextRule, LaneCondition laneCondition,
	...
	public final StorageRow processRule(Die die) {
		...
		if (lane != null) {
			...
			if (laneCondition == null && priority == null) {
				...
				message = "Fired Rule: " + "Retrieve " + die.getPartProductionVolume().name() + " Die from "
						+ laneCondition.name() + " Row of StorageAreaPriority: " + priority.name() + " --->  "
						+ lane.getRowName();
			}
	...
	private StorageRow processNextRule(Die part) {
		...
		if (nextRule != null) {
			...
			storageStateContext.saveToAuditLog(StorageConfig.OHCV_APP,
					"Unable to locate an applicable rule! to retrieve Die :" + part.getDescription(),
					StorageConfig.OHCV_APP_STOREOUT);
