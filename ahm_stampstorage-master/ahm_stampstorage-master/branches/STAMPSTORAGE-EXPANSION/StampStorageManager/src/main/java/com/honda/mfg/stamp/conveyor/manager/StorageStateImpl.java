...
/**
 * User: Adam S. Kendell Date: Mar 1, 2011
 */
public class StorageStateImpl implements StorageState {
	...
	private List<BackOrder> backOrder;
	private boolean stale;

	public StorageStateImpl(List<StorageRow> rows) {
		this.rows = rows;
		this.stale = false;
		this.backOrder = new ArrayList<BackOrder>();
	}

	public List<BackOrder> getBackOrder() {
		LOG.debug(" getting BackOrder - " + backOrder.toString());
		return this.backOrder;
	}

	public void setBackOrder(List<BackOrder> backOrder) {
		LOG.debug(" Setting BackOrder - " + backOrder.toString());
	...
	public boolean isCarrierPartsOnBackOrder(Die die) {
		if (backOrder == null || backOrder.size() == 0) {
			return false;
		} else {
			for (BackOrder tempBackOrder : backOrder) {
				if (tempBackOrder != null && tempBackOrder.getDieId().equals(die.getId())) {
					return true;
				}
			}
		}
	...
	public void removeFromBackOrderList(Die die, OrderMgr orderMgr) {
		try {
			List<BackOrder> newBackOrder = new ArrayList<BackOrder>();

			for (BackOrder tempBackOrder : backOrder) {
				if (tempBackOrder != null && tempBackOrder.getDieId().equals(die.getId())
						&& tempBackOrder.getOrderMgrId().equals(orderMgr.getId())) {
					LOG.debug(" removing backOrder from backOrder Array-" + tempBackOrder.toString());
				} else {
					LOG.debug(" retaining backOrder -" + tempBackOrder.toString());
					newBackOrder.add(tempBackOrder);
				}
			}
	...
	public void removeFromBackOrderList(OrderMgr orderMgr) {
		try {
			List<BackOrder> newBackOrder = new ArrayList<BackOrder>();

			for (BackOrder tempBackOrder : backOrder) {
				if (tempBackOrder.getOrderMgrId().equals(orderMgr.getId())) {
					LOG.debug(" removing backOrder from backOrder Array-" + tempBackOrder.toString());
				} else {
					LOG.debug(" retaining backOrder -" + tempBackOrder.toString());
					newBackOrder.add(tempBackOrder);
	...
	public List<StorageRow> queryForRows(Matcher<StorageRow> matcher) {
		Iterable<StorageRow> filteredRows = matcher == null ? rows
				: Iterables.filter(rows, new PredicateWrapper<StorageRow>(matcher));
	...
	public List<StorageRow> queryForRows(Matcher<StorageRow> matcher, List<StorageRow> storageRows) {
		...
		if (storageRows != null) {
			Iterable<StorageRow> filteredRows = matcher == null ? storageRows
					: Iterables.filter(storageRows, new PredicateWrapper<StorageRow>(matcher));
...
//            removeFromBackOrderList(carrier.getDie());
//        }
...
//TODO:CHECK FOR MAINT BITS
			if (storageRow.getStop().equals(row.getStop())) {
				...
				if (carrier.getCurrentLocation() != null
						&& carrier.getCurrentLocation().getStopType().equals(StopType.REWORK)) {
				...
				if (row.carrierExistsInRow(carrier)) {
					...
					if (carrier.getCarrierNumber().equals(carrierOut.getCarrierNumber())) {
						if (!carrier.getDestination().equals(row.getStop())) {
							LOG.debug("releasing carrier " + carrier.getCarrierNumber() + " from Row --"
									+ row.getRowName());
							row.release();
							break;
						}
	...
	public void removeCarrierFromRow(Integer carrierNumber, Long laneStopConveyorId) {
		...
		if (carrierMesList != null) {
			...
			for (CarrierMes carrierMes : carrierMesList) {
				if (!carrierMes.getCarrierNumber().equals(carrierNumber)
						&& carrierMes.getDestination().equals(laneStopConveyorId)) {
	...
	public boolean hadValidLaneDestination(Carrier carrier) {
		...
		if (carrier.getDestination().isRowStop()) {
			...
			if (!carrierExistsInStorageState(carrier)) {
				...
				long countCarriersInDestinationLane = CarrierMes
						.countCarriersWithCurrentLocationStop(carrier.getDestination());

				if (countCarriersInDestinationLane >= getLaneCapacityByAssociatedStop(carrier.getDestination())
						.intValue()) {
					flag = false;
				}
			}
	...
	// release all existing carriers from lane set current location and destination
	// to zero if not part of carrier list
				...
				while (!lane.isEmpty()) {
					...
					if (!exists(carriers, c)) {
						...
						if (maintenanceStops != null && maintenanceStops.size() > 0) {
							...
							sendCarrierUpdateMessage(newCarrier);
							LOG.info("moving carrier " + newCarrier.getCarrierNumber() + " to maintenance stop-"
									+ maintenanceStops.get(0).getName());
						} else {
	...
	private void populateRow(List<Carrier> carriersAlreadyInRow, List<Carrier> carriersMovingIntoRow,
			long laneStopConveyorId) {
		for (StorageRow lane : rows) {
			if (lane.getStop().getId().equals(laneStopConveyorId)) {
				lane.setOutOfOrder(true);
				while (!lane.isEmpty()) {
