...
public abstract class StoreInRuleBase implements StoreInRule {
	...
	public StoreInRuleBase(StorageStateContext storageStateContext, StoreInRule nextRule) {
	...
	public StoreInRuleBase(StorageStateContext storageStateContext, StoreInRule nextRule, LaneCondition laneCondition,
	...
	public final StorageRow processRule(Carrier carrier) {
		...
		if (lane != null) {
			if (laneCondition == null && priority == null) {
				message = "Fired Rule: StoreCarrierIntoRowOfLastCarrierIfSamePartsAndRowNotFullRule  --> carrierNumber: "
						+ carrier.getCarrierNumber() + "  --> " + lane.getRowName();
			} else {
				String lcondition = laneCondition != null ? laneCondition.name() : "";
				String priorityName = priority != null ? priority.name() : "";
				message = "Fired Rule: " + "Store " + carrier.getDie().getPartProductionVolume().name()
						+ " Carrier Into " + lcondition + " Row of StorageAreaPriority: " + priorityName
	...
	private StorageRow processNextRule(Carrier carrier) {
		...
		if (nextRule != null) {
			...
			storageStateContext.saveToAuditLog(StorageConfig.OHCV_APP,
					"Unable to locate an applicable rule! to Store Carrier : " + carrier.getCarrierNumber(),
					StorageConfig.OHCV_APP_STOREIN);
