...
/**
 * This is used Created by IntelliJ IDEA. User: Ambica Gawarla Date: 5/14/13
 * Time: 4:19 PM To change this template use File | Settings | File Templates.
 */
public class StoreCarrierIntoRowOfZonePriorityAndRowCondition extends StoreInRuleBase {
	...
	public StoreCarrierIntoRowOfZonePriorityAndRowCondition(StorageStateContext storageStateContext,
	...
	public StorageRow computeLane(Carrier carrier) {
		...
		StorageArea area = StoragePriority.getStorageAreaByPriorityForVolume(carrier.getPartProductionVolume(),
				priority);

		switch (laneCondition) {
		case PARTIAL:
			appropriateRow = getStorageStateStoreInWrapper()
					.getAppropriatePartialRowForDieInStorageArea(carrier.getDie().getId(), area);
			break;
		case VACANT:
		...
		case EMPTY:
			appropriateRow = getStorageStateStoreInWrapper()
					.getAppropriatePartialRowWithEmptyCarriersInStorageArea(area);
			break;
		case MIXED_BACK:
			appropriateRow = getStorageStateStoreInWrapper()
					.getAppropriateMixedBackRowWithDieInStorageArea(carrier.getDie().getId(), area);
			break;
		case MIXED:
		...
		case PARTIAL_DIFFERENT_DIE:
			appropriateRow = getStorageStateStoreInWrapper()
					.getAppropriatePartialDifferentDieRowInStorageArea(carrier.getDie().getId(), area);
			break;
		}
