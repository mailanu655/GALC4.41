...
import com.honda.mfg.stamp.conveyor.domain.CarrierMesInStorageLanesComparator;
...
public class StorageStateContextImpl implements StorageStateContext, StorageLifeCycle {
	...
	public void reload() {
		if (storageState != null)
			this.storageState.setStale(true);
		initializeState();
		rebuildLanes();
...
//    }
	...
	public Carrier populateCarrier(CarrierMes carrierMes) {
		return helper.populateCarrier(carrierMes); // To change body of implemented methods use File | Settings | File
													// Templates.
	}
	...
	public boolean spaceAvailable(Stop stop) {
		return helper.spaceAvailable(stop); // To change body of implemented methods use File | Settings | File
											// Templates.
	}
}
