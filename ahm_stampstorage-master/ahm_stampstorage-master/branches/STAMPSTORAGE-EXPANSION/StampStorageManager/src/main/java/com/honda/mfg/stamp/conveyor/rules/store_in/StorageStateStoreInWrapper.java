...
public class StorageStateStoreInWrapper {
	...
	public StorageRow getAppropriatePartialRowForDie(Long die, List<StorageRow> rows) {
		List<StorageRow> unsortedPartialRows = getStorageState().queryForRows(
				allOf(RowMatchers.isCurrentCapacityPartial(), RowMatchers.isLaneWithOnlySingleDieNumberOf(die)), rows);
		List<StorageRow> partialRowsWithMinimumPartsOfDie = unsortedPartialRows == null ? unsortedPartialRows
	...
	public StorageRow getAppropriateVacantRow(List<StorageRow> rows) {
		List<StorageRow> vacantRows = getStorageState().queryForRows(allOf(RowMatchers.isCurrentCapacityEmpty()), rows);
	...
	public StorageRow getAppropriatePartialRowWithEmptyCarriers(List<StorageRow> rows) {
		List<StorageRow> unsortedEmptyOnlyPartialRows = getStorageState()
				.queryForRows(allOf(RowMatchers.isCurrentCapacityPartial(), RowMatchers.hasEmptyCarriers()), rows);

		List<StorageRow> sortedRows = unsortedEmptyOnlyPartialRows == null ? unsortedEmptyOnlyPartialRows
	...
	public StorageRow getAppropriateMixedBackRowWithDie(Long die, List<StorageRow> rows) {
		List<StorageRow> unsortedMixedBackRows = getStorageState()
				.queryForRows(allOf(RowMatchers.isCurrentCapacityMixed(), RowMatchers.isCurrentCapacityPartial(),
						RowMatchers.hasCarrierAtTheLaneInWithDieNumber(die)), rows);
	...
	public StorageRow getAppropriateMixedRows(List<StorageRow> rows) {
		List<StorageRow> unsortedMixedRows = getStorageState().queryForRows(
				allOf(RowMatchers.isCurrentCapacityMixed(), RowMatchers.isCurrentCapacityPartial()), rows);
		List<StorageRow> mixedRows = unsortedMixedRows == null ? unsortedMixedRows
				: Ordering.from(RowComparators.getComparatorByCurrentCarrierCount()).sortedCopy(unsortedMixedRows);
	...
	public List<StorageRow> getStorageRowsByArea(StorageArea area) {
		List<StorageRow> rows = getStorageState()
				.queryForRows(allOf(RowMatchers.inStorageArea(area), RowMatchers.isLaneAvailable()));
		return rows;
	}
	...
	public StorageRow getAppropriatePartialRowForDieInStorageArea(Long die, StorageArea area) {
		...
		List<StorageRow> unsortedPartialRows = getStorageState()
				.queryForRows(allOf(RowMatchers.inStorageArea(area), RowMatchers.isLaneAvailable(),
						RowMatchers.isCurrentCapacityPartial(), RowMatchers.isLaneWithOnlySingleDieNumberOf(die)));
		List<StorageRow> partialRowsWithMinimumPartsOfDie = unsortedPartialRows == null ? unsortedPartialRows
				: Ordering.from(RowComparators.getComparatorByCarriersCountOfDieNumberAtLaneOut(die))
						.sortedCopy(unsortedPartialRows);
	...
	public StorageRow getAppropriateVacantRowInStorageArea(StorageArea area) {
		...
		List<StorageRow> vacantRows = getStorageState().queryForRows(allOf(RowMatchers.inStorageArea(area),
				RowMatchers.isLaneAvailable(), RowMatchers.isCurrentCapacityEmpty()));
		List<StorageRow> sortedById = vacantRows == null || vacantRows.size() == 0 ? null
				: Ordering.from(RowComparators.getComparatorByRowStopId()).sortedCopy(vacantRows);
		// return vacantRows == null || vacantRows.size() == 0 ? null :
		// vacantRows.get(0);
	...
	public StorageRow getAppropriateMixedBackRowWithDieInStorageArea(Long die, StorageArea area) {
		...
		List<StorageRow> unsortedMixedBackRows = getStorageState().queryForRows(allOf(RowMatchers.inStorageArea(area),
				RowMatchers.isLaneAvailable(), RowMatchers.isCurrentCapacityMixed(),
				RowMatchers.isCurrentCapacityPartial(), RowMatchers.hasCarrierAtTheLaneInWithDieNumber(die)));
		List<StorageRow> mixedRowsWithMinimumPartsOfDieAtRowIn = unsortedMixedBackRows == null ? unsortedMixedBackRows
				: Ordering.from(RowComparators.getComparatorByCarriersCountOfDieNumberAtLaneOut(die))
						.sortedCopy(unsortedMixedBackRows);
	...
	public StorageRow getAppropriateMixedRowInStorageArea(StorageArea area) {
		...
		List<StorageRow> unsortedMixedRows = getStorageState()
				.queryForRows(allOf(RowMatchers.inStorageArea(area), RowMatchers.isLaneAvailable(),
						RowMatchers.isCurrentCapacityMixed(), RowMatchers.isCurrentCapacityPartial()));
		List<StorageRow> mixedRows = unsortedMixedRows == null ? unsortedMixedRows
				: Ordering.from(RowComparators.getComparatorByCurrentCarrierCount()).sortedCopy(unsortedMixedRows);
	...
	public StorageRow getAppropriatePartialDifferentDieRowInStorageArea(Long die, StorageArea area) {
		StorageRow appropriateRow = null;
		if (area != null && die != null) {
			@SuppressWarnings("unchecked")
			List<StorageRow> candidateRows = getStorageState()
					.queryForRows(allOf(RowMatchers.isLaneAvailable(), RowMatchers.inStorageArea(area),
							RowMatchers.isCurrentCapacityPartial(), RowMatchers.isLaneWithSingleDieNumberNotOf(die)));
			if (candidateRows != null && candidateRows.size() > 0) {
				if (candidateRows.size() == 1)
					appropriateRow = candidateRows.get(0);
				else {
					// return the row with the lowest carrier count.
					int index = 0, lowest = 0;
					lowest = candidateRows.get(0).getCurrentCarrierCount();
					for (int i = 0; i < candidateRows.size(); i++) {
						if (candidateRows.get(i).getCurrentCarrierCount() < lowest) {
							lowest = candidateRows.get(i).getCurrentCarrierCount();
							index = i;
						}
					}
					appropriateRow = candidateRows.get(index);
				}
			}
		}
	...
	public StorageRow getAppropriatePartialRowWithEmptyCarriersInStorageArea(StorageArea area) {
		...
		List<StorageRow> unsortedEmptyOnlyPartialRows = getStorageState()
				.queryForRows(allOf(RowMatchers.inStorageArea(area), RowMatchers.isLaneAvailable(),
						RowMatchers.isCurrentCapacityPartial(), RowMatchers.hasEmptyCarriers()));
