...
public class CarrierReleaseProcessor {
			...
			if (atHeadOfStop && reachedDestination) {
				...
				if (carrierRelease != null && !helper.anyCarriersReleasingInStorageArea(currentLocation)) {
					...
					if (carrierRelease.getCurrentLocation().equals(currentLocation)) {
						...
						LOG.debug("Carrier-" + msg.getCarrierNumber() + " is not at-"
								+ carrierRelease.getCurrentLocation() + ", deleting carrier from release table");
					...
					if (!helper.anyCarriersSetToReleaseFromThisRow(currentLocation)) {
						...
						if (release != null) {
							if (carrierRelease.getCurrentLocation().equals(currentLocation)) {
								...
								LOG.debug("Carrier-" + msg.getCarrierNumber() + " is not at-"
										+ carrierRelease.getCurrentLocation()
										+ ", deleting carrier from release table");
								carrierRelease.remove();
							}
						}
					}
					...
					if (carrierRelease == null && currentLocation != null && currentLocation.isRowStop()) {
						if (helper.anyCarriersSetToReleaseFromThisRow(currentLocation)) {
							LOG.debug("Carrier-" + msg.getCarrierNumber() + " is at head Of row -"
									+ msg.getCurrentLocation() + " and carriers behind set to release ");
							...
							ReloadStorageStateMessage reloadStorageStateMessage = new ReloadStorageStateMessage(
									currentLocation);
	...
	public void releaseCheckProcessing(CarrierStatusMessage msg) {
		...
		if (currentLocation != null && (currentLocation.getStopType().equals(StopType.RELEASE_CHECK)
				|| (currentLocation.getStopArea().equals(StopArea.B_AREA)
						&& currentLocation.getStopType().equals(StopType.RECIRC_TO_ALL_ROWS)))) {
			...
			if (release != null) {
				...
				if (!helper.anyCarriersReleasingInStorageArea(carrierCurrLocation)) {
					if (release.getCurrentLocation().equals(carrierCurrLocation)) {
						...
						LOG.debug("Carrier-" + msg.getCarrierNumber() + " is not at-" + release.getCurrentLocation()
								+ ", deleting carrier from release table");
						release.remove();
					}
				}
	...
	public void updateStorageState(Carrier carrier, CarrierRelease carrierRelease) {
		...
		if (carrier.getCurrentLocation().isRowStop()
				&& !carrier.getCurrentLocation().equals(carrierRelease.getDestination())) {
			storage.getStorageState().removeCarrierFromRow(carrier.getCarrierNumber(),
					carrier.getCurrentLocation().getId());
		}
