...
/**
 * Created by IntelliJ IDEA. User: vcc30690 Date: 1/16/12 Time: 10:53 AM To
...
public class ReleaseManagerImpl implements ReleaseManager {
	...
	/**
	 * If Carrier is at head of row (buffer=1), and no other carriers releasing from
	 * this row, Publish an update message.
	 * 
	 * @param carrierNumber
	 * @param destination
	 * @param source
		...
		if (releaseMgr) {
			if (carrierMes.getBuffer().equals(Integer.valueOf(1))) {
				...
				if (releaseManagerHelper.anyCarriersReleasingInStorageArea(currentLocation)) {
					...
					LOG.info("No Carriers Releasing in storage area publishing carrier update for carrier-"
							+ carrierNumber);
					// creating a copy of carrier to make sure we send only carrier number and
					// destination information in carrier update
	...
	private void saveToCarrierReleaseTable(Integer carrierNumber, Stop currentLocation, Stop destination,
			String source) {
