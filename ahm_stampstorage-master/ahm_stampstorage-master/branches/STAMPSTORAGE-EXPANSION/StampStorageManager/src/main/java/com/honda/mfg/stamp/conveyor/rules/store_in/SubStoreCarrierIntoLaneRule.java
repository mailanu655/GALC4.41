...
/**
 * User: Adam S. Kendell Date: Mar 8, 2011
 */
public class SubStoreCarrierIntoLaneRule extends StoreInRuleBase {
	...
	public SubStoreCarrierIntoLaneRule(StorageStateContext storageStateContext, StoreInRule nextRule,
			StorageArea area) {
	...
	private StorageRow getAppropriateLane(List<StorageRow> lanes, Carrier carrier) {
		...
		LaneCondition[] laneConditions = { LaneCondition.PARTIAL, LaneCondition.VACANT, LaneCondition.EMPTY,
				LaneCondition.MIXED_BACK, LaneCondition.MIXED };
		for (LaneCondition condition : laneConditions) {
			switch (condition) {
			case PARTIAL:
				appropriateLane = storageStateStoreInWrapper.getAppropriatePartialRowForDie(carrier.getDie().getId(),
						lanes);
				break;
			case VACANT:
			...
			case EMPTY:
				...
				break;
			case MIXED_BACK:
				appropriateLane = storageStateStoreInWrapper.getAppropriateMixedBackRowWithDie(carrier.getDie().getId(),
						lanes);
				break;
			case MIXED:
