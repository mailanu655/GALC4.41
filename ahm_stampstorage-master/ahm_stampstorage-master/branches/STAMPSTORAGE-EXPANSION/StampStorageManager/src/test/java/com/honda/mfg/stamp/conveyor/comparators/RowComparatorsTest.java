...
public class RowComparatorsTest {
	...
	public void successfullySortListOfMixedBlockedLanesForPartType() {
		...
		List<StorageRow> mixedBlockedLanes = storageState.queryForRows(allOf(RowMatchers.isCurrentCapacityMixed(),
				RowMatchers.hasBlockedCarrierWithDieNumberAtTheLaneOut(part.getId())));

		List<StorageRow> sortedLanes = Ordering
				.from(RowComparators.getComparatorByDieNumberBlockingCarriersCountAtLaneOut(part.getId()))
	...
	public void successfullySortListOfMixedLanesForPartType() {
		...
		List<StorageRow> partialLanes = storageState
				.queryForRows(allOf(RowMatchers.isCurrentCapacityMixed(), new Not(RowMatchers.hasEmptyCarriers())));

		List<StorageRow> sortedLanes = Ordering
				.from(RowComparators.getComparatorByCarriersCountOfDieNumberAtLaneOut(part.getId()))
				.sortedCopy(partialLanes);

		assertEquals(3, sortedLanes.get(0).getCurrentCarrierCount());

		List<StorageRow> sortedLanes2 = Ordering
				.from(RowComparators.getComparatorByCarriersCountOfDieNumberAtLaneIn(part2.getId()))
				.sortedCopy(partialLanes);
