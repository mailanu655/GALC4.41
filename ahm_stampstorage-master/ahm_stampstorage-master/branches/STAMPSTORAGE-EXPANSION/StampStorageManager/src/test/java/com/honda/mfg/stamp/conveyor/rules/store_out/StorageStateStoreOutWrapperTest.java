...
public class StorageStateStoreOutWrapperTest {
	...
	public void findIfTotalCurrentCapacityOfLowCapacityLanesIsOverThresholdLimit() {
		StorageState storageState = new StorageStateImpl(
				getLanesForStoreOutTestMe(OM.A_AREA_CAPACITY, 0, StorageArea.A_AREA));
	...
	public void attemptingToFindIfTotalCurrentCapacityOfLowCapacityLanesIsOverThresholdLimitWhenNoLowCapacityLanesAvailable() {
		StorageState storageState = new StorageStateImpl(
				getLanesForStoreOutTestMe(OM.C_LOW_CAPACITY, 0, StorageArea.A_AREA));
		StorageStateContext context = new StorageStateContextMock(storageState);
		StorageStateStoreOutWrapper wrapper = new StorageStateStoreOutWrapper(context);
	...
	public void attemptingToGetLowCapacityLanesContainingOldestProductionRunPartCarriersForUnknownPart() {
		...
		List<StorageRow> lanes = wrapper.getStorageRowsWithOldestProductionRunPartCarriersForStorageArea(part,
				StorageArea.A_AREA);
		assertNull(lanes);
	}
	...
	public void attemptingToGetAppropriateFullLaneForPartTypeWhenNoFullLanesAvailable() {
		...
		Die part = new Die(101L, PartProductionVolume.HIGH_VOLUME);
		List<StorageRow> lowCapacityLanes = wrapper
				.getStorageRowsWithOldestProductionRunPartCarriersForStorageArea(part, StorageArea.A_AREA);
	...
	public List<StorageRow> getLanesForStoreOutTestMe(int capacity, int index, StorageArea area) {
		...
		for (int i = 0; i < 5; i++) {
			...
			Carrier c = getCarrier(101L, productionRunNo, timestamp, PartProductionVolume.HIGH_VOLUME,
					partialLane1.getStop(), partialLane1.getStop());
		...
		for (int i = 0; i < 3; i++) {
			...
			timestamp = new Timestamp(c1.getTimeInMillis());
			Carrier c = getCarrier(101L, productionRunNo, timestamp, PartProductionVolume.HIGH_VOLUME,
					partialLaneWithMinParts.getStop(), partialLaneWithMinParts.getStop());
		...
		for (int i = MAX_CAPACITY; i > 0; i--) {
			...
			timestamp = new Timestamp(c1.getTimeInMillis());
			Carrier c = getCarrier(101L, productionRunNo, timestamp, PartProductionVolume.HIGH_VOLUME,
					fullLane.getStop(), fullLane.getStop());
			fullLane.store(c);
		}
		...
		for (int i = 0; i < 5; i++) {
			...
			timestamp = new Timestamp(c1.getTimeInMillis());
			Carrier c = getCarrier(101L, productionRunNo, timestamp, PartProductionVolume.HIGH_VOLUME,
					mixedFrontLane.getStop(), mixedFrontLane.getStop());
			mixedFrontLane.store(c);
		}
		for (int i = 5; i < 9; i++) {
			c1.add(Calendar.DATE, -i);
			timestamp = new Timestamp(c1.getTimeInMillis());
			Carrier c = getCarrier(102L, productionRunNo2, timestamp, PartProductionVolume.HIGH_VOLUME,
					mixedFrontLane.getStop(), mixedFrontLane.getStop());
			mixedFrontLane.store(c);
		}
		...
		for (int i = 0; i < 5; i++) {
			c1.add(Calendar.DATE, -i);
			timestamp = new Timestamp(c1.getTimeInMillis());
			Carrier c = getCarrier(102L, productionRunNo, timestamp, PartProductionVolume.HIGH_VOLUME,
					mixedBlockedLane.getStop(), mixedBlockedLane.getStop());
		...
		for (int i = 5; i < 7; i++) {
			c1.add(Calendar.DATE, -i);
			timestamp = new Timestamp(c1.getTimeInMillis());
			Carrier c = getCarrier(101L, productionRunNo2, timestamp, PartProductionVolume.HIGH_VOLUME,
					mixedBlockedLane.getStop(), mixedBlockedLane.getStop());
			mixedBlockedLane.store(c);
		}
		...
		for (int i = 0; i < 3; i++) {
			...
			timestamp = new Timestamp(c1.getTimeInMillis());
			Carrier c = getCarrier(101L, productionRunNo, timestamp, PartProductionVolume.HIGH_VOLUME,
					mixedFrontWithMinimumParts.getStop(), mixedFrontWithMinimumParts.getStop());
		...
		for (int i = 0; i < 8; i++) {
			...
			timestamp = new Timestamp(c1.getTimeInMillis());
			Carrier c = getCarrier(102L, productionRunNo, timestamp, PartProductionVolume.HIGH_VOLUME,
					mixedBlockedLaneWithMax.getStop(), mixedBlockedLaneWithMax.getStop());
