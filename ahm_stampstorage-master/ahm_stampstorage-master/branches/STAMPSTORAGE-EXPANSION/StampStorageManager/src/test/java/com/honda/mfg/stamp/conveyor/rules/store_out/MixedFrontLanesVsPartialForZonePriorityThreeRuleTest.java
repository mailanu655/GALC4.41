...
public class MixedFrontLanesVsPartialForZonePriorityThreeRuleTest {
	...
	List<StorageRow> getMixedFrontLanes() {
		 ...
		 * production run lane3: mixed front med volume, low vol. at the back lane4: low
		 * volume, new production run, partial (capacity - 5)
		 * 
		 * C-High (capacity:20) ------ lane1: med volume, new prod. run, full lane
		 * lane2: med volume, old prod. run, full lane lane3: med vol old + 1 high vol
		 * new at the end lane4: med volume, new partial (capacity - 1)
		 * 
		 * C-Low (capacity:30) ------ lane1: empty new prod. run + 1 med vol new at the
		 * end lane2: med vol old full lane lane3: high vol old + 1 low vol new at the
		 * end lane4: high vol old full lane
		 * 
		 * B Area (capacity:31) ------ lane1: mixed front low vol: 2-low-SHIP, low-HOLD,
		 * low-SHIP, rest Empties lane2: high vol old full lane lane3: partial low
		 * volume (7 carriers) lane4: high vol old partial lane (capacity - 1)
		 * 
		...
		for (int i = 0; i < 5 - 1; i++) {
			aAreaLane3.store(getMediumVolumeCarrier(OM.PART_B, OM.PRODUCTION_RUN_NUMBER,
					OM.NEW_PRODUCTION_RUN_TIMESTAMP, aAreaLane3.getStop(), aAreaLane3.getStop()));
		}
		...
		cLowLane1.store(getMediumVolumeCarrier(OM.PART_B, OM.PRODUCTION_RUN_NUMBER, OM.NEW_PRODUCTION_RUN_TIMESTAMP,
				cLowLane1.getStop(), cLowLane1.getStop()));
		...
		bAreaLane1.store(getLowVolumeCarrier(OM.PART_C, OM.OLD_PRODUCTION_RUN_NUMBER, OM.OLD_PRODUCTION_RUN_TIMESTAMP,
				bAreaLane1.getStop(), bAreaLane1.getStop(), CarrierStatus.ON_HOLD));
		bAreaLane1.store(getLowVolumeCarrier(OM.PART_C, OM.OLD_PRODUCTION_RUN_NUMBER, OM.OLD_PRODUCTION_RUN_TIMESTAMP,
				bAreaLane1.getStop(), bAreaLane1.getStop()));
		for (int i = 0; i < bAreaLane1.getCapacity().intValue() - 4; i++) {
			bAreaLane1.store(getEmptyCarrier(OM.EMPTY, OM.OLD_PRODUCTION_RUN_NUMBER, OM.OLD_PRODUCTION_RUN_TIMESTAMP,
					bAreaLane1.getStop(), bAreaLane1.getStop()));
		}
		...
		for (int i = 0; i < 7; i++) {
			bAreaLane3.store(getLowVolumeCarrier(OM.PART_C, OM.OLD_PRODUCTION_RUN_NUMBER,
	...
	Carrier getLowVolumeCarrier(Long dieNumber, Integer prodRunRunNo, Timestamp productionRuntimestamp,
			Stop currentlocation, Stop destination, CarrierStatus cStatus) {
		Carrier c = getCarrier(dieNumber, prodRunRunNo, productionRuntimestamp, PartProductionVolume.LOW_VOLUME,
				currentlocation, destination);
