 ...
 */
public class RetrieveCarrierFromPartialLaneForZonePriorityTwoRuleTest {
	...
	public void successfullyRetrieveCarriersWithHighVolumeParts() {
		...
		StoreOutRule storeOutRule = new RetrieveCarrierFromRowOfZonePriorityAndRowCondition(context, null,
				LaneCondition.PARTIAL, StoragePriority.Priority.TWO);
	...
	private List<StorageRow> getLanes() {
		 ...
		 * full lane, old production run lane3: all med volume, old production run, last
		 * slot unfilled lane4: all med volume, new production run, last slot unfilled
		 * 
		 * C-Low (capacity:30) ------ lane1: empties, last 2 slots unfilled lane2: all
		 * high volume, old production run, last 2 slots unfilled lane3: low volume, old
		 * production run 1/2 lane lane4: high volume, full lane, old production run
		 * 
		 * B Area (capacity:31) ------ lane1: low volume, old production run, last 2
		 * slots unfilled lane2: high volume, old production run 1/2 lane lane3: med
		 * volume, old production run 1/3 lane lane4: high volume, other production run,
		 * last slot unfilled
		 * 
		...
		for (int i = 0; i < aAreaLane4.getCapacity() - 1; i++) {
			aAreaLane4.store(getLowVolumeCarrier(OM.PART_C, OM.PRODUCTION_RUN_NUMBER, OM.NEW_PRODUCTION_RUN_TIMESTAMP,
		...
		// cHighLane3.store(getHighVolumeCarrier(OM.PART_A, OM.PRODUCTION_RUN_NUMBER,
		// OM.NEW_PRODUCTION_RUN_TIMESTAMP, cHighLane3.getStop(),
		// cHighLane3.getStop()));
