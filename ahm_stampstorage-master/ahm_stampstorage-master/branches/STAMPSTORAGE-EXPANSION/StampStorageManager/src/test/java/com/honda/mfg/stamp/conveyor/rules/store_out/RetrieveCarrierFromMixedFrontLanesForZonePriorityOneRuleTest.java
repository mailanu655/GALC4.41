 ...
 */
public class RetrieveCarrierFromMixedFrontLanesForZonePriorityOneRuleTest {
	...
	public void successfullyRetrieveCarriersWithHighVolumeParts() {
		...
		StoreOutRule storeOutRule = new RetrieveCarrierFromRowOfZonePriorityAndRowCondition(context, null,
				LaneCondition.MIXED_FRONT, StoragePriority.Priority.ONE);
	...
	public void successfullyRetrieveCarriersWithLowVolumeParts() {
		...
		// Post condition / assertions
		assertNotNull(StorageRow);
		assertEquals(StorageArea.C_HIGH, StorageRow.getStorageArea());
		Carrier carrier = StorageRow.release();
		assertEquals(OM.PART_C, carrier.getDie().getId());
		assertEquals(OM.OLD_PRODUCTION_RUN_NUMBER, carrier.getProductionRunNo());
		assertEquals(StorageRow.getCapacity().intValue(), OM.C_HIGH_CAPACITY.intValue());
	}
	...
	public void successfullyRetrieveEmptyCarriers() {
		...
		assertEquals(StorageRow.getCapacity(), OM.A_AREA_CAPACITY);
		assertTrue(
				!StorageRow.isEmpty() && !StorageRow.isFull() && StorageRow.getDieNumbersForAllCarriers().size() > 1);
	}
...
//    }
	...
	List<StorageRow> getMixedFrontLanes() {
		...
		/**
		 * Setup the storage state as follows: A Area (capacity:10) ------ lane1: high
		 * volume, full lane, new production run lane2: high volume, full lane, old
		 * production run lane3: low volume, old production run + 1 low vol new in the
		 * end lane4: low volume, full lane, new production run
		 * 
		 * C-High (capacity:20) ------ lane1: low volume old, half lane + med vol new
		 * second half lane2: med volume, full lane, old production run lane3: med
		 * volume, old production run + 1 high vol new at the end lane4: empties + 1 med
		 * vol new at the end
		 * 
		 * C-Low (capacity:30) ------ lane1: empties + 1 low vol old at the end lane2:
		 * med volume old first half lane + low vol new second half lane3: high volume,
		 * old production run + 1 low vol new in the end lane4: high volume, full lane,
		 * old production run
		 * 
		 * B Area (capacity:31) ------ lane1: low volume, old production run + 1 high
		 * vol old at the end lane2: med volume, old production run + 1 high vol old in
		 * the end lane3: high volume, old production run + 1 new med vol in the end
		 * lane4: high volume, full lane, other production run
		 * 
		 * S Area (capacity:87) ------ lane1: high vol old first half lane + med old
		 * second half lane2: low volume, full lane, old production run
		 * 
		 */

		// low volume capacity
		for (int i = 0; i < aAreaLane1.getCapacity(); i++) {
			aAreaLane1.store(getHighVolumeCarrier(OM.PART_A, OM.PRODUCTION_RUN_NUMBER, OM.NEW_PRODUCTION_RUN_TIMESTAMP,
					aAreaLane1.getStop(), aAreaLane1.getStop()));
		}

		for (int i = 0; i < aAreaLane2.getCapacity(); i++) {
			aAreaLane2.store(getHighVolumeCarrier(OM.PART_A, OM.OLD_PRODUCTION_RUN_NUMBER,
					OM.OLD_PRODUCTION_RUN_TIMESTAMP, aAreaLane2.getStop(), aAreaLane2.getStop()));
		}

		for (int i = 0; i < aAreaLane3.getCapacity() - 1; i++) {
			aAreaLane3.store(getLowVolumeCarrier(OM.PART_A, OM.OLD_PRODUCTION_RUN_NUMBER,
					OM.OLD_PRODUCTION_RUN_TIMESTAMP, aAreaLane3.getStop(), aAreaLane3.getStop()));
		}
		aAreaLane3.store(getLowVolumeCarrier(OM.PART_A, OM.PRODUCTION_RUN_NUMBER, OM.NEW_PRODUCTION_RUN_TIMESTAMP,
				aAreaLane3.getStop(), aAreaLane3.getStop()));
		for (int i = 0; i < aAreaLane4.getCapacity() - 3; i++) {
			aAreaLane4.store(getEmptyCarrier(OM.EMPTY, OM.PRODUCTION_RUN_NUMBER, OM.NEW_PRODUCTION_RUN_TIMESTAMP,
					aAreaLane4.getStop(), aAreaLane4.getStop()));
		}
		aAreaLane4.store(getLowVolumeCarrier(OM.PART_B, OM.PRODUCTION_RUN_NUMBER, OM.NEW_PRODUCTION_RUN_TIMESTAMP,
				aAreaLane4.getStop(), aAreaLane4.getStop()));
		// medium volume capacity
		for (int i = 0; i < cHighLane1.getCapacity() / 2; i++) {
			cHighLane1.store(getLowVolumeCarrier(OM.PART_C, OM.OLD_PRODUCTION_RUN_NUMBER,
					OM.OLD_PRODUCTION_RUN_TIMESTAMP, cHighLane1.getStop(), cHighLane1.getStop()));
		}
		for (int i = 1 + (cHighLane1.getCapacity() / 2); i < cHighLane1.getCapacity(); i++) {
		...
		cHighLane3.store(getHighVolumeCarrier(OM.PART_A, OM.PRODUCTION_RUN_NUMBER, OM.NEW_PRODUCTION_RUN_TIMESTAMP,
				cHighLane3.getStop(), cHighLane3.getStop()));

		for (int i = 0; i < cHighLane4.getCapacity() - 1; i++) {
			cHighLane4.store(getEmptyCarrier(OM.EMPTY, OM.PRODUCTION_RUN_NUMBER, OM.NEW_PRODUCTION_RUN_TIMESTAMP,
					cHighLane4.getStop(), cHighLane4.getStop()));
		}
		cHighLane4.store(getMediumVolumeCarrier(OM.PART_B, OM.PRODUCTION_RUN_NUMBER, OM.NEW_PRODUCTION_RUN_TIMESTAMP,
				cHighLane4.getStop(), cHighLane4.getStop()));

		// high volume capacity
		for (int i = 0; i < cLowLane1.getCapacity() / 2; i++) {
			cLowLane1.store(getLowVolumeCarrier(OM.PART_C, OM.OLD_PRODUCTION_RUN_NUMBER,
					OM.OLD_PRODUCTION_RUN_TIMESTAMP, cLowLane1.getStop(), cLowLane1.getStop()));
		}
		for (int i = 1 + (cLowLane1.getCapacity() / 2); i < cLowLane1.getCapacity(); i++) {
			cLowLane1.store(getEmptyCarrier(OM.EMPTY, OM.PRODUCTION_RUN_NUMBER, OM.NEW_PRODUCTION_RUN_TIMESTAMP,
					cLowLane1.getStop(), cLowLane1.getStop()));
		}

		for (int i = 0; i < cLowLane2.getCapacity() / 2; i++) {
			cLowLane2.store(getMediumVolumeCarrier(OM.PART_B, OM.OLD_PRODUCTION_RUN_NUMBER,
					OM.OLD_PRODUCTION_RUN_TIMESTAMP, cLowLane2.getStop(), cLowLane2.getStop()));
		}
		for (int i = 1 + (cLowLane2.getCapacity() / 2); i < cLowLane2.getCapacity(); i++) {
			cLowLane2.store(getLowVolumeCarrier(OM.PART_A, OM.PRODUCTION_RUN_NUMBER, OM.NEW_PRODUCTION_RUN_TIMESTAMP,
					cLowLane2.getStop(), cLowLane2.getStop()));
		}
		...
		cLowLane3.store(getLowVolumeCarrier(OM.PART_C, OM.PRODUCTION_RUN_NUMBER, OM.NEW_PRODUCTION_RUN_TIMESTAMP,
				cLowLane3.getStop(), cLowLane3.getStop()));
		for (int i = 0; i < cLowLane4.getCapacity(); i++) {
			cLowLane4.store(getHighVolumeCarrier(OM.PART_A, OM.OLD_PRODUCTION_RUN_NUMBER,
					OM.OLD_PRODUCTION_RUN_TIMESTAMP, cLowLane4.getStop(), cLowLane4.getStop()));
		}

		// high volume capacity
		for (int i = 0; i < bAreaLane1.getCapacity() / 2; i++) {
			bAreaLane1.store(getEmptyCarrier(OM.EMPTY, OM.PRODUCTION_RUN_NUMBER, OM.NEW_PRODUCTION_RUN_TIMESTAMP,
					bAreaLane1.getStop(), bAreaLane1.getStop()));
		}
		for (int i = 1 + (bAreaLane1.getCapacity() / 2); i < bAreaLane1.getCapacity(); i++) {
			bAreaLane1.store(getLowVolumeCarrier(OM.PART_C, OM.OLD_PRODUCTION_RUN_NUMBER,
					OM.OLD_PRODUCTION_RUN_TIMESTAMP, bAreaLane1.getStop(), bAreaLane1.getStop()));
		}

		for (int i = 0; i < bAreaLane2.getCapacity() - 1; i++) {
			bAreaLane2.store(getMediumVolumeCarrier(OM.PART_B, OM.OLD_PRODUCTION_RUN_NUMBER,
					OM.OLD_PRODUCTION_RUN_TIMESTAMP, bAreaLane2.getStop(), bAreaLane2.getStop()));
		}
		bAreaLane2.store(getHighVolumeCarrier(OM.PART_A, OM.OLD_PRODUCTION_RUN_NUMBER, OM.OLD_PRODUCTION_RUN_TIMESTAMP,
				bAreaLane2.getStop(), bAreaLane2.getStop()));
		...
		for (int i = 0; i < sAreaLane1.getCapacity() / 2; i++) {
		...
		for (int i = 1 + (sAreaLane1.getCapacity() / 2); i < sAreaLane1.getCapacity(); i++) {
	...
	Carrier getEmptyCarrier(Long dieNumber, Integer prodRunRunNo, Timestamp productionRuntimestamp,
			Stop currentlocation, Stop destination) {
		return getCarrier(dieNumber, prodRunRunNo, productionRuntimestamp, PartProductionVolume.EMPTY, currentlocation,
				destination);
	}
