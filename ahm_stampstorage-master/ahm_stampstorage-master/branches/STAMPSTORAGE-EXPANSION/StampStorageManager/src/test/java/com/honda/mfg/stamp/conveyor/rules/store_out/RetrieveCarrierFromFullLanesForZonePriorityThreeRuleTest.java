 ...
 */
public class RetrieveCarrierFromFullLanesForZonePriorityThreeRuleTest {
	...
	public void successfullyRetrieveCarriersWithHighVolumeParts() {
		...
		StoreOutRule storeOutRule = new RetrieveCarrierFromRowOfZonePriorityAndRowCondition(context, null,
				LaneCondition.FULL, StoragePriority.Priority.THREE);
	...
	public void successfullyRetrieveEmptyCarriers() {
		...
		assertEquals(StorageRow.getCapacity(), OM.B_AREA_CAPACITY);
	}
	...
	private List<StorageRow> getLanes() {
		...
		/**
		 * Setup the storage state as follows: A Area ------ lane1: high volume, full
		 * lane, new production run lane2: high volume, full lane, old production run
		 * lane3: low volume, old production run + 1 new in the end lane4: low volume,
		 * full lane, new production run
		 * 
		 * C-High ------ lane1: med volume, full lane, new production run lane2: med
		 * volume, full lane, old production run lane3: med volume, old production run +
		 * 1 high vol, new run in the end lane4: med volume, full lane, new production
		 * run
		 * 
		 * C-Low ------ lane1: empty, full lane lane2: med volume, full lane, old
		 * production run lane3: low volume, old production run + 1 new in the end
		 * lane4: high volume, full lane, old production run
		 * 
		 * B Area ------ lane1: low volume, full lane, old production run lane2: high
		 * volume, full lane, old production run lane3: high volume, old production run
		 * + 1 new med vol in the end lane4: high volume, full lane, old production run
		 * 
		 * S Area ------ lane1: med volume, full lane, old production run lane2: low
		...
		for (int i = 0; i < bAreaLane4.getCapacity().intValue(); i++) {
			bAreaLane4.store(getEmptyCarrier(OM.EMPTY, OM.OTHER_PRODUCTION_RUN_NUMBER, OM.OLD_PRODUCTION_RUN_TIMESTAMP,
