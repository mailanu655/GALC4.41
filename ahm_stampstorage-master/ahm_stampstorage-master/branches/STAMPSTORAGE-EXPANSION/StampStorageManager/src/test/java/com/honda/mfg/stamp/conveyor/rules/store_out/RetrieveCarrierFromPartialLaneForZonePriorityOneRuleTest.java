...
/**
 * User: Vivek Bettada Date: 10/22/13
 */
public class RetrieveCarrierFromPartialLaneForZonePriorityOneRuleTest {
	...
	public void successfullyRetrieveCarriersWithHighVolumeParts() {
		...
		StoreOutRule storeOutRule = new RetrieveCarrierFromRowOfZonePriorityAndRowCondition(context, null,
				LaneCondition.PARTIAL, StoragePriority.Priority.ONE);
	...
	@Test
	public void successfullyRetrieveEmptyCarriersFromLowCapacityLane() {
		// Pre conditions
		StorageStateContext context = getContext();
		StoreOutRule storeOutRule = new RetrieveCarrierFromRowOfZonePriorityAndRowCondition(context, null,
				LaneCondition.PARTIAL, StoragePriority.Priority.ONE);
		...
		assertTrue(!StorageRow.isEmpty() && !StorageRow.isFull());
	}
	...
	private List<StorageRow> getLanes() {
		...
		StorageRow bAreaLane1 = OM.lane_A_AREA_Empty();
		bAreaLane1.setStop(new Stop(41L));
		StorageRow bAreaLane2 = OM.lane_A_AREA_Empty();
		bAreaLane2.setStop(new Stop(42L));
		StorageRow bAreaLane3 = OM.lane_A_AREA_Empty();
		bAreaLane3.setStop(new Stop(43L));
		StorageRow bAreaLane4 = OM.lane_A_AREA_Empty();
		 ...
		 * end lane4: all low volume, new production run - last slot unfilled
		 * 
		 * C-High (capacity:20) ------ lane1: empty carriers 1/3 lane lane2: med volume,
		 * full lane, old production run lane3: low volume old - 1/2 lane lane4: all med
		 * volume, new production run, last slot unfilled
		 * 
		 * C-Low (capacity:30) ------ lane1: empties, last 2 slots unfilled lane2: med
		 * vol old 1/3 lane lane3: low volume, old production run 1/2 lane lane4: high
		 * volume, full lane, old production run
		 * 
		 * B Area (capacity:31) ------ lane1: empties, last 2 slots unfilled lane2: high
		 * volume, old production run 1/2 lane lane3: med volume, old production run 1/3
		 * lane lane4: high volume, other production run, last slot unfilled
		 * 
		 * S Area (capacity:87) ------ lane1: med volume, full lane, old production run
		 * lane2: high volume, old production run 1/2 lane
		 * 
		 */
		...
		for (int i = 0; i < cHighLane1.getCapacity() / 3; i++) {
		...
		for (int i = 0; i < cHighLane3.getCapacity() / 2; i++) {
			cHighLane3.store(getLowVolumeCarrier(OM.PART_C, OM.OLD_PRODUCTION_RUN_NUMBER,
		...
		for (int i = 0; i < cLowLane2.getCapacity() / 3; i++) {
		...
		for (int i = 0; i < cLowLane3.getCapacity() / 2; i++) {
			cLowLane3.store(getLowVolumeCarrier(OM.PART_C, OM.OLD_PRODUCTION_RUN_NUMBER,
		...
		// high volume capacity
		for (int i = 0; i < bAreaLane1.getCapacity().intValue() - 2; i++) {
		...
		for (int i = 0; i < bAreaLane2.getCapacity() / 2; i++) {
		...
		for (int i = 0; i < bAreaLane3.getCapacity() / 3; i++) {
			bAreaLane3.store(getMediumVolumeCarrier(OM.PART_B, OM.OLD_PRODUCTION_RUN_NUMBER,
		...
		for (int i = 0; i < sAreaLane2.getCapacity() / 3; i++) {
