...
//    }
	...
	List<StorageRow> getMixedFrontLanes() {
		...
		for (int i = 0; i < 2; i++) {
			aAreaLane1.store(getMediumVolumeCarrier(OM.PART_B, OM.OLD_PRODUCTION_RUN_NUMBER,
		...
		for (int i = 0; i < 3; i++) {
			aAreaLane4.store(getLowVolumeCarrier(OM.PART_B, OM.OLD_PRODUCTION_RUN_NUMBER,
					OM.OLD_PRODUCTION_RUN_TIMESTAMP, aAreaLane4.getStop(), aAreaLane4.getStop()));
		}
