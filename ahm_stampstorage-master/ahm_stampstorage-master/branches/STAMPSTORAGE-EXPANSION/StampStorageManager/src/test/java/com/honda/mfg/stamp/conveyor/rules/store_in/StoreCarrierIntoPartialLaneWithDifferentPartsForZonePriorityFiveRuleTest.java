...
public class StoreCarrierIntoPartialLaneWithDifferentPartsForZonePriorityFiveRuleTest {
	...
	public void successfullyStoreEmptyCarrier() {
		...
		ignored4LaneImpl.store(OM.carrier_HighVolume_PartA()); // first in StorageRow
		...
		StoreInRule storeInRule = new StoreCarrierIntoRowOfZonePriorityAndRowCondition(OM.storageState(lanes, null),
				null, LaneCondition.PARTIAL_DIFFERENT_DIE, StoragePriority.Priority.FIVE);
		StorageRow actualLaneImpl = storeInRule.processRule(OM.carrier_Empty());
		System.out.println(actualLaneImpl);
