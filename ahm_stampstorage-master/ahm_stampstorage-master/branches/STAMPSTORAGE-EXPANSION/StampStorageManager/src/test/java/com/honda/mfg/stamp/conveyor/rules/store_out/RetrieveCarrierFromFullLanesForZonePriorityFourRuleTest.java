...
public class RetrieveCarrierFromFullLanesForZonePriorityFourRuleTest {
	...
	public void successfullyRetrieveCarriersWithHighVolumeParts() {
		...
		StoreOutRule storeOutRule = new RetrieveCarrierFromRowOfZonePriorityAndRowCondition(context, null,
				LaneCondition.FULL, StoragePriority.Priority.FOUR);
	...
	private List<StorageRow> getLanes() {
		...
		for (int i = 0; i < aAreaLane1.getCapacity(); i++) {
			aAreaLane1.store(getEmptyCarrier(OM.EMPTY, 999, OM.OLD_PRODUCTION_RUN_TIMESTAMP, aAreaLane1.getStop(),
					aAreaLane1.getStop()));
		}

		for (int i = 0; i < aAreaLane2.getCapacity(); i++) {
			aAreaLane2.store(getMediumVolumeCarrier(OM.PART_B, OM.OLD_PRODUCTION_RUN_NUMBER,
					OM.OLD_PRODUCTION_RUN_TIMESTAMP, aAreaLane2.getStop(), aAreaLane2.getStop()));
		}

		for (int i = 0; i < aAreaLane3.getCapacity() - 1; i++) {
			aAreaLane3.store(getMediumVolumeCarrier(OM.PART_B, OM.OLD_PRODUCTION_RUN_NUMBER,
					OM.OLD_PRODUCTION_RUN_TIMESTAMP, aAreaLane3.getStop(), aAreaLane3.getStop()));
		}
		aAreaLane3.store(getMediumVolumeCarrier(OM.PART_A, OM.PRODUCTION_RUN_NUMBER, OM.NEW_PRODUCTION_RUN_TIMESTAMP,
				aAreaLane3.getStop(), aAreaLane3.getStop()));
		for (int i = 0; i < aAreaLane4.getCapacity(); i++) {
			aAreaLane4.store(getLowVolumeCarrier(OM.PART_C, OM.OLD_PRODUCTION_RUN_NUMBER,
		...
		for (int i = 0; i < cHighLane1.getCapacity(); i++) {
			cHighLane1.store(getHighVolumeCarrier(OM.PART_A, OM.PRODUCTION_RUN_NUMBER, OM.NEW_PRODUCTION_RUN_TIMESTAMP,
		...
		for (int i = 0; i < cHighLane3.getCapacity(); i++) {
			cHighLane3.store(getHighVolumeCarrier(OM.PART_A, OM.OLD_PRODUCTION_RUN_NUMBER,
		...
		// OM.NEW_PRODUCTION_RUN_TIMESTAMP, cHighLane3.getStop(),cHighLane3.getStop()));
		for (int i = 0; i < cHighLane4.getCapacity(); i++) {
			cHighLane4.store(getEmptyCarrier(OM.EMPTY, OM.OTHER_PRODUCTION_RUN_NUMBER, OM.OLD_PRODUCTION_RUN_TIMESTAMP,
		...
		cLowLane3.store(getMediumVolumeCarrier(OM.PART_B, OM.PRODUCTION_RUN_NUMBER, OM.NEW_PRODUCTION_RUN_TIMESTAMP,
				cLowLane3.getStop(), cLowLane3.getStop()));
		for (int i = 0; i < cLowLane4.getCapacity() - 1; i++) {
			cLowLane4.store(getHighVolumeCarrier(OM.PART_A, OM.OTHER_PRODUCTION_RUN_NUMBER,
