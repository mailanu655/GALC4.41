...
/**
 * User: Jeffrey M Lutz Date: 3/2/11
 */
public class StorageStateImplTest {
	...
	public void findLaneThatIsFull() {
		StorageRow actualLaneImpl = LaneOM.storageState.queryForRow(not(RowMatchers.isCurrentCapacityEmpty()));
		assertEquals(LaneOM.partialPartOneLaneImpl, actualLaneImpl);
	}
	...
	public void findLaneThatIsEmptyAndGivenMaxCapacity() {
		...
		actualLaneImpl = LaneOM.storageState.queryForRow(
				allOf(RowMatchers.isCurrentCapacityEmpty(), RowMatchers.hasMaxCapacityOf(LaneOM.MAX_CAPACITY)));
		assertEquals(LaneOM.emptyLaneImpl, actualLaneImpl);
	}
	...
	public void findLaneThatIsPartialCapacityAndGivenMaxCapacityWithOnlySamePartType() {
		StorageRow actualLaneImpl;
		actualLaneImpl = LaneOM.storageState.queryForRow(
				allOf(RowMatchers.isCurrentCapacityPartial(), RowMatchers.hasMaxCapacityOf(LaneOM.MAX_CAPACITY),
	...
	public void findLanesThatAreMixedAndGivenMaxCapacity() {
		...
		actualLaneImpl = LaneOM.storageState.queryForRows(
				allOf(RowMatchers.isCurrentCapacityMixed(), RowMatchers.hasMaxCapacityOf(LaneOM.MAX_CAPACITY)));
	...
	public void testEquals() {
		...
		carrier = new Carrier(id++, productionRunTimestamp, prodRunNo, currentLocation, destination, carrierStatus,
				tagID, new Die(10L, PartProductionVolume.HIGH_VOLUME));
		// carrier.setQuantity(10);

		Carrier carrier1 = new Carrier(id++, productionRunTimestamp, prodRunNo, currentLocation, destination,
				carrierStatus, tagID, new Die(10L, PartProductionVolume.HIGH_VOLUME));
		...
		Carrier carrier3 = new Carrier(id++, productionRunTimestamp, prodRunNo, currentLocation, destination,
				carrierStatus, tagID, new Die(10L, PartProductionVolume.HIGH_VOLUME));
		// carrier3.setQuantity(10);

		Carrier carrier4 = new Carrier(id++, productionRunTimestamp, prodRunNo, currentLocation, destination,
				carrierStatus, tagID, new Die(10L, PartProductionVolume.HIGH_VOLUME));
	...
	public void successfullyTestIfCarrierPartsAreaOnBackOrder() {
		...
		assertFalse(storageState.isCarrierPartsOnBackOrder(LaneOM.partOneCarrier.getDie()));

		List<BackOrder> backOrder = new ArrayList<BackOrder>();
		backOrder.add(new BackOrder(1L, LaneOM.partOneCarrier.getDie().getId()));
		storageState.setBackOrder(backOrder);
		Die die1 = new Die(1L, PartProductionVolume.HIGH_VOLUME);
		assertTrue(storageState.isCarrierPartsOnBackOrder(die1));
		assertTrue(storageState.isCarrierPartsOnBackOrder(LaneOM.partOneCarrier.getDie()));
