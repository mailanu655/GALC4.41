...
public class RetrieveCarrierFromFullLanesForZonePriorityOneRuleTest {
	...
	public void successfullyRetrieveCarriersWithHighVolumeParts() {
		...
		StoreOutRule storeOutRule = new RetrieveCarrierFromRowOfZonePriorityAndRowCondition(context, null,
				LaneCondition.FULL, StoragePriority.Priority.ONE);
	...
	private List<StorageRow> getLanes() {
		 ...
		 * production run lane3: low volume, old production run + 1 new in the end
		 * lane4: low volume, full lane, new production run
		 * 
		 * C-High (capacity:20) ------ lane1: empty, full lane lane2: low volume, full
		 * lane, old production run lane3: med volume, old production run + 1 high vol,
		 * new run in the end lane4: med volume, full lane, new production run
		 * 
		 * C-Low (capacity:30) ------ lane1: empty, full lane lane2: med volume, full
		 * lane, old production run lane3: low volume, old production run + 1 new in the
		 * end lane4: low volume, full lane, old production run
		 * 
		 * B Area (capacity:31) ------ lane1: med volume, full lane, old production run
		 * lane2: high volume, full lane, old production run lane3: high volume, old
		 * production run + 1 new med vol in the end lane4: empty, full lane
		 * 
		...
		for (int i = 0; i < aAreaLane1.getCapacity(); i++) {
			aAreaLane1.store(getEmptyCarrier(OM.EMPTY, OM.PRODUCTION_RUN_NUMBER, OM.NEW_PRODUCTION_RUN_TIMESTAMP,
		...
		for (int i = 0; i < cHighLane2.getCapacity(); i++) {
			cHighLane2.store(getLowVolumeCarrier(OM.PART_C, OM.OLD_PRODUCTION_RUN_NUMBER,
		...
		for (int i = 0; i < cLowLane4.getCapacity(); i++) {
			cLowLane4.store(getLowVolumeCarrier(OM.PART_C, OM.OLD_PRODUCTION_RUN_NUMBER,
