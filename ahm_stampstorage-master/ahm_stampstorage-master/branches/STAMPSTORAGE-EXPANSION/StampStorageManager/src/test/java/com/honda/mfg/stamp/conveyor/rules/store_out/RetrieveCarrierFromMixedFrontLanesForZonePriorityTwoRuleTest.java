...
public class RetrieveCarrierFromMixedFrontLanesForZonePriorityTwoRuleTest {
	...
	public void successfullyRetrieveCarriersWithHighVolumeParts() {
		...
		StoreOutRule storeOutRule = new RetrieveCarrierFromRowOfZonePriorityAndRowCondition(context, null,
				LaneCondition.MIXED_FRONT, StoragePriority.Priority.TWO);
...
//    }
	...
	List<StorageRow> getMixedFrontLanes() {
		 ...
		 * 
		 * C-High (capacity:20) ------ lane1: med volume, full lane, new production run
		 * lane2: med volume, full lane, old production run lane3: med volume, old
		 * production run + 1 high vol new at the end lane4: empties + 1 med vol new at
		 * the end
		 * 
		 * C-Low (capacity:30) ------ lane1: empties + 1 low vol old at the end lane2:
		 * med volume, full lane, old production run lane3: high volume, old production
		 * run + 1 low vol new in the end lane4: high volume, full lane, old production
		 * run
		 * 
		...
		for (int i = 0; i < cLowLane3.getCapacity() - 1; i++) {
			cLowLane3.store(getEmptyCarrier(OM.EMPTY, OM.OLD_PRODUCTION_RUN_NUMBER, OM.OLD_PRODUCTION_RUN_TIMESTAMP,
		...
		bAreaLane1.store(getHighVolumeCarrier(OM.PART_A, OM.OLD_PRODUCTION_RUN_NUMBER, OM.OLD_PRODUCTION_RUN_TIMESTAMP,
