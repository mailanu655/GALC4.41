...
public class LaneOM {
	...
	static {
		...
		partOneCarrier = new Carrier(1, new Die(PART_A, PartProductionVolume.HIGH_VOLUME), 1, timestamp1,
				productionRunNo);
		partTwoCarrier = new Carrier(2, new Die(PART_B, PartProductionVolume.HIGH_VOLUME), 1, timestamp2,
				productionRunNo);
		...
		mixedWithSamePartCarrierEndLaneImpl = new StorageRow(index++,
				"mixed with same part carrier at end of StorageRow", MAX_CAPACITY, 1);
		mixedWithSamePartCarrierEndLaneImpl.store(partTwoCarrier);
		mixedWithSamePartCarrierEndLaneImpl.store(partOneCarrier);

		mixedWithDifferentPartCarrierEndLaneImpl = new StorageRow(index++,
				"mixed with different part carrier at end of StorageRow", MAX_CAPACITY, 1);
		...
		for (int i = 0; i < MAX_CAPACITY; i++) {
			...
			fullLaneImpl.store(
					new Carrier(i, new Die(PART_A, PartProductionVolume.HIGH_VOLUME), 1, timestamp, productionRunNo));
		}
