 ...
 */
public class RetrieveCarrierFromMixedBlockedLanesForZonePriorityTwoRuleTest {
	...
	public void successfullyRetrieveCarriersWithHighVolumeParts() {
		...
		assertNotNull(sRow);
		assertEquals(StorageArea.B_AREA, sRow.getStorageArea());
		...
		assertEquals(sRow.getCapacity().intValue(), OM.B_AREA_CAPACITY.intValue());
	}
...
//    }
	...
	List<StorageRow> getMixedFrontLanes() {
		 ...
		 * 
		 * C-High (capacity:20) ------ lane1: empty, full lane lane2: med volume, full
		 * lane, old production run lane3: Single high volume new followed by all medium
		 * new till the end lane4: med volume, full lane, new production run
		 * 
		 ...
		 * old production run lane4: Single low volume new followed by high volume old
		 * till the end
		 * 
		 ...
		 * end lane4: high volume, full lane, new production run
		 * 
		...
		bAreaLane2.store(getLowVolumeCarrier(OM.PART_C, OM.PRODUCTION_RUN_NUMBER, OM.NEW_PRODUCTION_RUN_TIMESTAMP,
		...
		for (int i = 0; i < bAreaLane4.getCapacity().intValue() - 1; i++) {
			bAreaLane4.store(getHighVolumeCarrier(OM.PART_A, OM.OTHER_PRODUCTION_RUN_NUMBER,
					OM.NEW_PRODUCTION_RUN_TIMESTAMP, bAreaLane4.getStop(), bAreaLane4.getStop()));
		}
