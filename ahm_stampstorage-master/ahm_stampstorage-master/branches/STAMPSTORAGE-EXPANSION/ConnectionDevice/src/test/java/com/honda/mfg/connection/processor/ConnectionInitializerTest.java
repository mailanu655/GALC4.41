...
public class ConnectionInitializerTest {
	...
	public void successfullyInitializeDeviceInNewThread() {
		...
		when(pair.getInitializeRequiredTime()).thenReturn(expectedFirst).thenReturn(expectedSecond)
				.thenReturn(expectedSecond).thenReturn(expectedThird).thenReturn(expectedFourth);
	...
	public void successfullyReceiveAsyncInitializedMessage() {
		...
		when(pair.getRequestProcessor()).thenReturn(request);
		when(pair.getInitializeRequiredTime()).thenReturn(1L);
	...
	public void throwsExceptionAttemptingToStartResponseProcessorInsideProcessorPair() {
		...
		when(pair.getRequestProcessor()).thenReturn(request);
		when(pair.getInitializeRequiredTime()).thenReturn(-1L).thenReturn(0L).thenReturn(1L);
		ConnectionInitializer initializer = new ConnectionInitializer(pair);

		// Perform test - Attempt to re-initialize but produces exception in
		// sendRequest()
		initializer.run();
