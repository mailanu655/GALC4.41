...
public class ConnectionResponseProcessorTest {
	...
	@Test
	public void successfullyStartResponseProcessorInAutoMode() throws InterruptedException {
		// Pre-conditions
		...
		processor.setMsgPublisher(new ConnectionResponsePublisher("Test"));
		when(reader.getResponse()).thenReturn("HELLO").thenThrow(new CommunicationsException("I am broken..."));
	...
	@Test
	public void successfullyStartResponseProcessorAndThenStopProcessing() throws InterruptedException {
		// Pre-conditions
		ConnectionResponseReader reader = mock(ConnectionResponseReader.class);
	...
	public void throwsExceptionAttemptingToGetResponse() {
		...
		when(responseReader.getResponse()).thenThrow(responseTimeoutException).thenReturn("{hello}")
				.thenThrow(networkCommunicationException).thenThrow(communicationException);

		// Perform test
		ConnectionResponseProcessor processor = new ConnectionResponseProcessor(responseReader, false, true);
		processor.setMsgPublisher(new ConnectionResponsePublisher("Test"));
		...
		int i = 0;
		ResponseTimeoutException actualResponseTimeoutException = (ResponseTimeoutException) processor.getSequence()
				.get(i++);
		...
		NetworkCommunicationException actualException = (NetworkCommunicationException) processor.getSequence()
				.get(i++);
		assertEquals(networkCommunicationException, actualException);

		CommunicationsException actualCommunicationsException = (CommunicationsException) processor.getSequence()
				.get(i++);
