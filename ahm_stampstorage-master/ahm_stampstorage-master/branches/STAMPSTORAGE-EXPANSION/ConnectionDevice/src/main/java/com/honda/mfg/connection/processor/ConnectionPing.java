...
public class ConnectionPing implements DevicePing {
	...
	/**
	 * this method called from {@link WatchdogAdapter#run()} it checks the elapsed
	 * time since latestPing. If elapsed time as a multiple of pollIntervalSec is
	 * greater than passiveWaitCount, returns false {@see WatchdogAdapter}
	 * 
	 * @throws @param passiveWaitCount : multiple of pollIntervalSec to wait
	 * @return
	 */
	@Override
	public boolean passivePing(int passiveWaitCount, int pollIntervalSec) {
		...
		if (waitCount > passiveWaitCount)
			return false;
		return true;
	}

	/**
	 * this method called from {@link WatchdogAdapter#run()} it computes the elapsed
	 * time since latestPing. returns elapsed time as a multiple of pollIntervalSec
	 * {@see WatchdogAdapter}
	 * 
	 * @throws @param passiveWaitCount : multiple of pollIntervalSec to wait
	 * @return
