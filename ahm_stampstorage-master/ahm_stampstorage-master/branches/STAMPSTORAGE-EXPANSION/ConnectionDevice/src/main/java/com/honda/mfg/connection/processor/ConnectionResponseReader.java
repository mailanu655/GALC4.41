...
public class ConnectionResponseReader {
	...
	public String getResponse() {
		...
		while (true) {
			...
			try {
				// Sync so Close method is blocked while reading stream
				synchronized (in) {
					while (in.ready()) {
						checkForTimeout(startTimeMs, response.toString());
				...
				Thread.sleep(20); // Throttle reader to save CPU and allow other threads to respond and allow Syc
									// of connection.
	...
	private void checkForTimeout(long startTime, String partialResponse) {
		String timeoutMessage = "Timed out after " + MAX_RESPONSE_TIMEOUT + " ms. Response received so far:  \""
				+ partialResponse + "\"";
