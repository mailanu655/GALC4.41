...
/**
 * User: Jeffrey M Lutz Date: 4/1/11
 */
public class SocketFactory {
	...
	public Socket createSocket()
			throws UnknownHostException, IllegalBlockingModeException, IllegalArgumentException, SecurityException,
			SocketTimeoutException, IOException, IllegalArgumentException, IllegalBlockingModeException {
		Socket socket;
	...
	protected void connectSocket(Socket socket, InetSocketAddress address)
			throws IllegalArgumentException, IllegalBlockingModeException, SocketTimeoutException, IOException {
		socket.connect(address, connectTimeoutMilli);
	}

	protected void bindSocket(Socket socket) throws UnknownHostException, IOException, IllegalArgumentException {
		socket.bind(null);
	}

	private Socket getNewSocket() throws IllegalArgumentException, SecurityException {
		return constructNewSocket();
	}

	private InetSocketAddress getAddress() throws IllegalArgumentException, SecurityException {
	...
	public String getConfigMsg() {
		return "HOSTNAME:PORT " + hostName + ":" + portNumber + " -->  Connect Timeout: " + connectTimeoutMilli
				+ " (ms.)";
	}
