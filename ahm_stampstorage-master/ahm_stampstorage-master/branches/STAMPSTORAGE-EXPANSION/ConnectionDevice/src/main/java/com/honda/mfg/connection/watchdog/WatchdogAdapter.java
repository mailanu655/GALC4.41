	...
	private class Runner implements Runnable {
		public void run() {
			...
			if (!isPassivePing()) {
				...
				if (healthy) {
					LOG.info("Ping:  healthy    --> " + devicePing.getClass().getSimpleName() + " --> Response time: "
							+ delta + " (ms)");
					watchdog.healthyEvent();
				} else {
					LOG.info("Ping:  unhealthy  --> " + devicePing.getClass().getSimpleName() + " --> Response time: "
							+ delta + " (ms)");
					watchdog.unhealthyEvent();
				}
			} else { // passive, listen-only mode
				// boolean primaryHealthy = devicePing.passivePing(getPassiveWaitCount(),
				// getPollIntervalSecs());
				// if(!primaryHealthy) {
