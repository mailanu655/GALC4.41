...
/**
 * Created by IntelliJ IDEA. User: vcc30690 Date: 11/30/11 Time: 11:17 AM To
...
@Transactional
public class OrderFulfillmentManagerTest extends AbstractTestBase {
	...
	// @Test
	public void successfullyRunOrderFulfillmentManager() {
		loadTestData();
		loadTestParmData();
		...
		WeldOrder weldOrder2 = WeldOrder.findWeldOrdersByOrderStatus(OrderStatus.InProcess).get(1);
		OrderFulfillmentManager manager2 = new OrderFulfillmentManager(orderMgr2, storage, helper, releaseManager,
				"fulfillmentCarrierReleaseCount", "recirculationCarrierReleaseCount");
		...
		assertEquals(OrderStatus.InProcess, (WeldOrder.findWeldOrder(weldOrder2.getId())).getOrderStatus());
		assertEquals(OrderStatus.RetrievingCarriers, (WeldOrder.findWeldOrder(weldOrder.getId())).getOrderStatus());
		...
		assertEquals(OrderStatus.RetrievingCarriers, (WeldOrder.findWeldOrder(weldOrder.getId())).getOrderStatus());
		fulfillments = OrderFulfillment.findAllOrderFulfillmentsByOrder(weldOrder);
		carriersCycle1 = OrderFulfillment.findAllOrderFulfillmentsByOrderForCycle(weldOrder, 3);
		carriersCycle2 = OrderFulfillment.findAllOrderFulfillmentsByOrderForCycle(weldOrder, 4);
		...
		manager.run();
		assertEquals(OrderStatus.AutoCompleted, (WeldOrder.findWeldOrder(weldOrder.getId())).getOrderStatus());
	...
	@Test
	public void successfullyRunOrderFulfillmentManagerWhenStorageStateIsStale() {
		...
		NewFulfillmentHelper fulfillmentHelper = mock(NewFulfillmentHelper.class);
		when(fulfillmentHelper.getActiveOrder((OrderMgr) any())).thenReturn(new WeldOrder()).thenReturn(null)
				.thenThrow(new RuntimeException());
		OrderFulfillmentManager manager = new OrderFulfillmentManager((OrderMgr) any(), storage, fulfillmentHelper,
				releaseManager, "fulfillmentCarrierReleaseCount", "recirculationCarrierReleaseCount");
	...
	@Test
	public void successfullyReceiveStaleDataMessage() {
		AnnotationProcessor.process(this);
		WeldOrder weldOrder = new WeldOrder();
		weldOrder.setOrderStatus(OrderStatus.RetrievingCarriers);
		weldOrder.setDeliveryStatus(OrderStatus.Initialized);
		Storage storage = mock(Storage.class);

		ReleaseManager releaseManager = mock(ReleaseManager.class);
		NewFulfillmentHelper fulfillmentHelper = mock(NewFulfillmentHelper.class);
		when(fulfillmentHelper.getActiveOrder((OrderMgr) any())).thenReturn(weldOrder)
				.thenThrow(new RuntimeException());
		OrderFulfillmentManager manager = new OrderFulfillmentManager((OrderMgr) any(), storage, fulfillmentHelper,
				releaseManager, "fulfillmentCarrierReleaseCount", "recirculationCarrierReleaseCount");

		StaleDataMessage message = new StaleDataMessage(true);
		EventBus.publish(message);

		long delta = System.currentTimeMillis();
		while (System.currentTimeMillis() - delta < (1 * 1000L)) {
		}
		StaleDataMessage message2 = new StaleDataMessage(true);
		EventBus.publish(message2);

		long delta2 = System.currentTimeMillis();
		while (System.currentTimeMillis() - delta2 < (1 * 1000L)) {
		}
	...
	@Test
	public void successfullyReceiveReloadStorageStateMessage() {
		AnnotationProcessor.process(this);
		WeldOrder weldOrder = new WeldOrder();
		weldOrder.setOrderStatus(OrderStatus.RetrievingCarriers);
		weldOrder.setDeliveryStatus(OrderStatus.Initialized);
		...
		currentLocation.setName("stop");
		ReloadStorageStateMessage message = new ReloadStorageStateMessage(currentLocation);
		EventBus.publish(message);
	...
	public void successfullyReleaseCarriers() {
		...
		StorageState storageState = mock(StorageState.class);
		when(storage.getStorageState()).thenReturn(storageState);
		when(storageState.isStale()).thenReturn(false);
		ReleaseManager releaseManager = mock(ReleaseManager.class);
		NewFulfillmentHelper fulfillmentHelper = mock(NewFulfillmentHelper.class);
		when(fulfillmentHelper.getActiveOrder((OrderMgr) any())).thenReturn(weldOrder);
		when(fulfillmentHelper.getMinCycleCountForOrderForFulfillmentStatus(weldOrder,
				CarrierFulfillmentStatus.RETRIEVED)).thenReturn(0).thenReturn(0).thenReturn(1).thenReturn(1);
		when(fulfillmentHelper.getMinCycleCountForOrderForFulfillmentStatus(weldOrder,
				CarrierFulfillmentStatus.RELEASED)).thenReturn(1).thenReturn(0).thenReturn(1).thenReturn(0);
		OrderFulfillmentManager manager = new OrderFulfillmentManager((OrderMgr) any(), storage, fulfillmentHelper,
				releaseManager, "fulfillmentCarrierReleaseCount", "recirculationCarrierReleaseCount");
	...
	// @Test
	public void successfullyAutoCompleteOrder() {
		loadOrderFulfillmentTable2();
		loadTestParmData();
		Storage storage = mock(Storage.class);
		StorageState storageState = mock(StorageState.class);
		when(storage.getStorageState()).thenReturn(storageState);
		...
		NewFulfillmentHelper fulfillmentHelper = mock(NewFulfillmentHelper.class);
		WeldOrder weldOrder = WeldOrder.findWeldOrdersByOrderStatus(OrderStatus.InProcess).get(0);
		List<OrderFulfillment> orderFulfillments = OrderFulfillment.findAllOrderFulfillments();
		when(fulfillmentHelper.getActiveOrder((OrderMgr) any())).thenReturn(weldOrder);
		when(fulfillmentHelper.getAllOrderFulfillmentsByOrder(weldOrder)).thenReturn(orderFulfillments);
		OrderFulfillmentManager manager = new OrderFulfillmentManager((OrderMgr) any(), storage, fulfillmentHelper,
				releaseManager, "fulfillmentCarrierReleaseCount", "recirculationCarrierReleaseCount");
	...
	public void loadTestData() {
		...
		Die someDie1 = new Die();
		someDie1.setId(103l);
		someDie1.setDescription("left_die_103");
		someDie1.setPartProductionVolume(PartProductionVolume.MEDIUM_VOLUME);

		Die someDie2 = new Die();
		someDie2.setId(104l);
		someDie2.setDescription("right_die_104");
		someDie2.setPartProductionVolume(PartProductionVolume.MEDIUM_VOLUME);
		...
		model2.setLeftDie(someDie1);
		model2.setRightDie(someDie2);
...
//        fulfillment1.setCurrentLocation(exitStop24);
//        fulfillment1.setDestination(exitStop24);
//        fulfillment1.setCarrierFulfillmentStatus(CarrierFulfillmentStatus.QUEUED);
//        fulfillment1.setDie(leftDie);
//        fulfillment1.setQuantity(new Integer(15));
...
//        fulfillment2.setCurrentLocation(exitStop24);
//        fulfillment2.setDestination(exitStop24);
//        fulfillment2.setCarrierFulfillmentStatus(CarrierFulfillmentStatus.QUEUED);
//        fulfillment2.setDie(leftDie);
//        fulfillment2.setQuantity(new Integer(15));
//        fulfillment2.persist();
//
//        OrderFulfillment fulfillment3 = new OrderFulfillment();
//        OrderFulfillmentPk pk3 = new OrderFulfillmentPk(weldOrder, 104,2);
//        fulfillment3.setId(pk3);
//        fulfillment3.setCurrentLocation(stop25);
//        fulfillment3.setDestination(stop25);
//        fulfillment3.setCarrierFulfillmentStatus(CarrierFulfillmentStatus.RELEASED);
//        fulfillment3.setDie(rightDie);
//        fulfillment3.setQuantity(new Integer(15));
//        fulfillment3.persist();
//
//        OrderFulfillment fulfillment4 = new OrderFulfillment();
//        OrderFulfillmentPk pk4 = new OrderFulfillmentPk(weldOrder, 105,2);
//        fulfillment4.setId(pk4);
//        fulfillment4.setCurrentLocation(stop25);
//        fulfillment4.setDestination(stop25);
//        fulfillment4.setCarrierFulfillmentStatus(CarrierFulfillmentStatus.QUEUED);
//        fulfillment4.setDie(rightDie);
//        fulfillment4.setQuantity(new Integer(15));
//        fulfillment4.persist();
		...
		CarrierMes carrier2 = getCarrierMes(oneDayOld, new Integer(103), stop3, stop3, CarrierStatus.SHIPPABLE,
				new Integer(200), someDie1);
		CarrierMes carrier3 = getCarrierMes(oneDayOld, new Integer(103), stop4, stop4, CarrierStatus.SHIPPABLE,
				new Integer(202), someDie1);
		CarrierMes carrier1 = getCarrierMes(twoDaysOld, new Integer(90), stop7, stop7, CarrierStatus.SHIPPABLE,
				new Integer(696), leftDie);
		CarrierMes carrier11 = getCarrierMes(twoDaysOld, new Integer(99), stop8, stop8, CarrierStatus.SHIPPABLE,
				new Integer(130), rightDie);
		CarrierMes carrier12 = getCarrierMes(twoDaysOld, new Integer(90), stop9, stop9, CarrierStatus.SHIPPABLE,
				new Integer(720), leftDie);
		CarrierMes carrier4 = getCarrierMes(oneDayOld, new Integer(100), stop6, stop6, CarrierStatus.SHIPPABLE,
				new Integer(109), leftDie);
		CarrierMes carrier41 = getCarrierMes(oneDayOld, new Integer(100), stop6, stop6, CarrierStatus.SHIPPABLE,
				new Integer(596), leftDie);
		CarrierMes carrier44 = getCarrierMes(oneDayOld, new Integer(100), stop6, stop6, CarrierStatus.SHIPPABLE,
				new Integer(505), leftDie);
		CarrierMes carrier42 = getCarrierMes(twoDaysOld, new Integer(90), stop5, stop5, CarrierStatus.SHIPPABLE,
				new Integer(35), leftDie);
		CarrierMes carrier43 = getCarrierMes(twoDaysOld, new Integer(90), stop5, stop5, CarrierStatus.SHIPPABLE,
				new Integer(425), leftDie);
		CarrierMes carrier5 = getCarrierMes(oneDayOld, new Integer(101), stop2, stop2, CarrierStatus.SHIPPABLE,
				new Integer(131), rightDie);
		CarrierMes carrier51 = getCarrierMes(oneDayOld, new Integer(101), stop2, stop2, CarrierStatus.SHIPPABLE,
				new Integer(203), rightDie);
		CarrierMes carrier52 = getCarrierMes(twoDaysOld, new Integer(99), stop1, stop1, CarrierStatus.SHIPPABLE,
				new Integer(126), rightDie);
		CarrierMes carrier53 = getCarrierMes(twoDaysOld, new Integer(99), stop1, stop1, CarrierStatus.SHIPPABLE,
				new Integer(465), rightDie);
		CarrierMes carrier54 = getCarrierMes(twoDaysOld, new Integer(99), stop1, stop1, CarrierStatus.SHIPPABLE,
	...
	void loadOrderFulfillmentTable() {
		...
		OrderFulfillmentPk pk1 = new OrderFulfillmentPk(weldOrder, 101, 1);
		fulfillment1.setId(pk1);
		fulfillment1.setCurrentLocation(stop24);
		fulfillment1.setDestination(stop24);
		fulfillment1.setCarrierFulfillmentStatus(CarrierFulfillmentStatus.QUEUED);
		fulfillment1.setDie(leftDie);
	...
	void loadOrderFulfillmentTable2() {
		Stop stop3 = new Stop(700L);
		stop3.setName("stop3");
		stop3.setStopType(StopType.NO_ACTION);
		stop3.setStopArea(StopArea.UNDEFINED);
		...
		fulfillment2.setCurrentLocation(stop4);
		fulfillment2.setDestination(stop4);
		fulfillment2.setCarrierFulfillmentStatus(CarrierFulfillmentStatus.RETRIEVED);
		fulfillment2.setDie(leftDie);
		fulfillment2.setQuantity(new Integer(10));
		fulfillment2.persist();
