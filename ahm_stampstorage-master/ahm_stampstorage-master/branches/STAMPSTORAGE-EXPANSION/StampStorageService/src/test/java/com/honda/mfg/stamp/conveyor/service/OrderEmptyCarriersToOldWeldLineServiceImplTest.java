 ...
 */
public class OrderEmptyCarriersToOldWeldLineServiceImplTest {

	@Test
	public void runOrderEmptyCarrierService() {

		Stop stop = new Stop(1201l);
		...
		when(storage.retrieveEmptyCarrierForOldWeldLine()).thenReturn(lane)
				.thenThrow(new NoApplicableRuleFoundException("no applicable rule found"));
		...
		when(serviceRoleWrap.isPassive()).thenReturn(false).thenReturn(false).thenReturn(false).thenReturn(false)
				.thenReturn(true);
		when(helper.getCarrierCountForOldWeldLineEmptyArea()).thenReturn(5).thenReturn(10).thenReturn(7)
				.thenThrow(new RuntimeException("some exception"));

		ReleaseManager releaseManager = mock(ReleaseManager.class);

		when(helper.getOldWeldLineEmptyCarrierDeliveryStop()).thenReturn(new Stop());
		...
		when(helper.getParmValue("oldWeldLineEmptyUnderFilledQuantity")).thenReturn(10);
		when(helper.getParmValue("carrierCount")).thenReturn(6);
		OrderEmptyCarriersService emptyCarriersService = new OrderEmptyCarriersToOldWeldLineServiceImpl(helper, storage,
				releaseManager, "oldWeldLineEmptyUnderFilledQuantity", "carrierCount");
	...
	@Test
	public void runOrderEmptyCarrierServiceOLdWeldLIneNotUnderFilled() {
		...
		when(helper.getCarrierCountForOldWeldLineEmptyArea()).thenReturn(10).thenThrow(new RuntimeException())
				.thenReturn(7).thenReturn(7).thenReturn(10);
		when(helper.anyOrderInProcess()).thenReturn(false).thenReturn(false).thenReturn(true).thenReturn(false)
				.thenReturn(true);
		when(helper.anyCarrierSetToReleaseFromRows()).thenReturn(false).thenReturn(false).thenReturn(false)
	...
	public void runOrderEmptyCarrierServiceOLdWeldLIneWhenDeliveryStopNotSet() {
		...
		ReleaseManager releaseManager = mock(ReleaseManagerImpl.class);
		when(helper.getOldWeldLineEmptyCarrierDeliveryStop()).thenReturn(null);
		when(helper.getParmValue(Matchers.anyString())).thenReturn(6);
		OrderEmptyCarriersService emptyCarriersService = new OrderEmptyCarriersToOldWeldLineServiceImpl(helper, storage,
				releaseManager, "10", "6");
		emptyCarriersService.setServiceRoleWrapper(serviceRoleWrap);
