...
public class NewFulfillmentHelperImplTest {
	...
	public void successfullyGetAllOrderFulfillmentsByOrderAndCarrierFulfillmentStatus() {
		...
		NewFulfillmentHelper helper = new NewFulfillmentHelperImpl();
		List list = null;
		assertNotNull(list = helper.getAllOrderFulfillmentsByOrderAndCarrierFulfillmentStatus(order,
	...
	public void successfullyGetAllOrderFulfillmentsByCarrierFulfillmentStatusAndDie() {
		...
		NewFulfillmentHelper helper = new NewFulfillmentHelperImpl();
		List list = null;
		assertNotNull(list = helper.getAllOrderFulfillmentsByCarrierFulfillmentStatusAndDie(order,
				CarrierFulfillmentStatus.SELECTED, order.getLeftDie()));
...
//    }
	...
	public void successfullySaveWeldOrderComments() {
		...
		NewFulfillmentHelper helper = new NewFulfillmentHelperImpl();
		helper.saveWeldOrderComments(order, comments, DieType.BOTH, CommentCategory.FULFILLMENT);
		helper.saveWeldOrderComments(order1, comments, DieType.BOTH, CommentCategory.DELIVERY);
		WeldOrder tempOrder = WeldOrder.findWeldOrder(order.getId());
		assertEquals(comments, tempOrder.getLeftFulfillmentComments());
		assertEquals(comments, tempOrder.getRightFulfillmentComments());
		tempOrder = WeldOrder.findWeldOrder(order1.getId());
		assertEquals(comments, tempOrder.getLeftDeliveryComments());
		assertEquals(comments, tempOrder.getRightDeliveryComments());
	}
	...
	public void successfullySaveWeldOrderStatusAndComments() {
		...
		String comments = " test save comments";
		NewFulfillmentHelper helper = new NewFulfillmentHelperImpl();
		order1.setOrderStatus(OrderStatus.InProcess);
		order1.persist();
		helper.saveWeldOrderStatusAndComments(order, orderStatus, comments, DieType.BOTH);
		helper.saveWeldOrderStatusAndComments(order1, orderStatus, comments, DieType.RIGHT);
		WeldOrder tempOrder = WeldOrder.findWeldOrder(order.getId());
		assertEquals(comments, tempOrder.getLeftFulfillmentComments());
		assertEquals(comments, tempOrder.getRightFulfillmentComments());
		tempOrder = WeldOrder.findWeldOrder(order1.getId());
		assertEquals(comments, tempOrder.getRightFulfillmentComments());
		assertFalse(comments.equals(tempOrder.getLeftFulfillmentComments()));
	}
	...
	public void successfullySaveWeldDeliveryStatusAndComments() {
		...
		String comments = " test save comments";
		NewFulfillmentHelper helper = new NewFulfillmentHelperImpl();
		helper.saveWeldDeliveryStatusAndComments(order, orderStatus, comments, DieType.BOTH);
		helper.saveWeldDeliveryStatusAndComments(order1, orderStatus, comments, DieType.LEFT);
		WeldOrder tempOrder = WeldOrder.findWeldOrder(order.getId());
		assertEquals(comments, tempOrder.getLeftDeliveryComments());
		assertEquals(comments, tempOrder.getRightDeliveryComments());
		tempOrder = WeldOrder.findWeldOrder(order1.getId());
		assertEquals(comments, tempOrder.getLeftDeliveryComments());
		assertFalse(comments.equals(tempOrder.getRightDeliveryComments()));
	...
	public void successfullyGetCarrierDestinationForRecirculation() {
		...
		Stop destination1 = helper.getCarrierDestinationForRecirculation(
				StorageRow.findStorageRowsByStop(Stop.findStop(1201L)), Die.findDie(1L));
		Stop emptyDestination1 = helper.getCarrierDestinationForRecirculation(
				StorageRow.findStorageRowsByStop(Stop.findStop(1201L)), Die.findDie(999L));
		assertNotSame(destination1, emptyDestination1);

		Stop destination2 = helper.getCarrierDestinationForRecirculation(
				StorageRow.findStorageRowsByStop(Stop.findStop(1236L)), Die.findDie(1L));
		Stop emptyDestination2 = helper.getCarrierDestinationForRecirculation(
				StorageRow.findStorageRowsByStop(Stop.findStop(1236L)), Die.findDie(999L));

		Stop destination3 = helper.getCarrierDestinationForRecirculation(
				StorageRow.findStorageRowsByStop(Stop.findStop(1221L)), Die.findDie(1L));

		assertEquals(destination2, emptyDestination2);
		assertEquals(destination1, Stop.findStop(905L));
		assertEquals(emptyDestination1, Stop.findStop(5200L));
		assertEquals(destination2, Stop.findStop(3018L));
		assertEquals(destination3, Stop.findStop(704L));
	}
	...
	public void loadTestData() {
		...
		Stop stop95 = new Stop(905L);
		stop95.setName("ST9-5");
		stop95.setStopType(StopType.RECIRC_TO_ALL_ROWS);
		stop95.setStopArea(StopArea.STORE_OUT_ROUTE);

		Stop stop21 = new Stop(1221L);
		stop21.setName("Row-21");
		stop21.setStopType(StopType.NO_ACTION);
		stop21.setStopArea(StopArea.ROW);

		StorageRow row21 = new StorageRow();
		row21.setRowName("ROW21");
		row21.setStop(stop21);
		row21.setCapacity(21);
		row21.setAvailability(StopAvailability.AVAILABLE);
		row21.setStorageArea(StorageArea.A_AREA);
