...
public class ReleaseEmptyCarriersServiceImplTest {
	...
	public void runReleaseEmptyCarrierService() {
		...
		when(helper.getCarrierCountForEmptyArea()).thenReturn(5).thenReturn(8).thenReturn(10).thenReturn(10)
				.thenReturn(11).thenThrow(new RuntimeException());
		when(helper.getCarriersToMoveToEmptyStorageArea(anyInt())).thenReturn(getCarrierMesList(3))
				.thenReturn(getCarrierMesList(2)).thenReturn(getCarrierMesList(0)).thenReturn(getCarrierMesList(3))
		...
		ReleaseEmptyCarriersService releaseEmptyCarriersService = new ReleaseEmptyCarriersServiceImpl(helper,
				releaseManager, "emptyAreaUnderFilledQuantity", "releaseCarrierCount");
