...
/**
 * Created by IntelliJ IDEA. User: vcc30690 Date: 11/21/11 Time: 2:51 PM To
...
public class NewFulfillmentHelperImpl extends AbstractHelperImpl implements NewFulfillmentHelper {
	...
	public List<OrderFulfillment> getAllOrderFulfillmentsByOrderAndCarrierFulfillmentStatus(WeldOrder order,
	...
	public List<OrderFulfillment> getAllOrderFulfillmentsByCarrierFulfillmentStatusAndDie(WeldOrder order,
			CarrierFulfillmentStatus carrierFulfillmentStatus, Die die) {
		List<OrderFulfillment> fulfillments = getAllOrderFulfillmentsByOrder(order);
	...
	public synchronized void updateOrderFulfillmentStatus(OrderFulfillment orderFulfillment,
			CarrierFulfillmentStatus carrierFulfillmentStatus, Stop destination) {
	...
	/**
	 * selects carriers in a given cycle that don't already have their destination
	 * set to a queue stop
	 *
	 * @param order
	...
	public List<Integer> getNewCarriersToRelease(WeldOrder order, Integer cycle) {
		...
		if (fulfillments != null) {
			for (OrderFulfillment fulfillment : fulfillments) {
				...
				if (!fulfillment.getDestination().equals(queueStop)
						&& fulfillment.isStatus(CarrierFulfillmentStatus.SELECTED)) {
	 ...
	 * @see com.honda.mfg.stamp.conveyor.newfulfillment.NewFulfillmentHelper#saveCarriers(com.honda.mfg.stamp.conveyor.domain.WeldOrder,
	 *      java.util.List)
	 */
	...
	public void saveFulfillment(WeldOrder order, Carrier carrier, Integer cycle,
			CarrierFulfillmentStatus fulfillmentStatus) {
	...
	@Override
	public synchronized void saveWeldDeliveryStatusAndComments(WeldOrder order, OrderStatus orderStatus,
			String comments, DieType dieType) {
		WeldOrder activeOrder = WeldOrder.findWeldOrder(order.getId());
		if (!activeOrder.getDeliveryStatus().equals(OrderStatus.ManuallyCompleted)) {
			activeOrder.setDeliveryStatus(orderStatus);
			activeOrder.setComments(comments);
			saveExtendedComments(activeOrder, comments, dieType, CommentCategory.DELIVERY);
			activeOrder.merge();
		} else {
			LOG.debug("order -" + order.getId() + " manually completed");
		}
	...
	public boolean canClearBackOrder(Stop queueStop, Die die, String deliveryCycleSize) {
		...
		// getAllOrderFulfillmentsByCarrierFulfillmentStatusAndDie(order,CarrierFulfillmentStatus.QUEUED,
		// die);
		int deliverySize = getParmValue(deliveryCycleSize);
		if (count >= (deliverySize * 2)) {
			LOG.info("queue stop:" + queueStop.getName() + "carrier count: " + count
					+ " enough ( 2* deliveryCycleSize) to clear back Order ");
			return true;
		}

		LOG.info("queue stop:" + queueStop.getName() + "carrier count: " + count
				+ " Not Enough ( 2* deliveryCycleSize) to clear back Order ");
		return false;
	}
	...
	/**
	 * determine if there are any orders currently Selecting or Retrieving for the
	 * given order manager (weld-line)
	 *
	 * @param weldLine
	 * @return
	 * @see RetrieveCycleManager#process(WeldOrder, NewFulfillmentHelper,
	 *      com.honda.mfg.stamp.conveyor.release.ReleaseManager)
	...
	/**
	 * get the current available capacity at left queue stop of order manager (weld
	 * line)
	 *
	 * @return long
	 */
	...
	public List<WeldOrder> getActiveFulfillmentOrders() {
		List<WeldOrder> activeOrders = new ArrayList<WeldOrder>();
		List<OrderMgr> orderMgrs = OrderMgr.findAllOrderMgrs();
		for (OrderMgr orderMgr : orderMgrs) {
			WeldOrder tempOrder = getActiveOrder(orderMgr);
			if (tempOrder != null) {
	...
	private synchronized void saveExtendedComments(WeldOrder order, String comments, DieType dieType,
			CommentCategory category) {
		if (dieType != null && category != null) {
			if (category.equals(CommentCategory.DELIVERY)) {
				if (dieType.equals(DieType.BOTH) || dieType.equals(DieType.LEFT)) {
					order.setLeftDeliveryComments(comments);
				}
				if (dieType.equals(DieType.BOTH) || dieType.equals(DieType.RIGHT)) {
					order.setRightDeliveryComments(comments);
				}
			} else {
				if (dieType.equals(DieType.BOTH) || dieType.equals(DieType.LEFT)) {
					order.setLeftFulfillmentComments(comments);
				}
				if (dieType.equals(DieType.BOTH) || dieType.equals(DieType.RIGHT)) {
					order.setRightFulfillmentComments(comments);
				}
			}
		}
	}
	...
	@Override
	public synchronized void saveWeldOrderComments(WeldOrder order, String comments, DieType dieType,
			CommentCategory category) {
		WeldOrder activeOrder = WeldOrder.findWeldOrder(order.getId());
		activeOrder.setComments(comments);
		saveExtendedComments(activeOrder, comments, dieType, category);
		activeOrder.merge();
	}
	...
	public synchronized void saveWeldOrderStatusAndComments(WeldOrder order, OrderStatus orderStatus, String comments,
		...
		if (!activeOrder.getOrderStatus().equals(OrderStatus.ManuallyCompleted)) {
			activeOrder.setOrderStatus(orderStatus);
			activeOrder.setComments(comments);
			saveExtendedComments(activeOrder, comments, dieType, CommentCategory.FULFILLMENT);
			activeOrder.merge();
		} else {
			LOG.debug("order -" + order.getId() + " manually completed");
		}
	...
	public void deleteAllUnReleasedOrderFulfillments(WeldOrder order) {
		...
		List<OrderFulfillment> fulfillments = OrderFulfillment
				.findAllOrderFulfillmentsByOrderWithCarrierFulfillmentStatus(order, CarrierFulfillmentStatus.RETRIEVED);
	...
	public Stop getCarrierDestinationForRecirculation(StorageRow lane, Die die) {
		...
		if (isEmpty(die) && !lane.getStorageArea().equals(StorageArea.B_AREA)
				&& !lane.getStorageArea().equals(StorageArea.S_AREA)) {
		...
		if (!isEmpty(die) && lane.getStorageArea().equals(StorageArea.C_HIGH)
				|| lane.getStorageArea().equals(StorageArea.C_LOW)) {
			area = StopArea.STORE_OUT_ROUTE;
		}
	...
	@Override
	public void deleteAllOrderFulfillmentsByOrderWithCarrierFulfillmentStatus(WeldOrder order,
			CarrierFulfillmentStatus carrierFulfillmentStatus) {
		...
		if (fulfillments != null && fulfillments.size() > 0) {
			for (OrderFulfillment fulfillment : fulfillments) {
				if (fulfillment.getCarrierFulfillmentStatus().equals(carrierFulfillmentStatus)
						&& isCarrierInRow(fulfillment.getId().getCarrierNumber())) {
					fulfillment.remove();
				}
			}
		}
	...
	public Die getDieForLastCycle(WeldOrder order) {
		...
		List<OrderFulfillment> orderFulfillments = OrderFulfillment.findAllOrderFulfillmentsByOrderForCycle(order,
				cycle);
	...
	/*
	 * gets the total number of fulfillments for given order and die, including all
	 * cycles for given die
	 * 
	 * @see com.honda.mfg.stamp.conveyor.newfulfillment.NewFulfillmentHelper#
	 * getCarrierCountForDie(com.honda.mfg.stamp.conveyor.domain.WeldOrder,
	 * com.honda.mfg.stamp.conveyor.domain.Die)
	 */
	@Override
	...
	/**
	 * counts part quantities in all carrier fulfillments for this order, in any
	 * status and compares it with ordered quantity
	 *
	 * @param weldOrder
	 * @param die
	 * @return true if requested quantity is more than fulfilled quantity
	 * @see com.honda.mfg.stamp.conveyor.newfulfillment.NewFulfillmentHelper#needMoreCarriersWithDie(com.honda.mfg.stamp.conveyor.domain.WeldOrder,
	 *      com.honda.mfg.stamp.conveyor.domain.Die)
	 */
	@Override
	public boolean needMoreCarriersWithDie(WeldOrder weldOrder, Die die) {
		...
		// Also count carriers with same die, left-over from prior order(s) that are
		// still in the queue
	...
	public boolean isCarrierPartOfActiveOrder(WeldOrder order, CarrierMes carrierMes) {
		List<OrderFulfillment> fulfillments = getAllOrderFulfillmentsByOrder(order);
