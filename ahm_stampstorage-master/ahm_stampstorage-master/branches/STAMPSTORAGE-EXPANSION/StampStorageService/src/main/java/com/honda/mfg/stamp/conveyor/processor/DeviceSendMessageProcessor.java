...
public class DeviceSendMessageProcessor {
	...
	@EventSubscriber(eventClass = ResetAlarmMessage.class, referenceStrength = ReferenceStrength.STRONG)
	public void resetAlarmMessageListener(ResetAlarmMessage infoMessage) {
		message = infoMessage;
		MesMessageBuilder mesMessageBuilder = new MesMessageBuilder();
		GeneralMessage mesMsg = mesMessageBuilder.buildMesMessage(infoMessage, ResetAlarmMessage.class);
		if (device != null) {
			device.sendMessage(mesMsg);
		}

		LOG.info("Sent Reset Alarm Message   -->   " + mesMsg.getMessage());
	}
