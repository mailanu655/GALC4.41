...
/**
 * Created by IntelliJ IDEA. User: vcc30690 Date: 11/17/11 Time: 2:07 PM To
...
public class RetrieveCycleManager {
	...
	public void process(WeldOrder order, NewFulfillmentHelper helper, ReleaseManager releaseManager) {
		...
		try {
			...
			if (bNeedLefts && currentLeftCapacity >= getFulfillmentCycleSize(helper)) {
				...
				helper.saveWeldOrderComments(order, "No Space available in Left queue", DieType.LEFT,
			...
			if (bNeedRights && currentRightCapacity >= getFulfillmentCycleSize(helper)) {
				...
				helper.saveWeldOrderComments(order, "No Space available in Right queue", DieType.RIGHT,
						CommentCategory.FULFILLMENT);
			}
	 ...
	 */
	private List<Carrier> getCarriersForNewCycle(WeldOrder order, Die die, int orderedPartQty, int queuedPartQty,
			int carrierCount, ReleaseManager releaseManager, NewFulfillmentHelper helper) {
		ArrayList<Carrier> carriers = new ArrayList<Carrier>();

		StringBuilder sb = new StringBuilder();
		sb.append("carrier count for die ").append(die).append(" is-").append(", carrier count-").append(carrierCount)
				.append(", orderedPartQty-").append(orderedPartQty).append(", queuedPartQty-").append(queuedPartQty);
		...
		try {
			while (orderedPartQty > totalPartQty && thisCount < carrierCount) {
				...
				if (lane != null) {
					...
					if (canRelease) {
						if (lane.getCarrierAtRowOut().getDie().equals(die)
								&& !lane.getCarrierAtRowOut().getCarrierStatus().equals(CarrierStatus.ON_HOLD)) {

							carrier = storage.getStorageState().releaseCarrierFromLane(lane);
							if (carrier.getCarrierStatus().equals(CarrierStatus.ON_HOLD)
									|| !carrier.getDie().equals(die)) {
								LOG.debug("carrier-" + carrier.getCarrierNumber() + " with Die - "
										+ carrier.getDie().toString() + " carrier status - "
										+ carrier.getCarrierStatus().toString());
								break;
							}
							...
							LOG.info(" Carrier At Lane Out for Row :" + lane.getRowName()
									+ " does not have the required Die: " + die);
						}
					}
	...
	public boolean recirculateCarriers(StorageRow lane, Die die, ReleaseManager releaseManager,
			NewFulfillmentHelper helper) {
		...
		while (count < lane.getCapacity()) {
			if (!lane.isEmpty()) {
				...
				LOG.info("moving blocking carrier with carrier number " + blockingCarrier.getCarrierNumber()
						+ " from lane " + lane.getRowName() + " to " + destination);
				WeldOrder order = OrderFulfillment.findOrderByCarrier(blockingCarrier);

				if (order == null || (order != null && ((order.getOrderStatus().equals(OrderStatus.ManuallyCompleted)
						|| order.getOrderStatus().equals(OrderStatus.AutoCompleted))))) {
					releaseManager.releaseCarrier(blockingCarrier.getCarrierNumber(), blockingCarrier.getDestination(),
							StorageConfig.OHCV_APP_RECIRC, true);
	...
	void selectAndSaveNewCarriers(WeldOrder weldOrder, Die die, NewFulfillmentHelper helper,
			ReleaseManager releaseManager, Integer carrierCount) {

		LOG.info("Selecting Carriers for Die " + die);
		DieType dieType = (weldOrder.getLeftDie().equals(die)) ? DieType.LEFT : DieType.RIGHT;
		helper.saveWeldOrderComments(weldOrder, "Selecting Carriers for Die " + die, dieType,
				CommentCategory.FULFILLMENT);
...
//        }
		...
		List<Carrier> carriers = getCarriersForNewCycle(weldOrder, die, orderedQuantity, queuedQuantity, carrierCount,
