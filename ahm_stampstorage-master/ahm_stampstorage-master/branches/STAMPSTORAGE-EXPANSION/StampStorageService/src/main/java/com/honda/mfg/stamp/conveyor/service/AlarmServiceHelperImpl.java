...
/**
 * Created by IntelliJ IDEA. User: Ambica Gawarla Date: 4/2/12 Time: 1:20 PM To
...
public class AlarmServiceHelperImpl extends AbstractHelperImpl implements AlarmServiceHelper {
	...
	private AlertingServices alertingService;

	public AlarmServiceHelperImpl(AlertingServices alertingService) {
	...
	public List<AlarmEvent> getExpiredAlarms() {
		...
		for (AlarmEvent event : alarmEvents) {
			// LOG.info("Alarm Number-" + event.getAlarmNumber() + " Location-" +
			// event.getLocation());
	...
	public void alertPlc(AlarmEvent alarmEvent) {
		...
		PlcAlarmMessage plcAlarmMessage = new PlcAlarmMessage(alarm.getAlarmNumber().toString(),
				alarm.getLocation().toString(), alarm.getDescription());
	...
	public void alertContacts(AlarmEvent alarmEvent) {
		...
		if (alarmContacts.size() > 0) {
			for (AlarmContact alarmContact : alarmContacts) {
				...
				if (alarmContact.getContactType().equals(ContactType.EMAIL)
						|| alarmContact.getContactType().equals(ContactType.BOTH)) {
					LOG.info(" sending email for alarm-" + alarmEvent.getAlarmNumber() + " to-"
							+ contact.getContactName());
					alertingService.sendEmail(contact.getEmail(), alarmDefinition.getDescription());
				}

				if (alarmContact.getContactType().equals(ContactType.PAGER)
						|| alarmContact.getContactType().equals(ContactType.BOTH)) {
					LOG.info(" sending page for alarm-" + alarmEvent.getAlarmNumber() + " to-"
							+ contact.getContactName());
