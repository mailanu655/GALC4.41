...
public final class ServiceRoleWrapperImpl implements ServiceRoleWrapper, SocketConnectionConstantsInterface {
	...
	private void processServiceRoles() {
		...
		for (InetAddress thisInet : ipAll) {
			...
			if (thisIp != null && !"".equals(thisIp.trim())) {
				List<ServiceRole> thisRoleList = ServiceRole.findServiceRolesByIpEqualsAndPort(thisIp.trim(), port)
						.getResultList();
				List<ServiceRole> activeRoleList = ServiceRole.findServiceRolesByCurrentActiveNot(false)
						.getResultList();
				if (thisRoleList.size() > 0) {
					thisRole = (ServiceRole) thisRoleList.get(0);
				}
				if (thisRole == null) {
					continue;
				}
				...
				if (activeRoleList.size() > 0) {
					activeRole = (ServiceRole) activeRoleList.get(0);
				}
				...
				// 2013-11-12:VB The following is a work-around for the read errors in
				// ConnectionResponseProcessor
				// In this case stop the current service and force start the next in fail-over
				// order
					...
					if (!isPassive() && nextRole != null) {
						...
						StringBuilder auditMessage = new StringBuilder(
								"Service stop: " + thisIp + "#" + thisRole.getServiceName());
	...
	/**
	 * set current instance as active in database, set all other roles that are
	 * currently active to passive. ordinarily, there should be only 1 currently
	 * active - but if there are multiple, set them all inactive
	 * 
	...
	private void logAuditError(String message) {
		...
		if (getRoleChangeReason() != null) {
			roleChange = getRoleChangeReason().getMessage();
		}
	...
	public void init() {
		try {
			...
			port = Integer.valueOf(socketProperties.getProperty(SERVER_PORT_PROPERTY_KEY, String.valueOf(DEFAULT_PORT)))
					.intValue();
	...
	private boolean shouldIStart(ServiceRole thisRole, ServiceRole activeRole) {
		...
		// do nothing, already running as active
			...
			shouldI = false;
		} else if (!thisRole.getCurrentActive() && thisRole.getDesignatedPrimary() && activeRole == null) {
			shouldI = true;
	...
	protected boolean nextInLine(List<ServiceRole> allRoles, ServiceRole thisRole, ServiceRole activeRole) {
		...
		if (allRoles == null || allRoles.size() == 0) {
			isNext = false;
		} else if (thisRole == null || activeRole == null) {
			isNext = false;
		}
		// if failover order is set to 0, don't participate in failover
		else if (thisRole.getFailoverOrder() <= 0) {
			isNext = false;
		} else if (thisRole.getId() == activeRole.getId()) {
			isNext = true;
		}
		// this service is the same as the one that is currently marked active
			...
			if (thisPos - thatPos % listSize == 1) { // operator precedence is: %, -, ==
				...
				// if wait count value was previously incremented, it is time for this instance
				// to startup
				// else, it is the first missed ping, increase the wait count
				if (serviceWatchdogAdapter.isAdaptiveWaitCount()) {
					isNext = true;
				} else {
	...
	public void resetConnectionState(ConnectionEventMessage msg) {
		...
		// else don't do anything --> missedPing=true will be set when MissedPingPassive
		// is received.
	}
	...
	/**
	 * boolean trigger indicates if ConnectionResponseReader has encountered too
	 * many exceptions set by
	 */
	...
	/**
	 * to make long-running threads go into inactive state; this does not apply to
	 * threads that are scheduled at fixed rate - they will simply check if state is
	 * active or passive.
	...
	protected void setServiceWatchdogAdapter(WatchdogAdapterInterface serviceWatchdogAdapter) {
	...
	public void addObserver(ServiceRoleObserver newObserver) {
		if (newObserver != null) {
			serviceRoleObservers.add(newObserver);
		}
	}
