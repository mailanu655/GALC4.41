...
/**
 * Created by IntelliJ IDEA. User: Ambica Gawarla Date: 6/27/13 Time: 3:10 PM To
...
public class BackOrderProcessor {
	...
	/**
	 * Clears backorder if enough carriers in the queue or sends update message if
	...
	public void carrierStatusMessageListener(CarrierStatusMessage statusMessage) {
		...
		try {
			...
			if (currentStop != null && destinationStop != null) {
				if (currentStop.getStopType().equals(StopType.STORE_IN_ALL_LANES)
						&& destinationStop.getStopType().equals(StopType.STORE_IN_ALL_LANES)) {
					...
					if (storage.getStorageState().isCarrierPartsOnBackOrder(carrier.getDie())) {
						...
						if (carrier.getCarrierStatus().equals(CarrierStatus.ON_HOLD)) {
							...
							for (WeldOrder order : orders) {
								...
								if (statusMessage.getDie().equals(order.getLeftDie())) {
									if (helper.isSpaceAvailableInQueue(order.getLeftQueueStop())) {
										if (!helper.needMoreCarriersWithDie(order, order.getLeftDie())
												|| helper.canClearBackOrder(order.getLeftQueueStop(),
														order.getLeftDie(), deliveryCycleSize)) {
											LOG.info(" Have Enough Carriers in Queue clearing back order for -"
													+ order.getLeftDie() + " - by OrderMgr" + order.getOrderMgr());
											storage.getStorageState().removeFromBackOrderList(order.getLeftDie(),
													order.getOrderMgr());

										} else {
											carrier.setDestination(order.getLeftQueueStop());
											helper.saveFulfillment(order, carrier, cycle,
									...
									if (helper.isSpaceAvailableInQueue(order.getRightQueueStop())) {
										...
										if (helper.canClearBackOrder(order.getRightQueueStop(), order.getRightDie(),
												deliveryCycleSize)
												|| !helper.needMoreCarriersWithDie(order, order.getRightDie())) {
											LOG.info(" Have Enough Carriers in Queue clearing back order for -"
													+ order.getRightDie() + " - by OrderMgr" + order.getOrderMgr());
											storage.getStorageState().removeFromBackOrderList(order.getRightDie(),
													order.getOrderMgr());
										} else {
							...
							if (!storage.getStorageState().isCarrierPartsOnBackOrder(carrier.getDie())) {
								LOG.info(" No BackOrder Request Exists for Die - " + carrier.getDie()
										+ " so storing carrier -" + carrier.getCarrierNumber());
								...
								LOG.info(" BackOrder Request Exists for Die - " + carrier.getDie()
										+ " so Not storing carrier -" + carrier.getCarrierNumber());
							}
						}
					} else {
