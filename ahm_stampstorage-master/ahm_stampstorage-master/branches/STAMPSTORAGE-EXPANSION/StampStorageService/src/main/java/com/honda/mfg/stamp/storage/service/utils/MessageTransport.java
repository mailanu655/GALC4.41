...
/**
 * Utility class to send and receive messages over socket streams. This class
 * uses <END_OF_MESSAGE> sequence including <> to separate messages
 * 
 * @author VCC44349
 *
 */
public class MessageTransport implements Runnable {
	...
	/**
	 * reads and processes messages PingMessage results in an immediate PingMessage
	 * response Other message types are processed by {CommandProcessor}
	 * 
	 * @throws IOException
	 */
		...
		while (!done) {
			...
			try {
				...
				if (indexSeparator >= 0) {
					...
					rightPartial = sBuf.substring(indexSeparator + messageSeparator.length(), sBuf.length());
					try {
						if (leftMessage != null && leftMessage.contains("PING")) {
							objectSource.setPingReceivedFlag();
							PingMessage p = new PingMessage(Calendar.getInstance().getTime().toString(), true);
							MessageRequest request = new ConnectionRequest(p, messageSeparator);
							writeObject(request);
							LOG.info("MessageTransport.process(): PING message received, sent: " + p);
						}
						// The CarrierUpdateRequestMessage type is hard-coded
						// for now, will be made generic later
						else if (leftMessage != null) {
							Message thisMessage = (Message) jSonParser.parse(leftMessage,
									JsonServiceWrapperMessage.class); // CarrierUpdateRequestMessage.class);
							if (messageSink != null) {
								messageSink.put(objectSource, thisMessage);
								LOG.info("MessageTransport.process(): message received) " + thisMessage);
							}
						}
					} catch (Exception ex) {
	...
	/**
	 * reads data into a character buffer, size specified by
	 * {@link MessageTransport#READ_BUF_SZ}
	 * 
	 * @return {@link char[]}
	 ...
	 * @throws Exception
	 */
	public char[] readObjectAsCharArray() throws Exception {
		char[] ret = null;
		try {
