...
/**
 * This class processes messages and calls appropriate StorageStateUpdateService
 * operations depending on message type. Called by
 * {@link com.honda.mfg.stamp.storage.service.utils.MessageTransport#process()}
...
public final class CommandProcessor implements MessageSinkInterface, CommandProcessorInterface {
	 ...
	 * @param carrierManager : ref to {@link StorageStateUpdateService} to perform
	 *                       operation
	 */
	public static void processCommand(ServiceRequestMessage msg, Object objSrc,
			StorageStateUpdateService carrierManager) {
		// int ret = 1;
		...
		if (cOp == CarrierUpdateOperations.STORE_REWORK) {
			...
			LaneUpdateRequestMessage req = (LaneUpdateRequestMessage) msg;
			if (req.getCarrierNumber() != null) {
				carrierManager.addCarrierToRow(req.getCarrierNumber(), req.getPosition(), req.getLaneStop());
			}
			...
			if (carrierNumberList != null && !carrierNumberList.isEmpty()) {
				...
				for (Integer carrierNum : carrierNumberList) {
					...
					// current location was being set in the LanesController, presumably to fix
					// carriers that are
			...
			// StorageStateRefreshRequestMessage req =
			// (StorageStateRefreshRequestMessage)msg;
