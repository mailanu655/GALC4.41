 ...
 */
public class OrderEmptyCarriersToOldWeldLineServiceImpl extends OrderEmptyCarrierAbstractServiceImpl
		implements Runnable {
	...
	public OrderEmptyCarriersToOldWeldLineServiceImpl(EmptyCarriersServiceHelper orderEmptyCarriersServiceHelper,
			Storage storage, ReleaseManager releaseManager, String oldWeldLineEmptyUnderFilledQuantity,
			String carrierCount) {

		super(orderEmptyCarriersServiceHelper, storage, releaseManager,
				orderEmptyCarriersServiceHelper.getOldWeldLineEmptyCarrierDeliveryStop(),
				oldWeldLineEmptyUnderFilledQuantity, carrierCount);
	...
	public void run() {
		...
		try {
			int carrierAtOldWeldLineDestCount = orderEmptyCarriersServiceHelper
					.getCarrierCountForOldWeldLineEmptyArea();
			LOG.info("Carriers at or in route to old weld line empty carrier destination: "
					+ carrierAtOldWeldLineDestCount);
			...
			if (deliveryStop == null) {
				...
				if (isAreaUnderFilled(carrierAtOldWeldLineDestCount)
						&& !orderEmptyCarriersServiceHelper.anyOrderInProcess()
						&& !orderEmptyCarriersServiceHelper.anyCarrierSetToReleaseFromRows()) {
