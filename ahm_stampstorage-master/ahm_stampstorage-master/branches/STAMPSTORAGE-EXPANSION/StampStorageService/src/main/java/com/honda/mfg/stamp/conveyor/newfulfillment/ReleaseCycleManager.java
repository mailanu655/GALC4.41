...
/**
 * Created by IntelliJ IDEA. User: vcc30690 Date: 11/16/11 Time: 3:11 PM To
...
public class ReleaseCycleManager {
	...
	public void process(WeldOrder order, Integer cycle, NewFulfillmentHelper helper, ReleaseManager releaseManager) {
		...
		if (carrierNumbers != null) {
			...
			// helper.saveWeldOrderComments(order, "Waiting for carriers to
			// Release",DieType.BOTH,CommentCategory.DELIVERY);
		}
	}

	private void releaseCarrier(Integer carrierNumber, NewFulfillmentHelper helper, WeldOrder order,
			ReleaseManager releaseManager, Integer cycle) {

		try {
			if (helper.isCarrierInA(carrierNumber)
					&& !helper.noOrderWithDeliveryStatus(OrderStatus.DeliveringCarriers)) {
				LOG.info("Waiting for other order to finish Delivering");
				// helper.saveWeldOrderComments(order, "Waiting for other order to finish
	 ...
	 * @param cycle
	 */
	private void updateOrderFulfillmentAndReleaseCarrier(WeldOrder order, Integer carrierNumber, Integer cycle,
