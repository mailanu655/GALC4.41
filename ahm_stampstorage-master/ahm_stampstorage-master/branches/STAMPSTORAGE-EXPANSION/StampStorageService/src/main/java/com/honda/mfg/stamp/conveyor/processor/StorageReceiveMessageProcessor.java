	...
	public void carrierStatusMessageListener(CarrierStatusMessage statusMessage) {
		...
		try {
			...
			if (currentStop != null && destinationStop != null) {
				if ((isStoreInStop(currentStop) && isStoreInStop(destinationStop))
						|| (isRecircStop(currentStop) && isRecircStop(destinationStop))) {
					...
					if (!storage.getStorageState().isCarrierPartsOnBackOrder(carrier.getDie())) {
						...
						LOG.info(" carrier " + statusMessage.getCarrierNumber() + " has back order parts"
...
//                    }
					...
					if (!currentStop.equals(destinationStop)) {
						LOG.info("Carrier " + statusMessage.getCarrierNumber()
								+ " moved in to wrong lane: currentLocation = " + currentStop.getId() + " destination="
						...
						LOG.info("Carrier " + statusMessage.getCarrierNumber() + " moved in to lane: currentLocation = "
								+ currentStop.getId() + " destination=" + destinationStop.getId());
						storage.getStorageState().reorderCarriersInRow(destinationStop.getId());
					}

				} else if ((currentStop.getStopType().equals(StopType.RELEASE_CHECK)
						|| currentStop.getStopType().equals(StopType.EMPTY_CARRIER_DELIVERY))) {
					LOG.info("Carrier " + statusMessage.getCarrierNumber() + " moved out of lane: currentLocation = "
							+ currentStop.getId() + " destination=" + destinationStop.getId());
				...
				} else if (destinationStop.getStopType().equals(StopType.FULL_CARRIER_DELIVERY)
						&& currentStop.getStopType().equals(StopType.FULL_CARRIER_DELIVERY)) {
					if (currentStop.getStopArea().equals(StopArea.WELD_LINE_1)) {
						LOG.info("Moving Carrier " + carrier.getCarrierNumber() + " with currentLocation "
								+ currentStop.getDescription() + " and destination " + destinationStop.getDescription()
								+ " to weldline 1 robotStop 3700");
						publishCarrierUpdate(carrier.getCarrierNumber(),
								getConsumptionStopByStopArea(StopArea.WELD_LINE_1));
					} else if (currentStop.getStopArea().equals(StopArea.WELD_LINE_2)) {
						LOG.info("Moving Carrier " + carrier.getCarrierNumber() + " with currentLocation "
								+ currentStop.getDescription() + " and destination " + destinationStop.getDescription()
								+ " to weldline 2 robotStop 10900");
						publishCarrierUpdate(carrier.getCarrierNumber(),
								getConsumptionStopByStopArea(StopArea.WELD_LINE_2));
					}
	...
	private void markCarrierConsumed(Carrier carrier) {
		if (testMode == 0) {
			List<OrderFulfillment> orderFulfillments = OrderFulfillment
					.findAllNotConsumedOrderFulfillmentsByCarrier(carrier);
