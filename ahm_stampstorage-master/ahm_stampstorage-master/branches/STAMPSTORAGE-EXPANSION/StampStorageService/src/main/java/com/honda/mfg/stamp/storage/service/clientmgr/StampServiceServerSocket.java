...
/**
 * This class provides a server socket to accept connections on the port that is
 * specified in stamp-service-socket.properties. Sockets are managed by a
 * {@link StampServiceSocketConnection} in its own thread
 * 
 * @author VCC44349
...
public class StampServiceServerSocket implements Runnable, StampServiceServerSocketInterface, ServiceRoleObserver {
	 ...
	 */
	public StampServiceServerSocket(String configFileName) throws Exception {
		try {
	...
	/**
	 * Constructor takes a {@link java.util.Properties} parameter for socket server
	 * properties
	 * 
	 * @param properties
	 */
	...
	public void init() throws Exception {
		...
		try {
			serverPort = Integer.valueOf(props.getProperty(SERVER_PORT_PROPERTY_KEY, String.valueOf(DEFAULT_PORT)))
					.intValue();

			maxNumConnections_ = Integer
					.valueOf(props.getProperty(MAX_NUM_CONNECTIONS, String.valueOf(maxNumConnections_))).intValue();
			...
			serverSocketTimeOut = Integer
					.valueOf(props.getProperty(SERVER_SOCKET_TIMEOUT_PROPERTY_KEY, String.valueOf(serverSocketTimeOut)))
					.intValue();

			maxNumTimeouts = Integer.valueOf(props.getProperty(MAX_NUM_TIMEOUTS, String.valueOf(maxNumTimeouts)))
					.intValue();
			LOG.info("StampServiceServerSocket.init():ServerSocket maxNumTimeouts_ set to: " + maxNumTimeouts);

			preWriteDelay = Integer.valueOf(props.getProperty(PRE_WRITE_DELAY, String.valueOf(preWriteDelay)))
					.intValue();
	...
	/*
	 * This is the run method for the socket server. Creates the server socket and
	 * waits for client connections. Client sockets are handed off to {@link
	 * StampServiceSocketConnection} for further processing on a separate thread,
	 * and continues to wait for client connections (non-Javadoc)
	 * 
			...
			while (!done) {
				...
				try {
					...
					if (serverSocket == null) {
						...
						LOG.info("StampServiceServerSocket.run():ServerSocket timeout set to: " + serverSocketTimeOut
								+ " milliseconds");
					}
					...
					LOG.info(
							"StampServiceServerSocket.run():ServerSocket latch released, accepting client connections...");
					...
					StampServiceSocketConnection stampServiceSocketConnection = new StampServiceSocketConnection(
							clientSocket, props);
	...
	/**
	 * close socket, but re-enter loop, i.e., don't set done. typically used for
	...
	public void setSocketMap(Map<String, StampServiceSocketConnectionInterface> socketMap) {
