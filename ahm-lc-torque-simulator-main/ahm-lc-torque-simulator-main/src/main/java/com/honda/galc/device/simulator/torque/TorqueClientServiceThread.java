...
public class TorqueClientServiceThread extends Thread {
	 ...
	 * @param strBuf
	 */
	private void handleMessage(final StringBuffer strBuf) {
		try {
			String str = strBuf.toString();
			final OPMessageType msgType = OPMessageType.getMessageType(strBuf);
			...
			Platform.runLater(new Runnable() {
				public void run() {
					TorqueSimulator.eventBus.post(new OPMessageEvent(deviceId, msgType, strBuf.toString()));
	            }
	        });
	...
	public void sendTorque(Torque torque) {
		if ((deviceStateMachine.getCurrentState() & TorqueSimulatorStateMachine.READY_FOR_TORQUE_READING) 
				!= TorqueSimulatorStateMachine.READY_FOR_TORQUE_READING ) {
			Logger.log(fOut, "Controller not in torque ready state: " + StringUtil.padLeft(printBinary(deviceStateMachine.getCurrentState()), 8, '0', true));
			Logger.log(fOut, "Expected state: " + StringUtil.padLeft(printBinary(TorqueSimulatorStateMachine.READY_FOR_TORQUE_READING), 8, '0', true));
		}
	...
	public String printBinary(int val) {
