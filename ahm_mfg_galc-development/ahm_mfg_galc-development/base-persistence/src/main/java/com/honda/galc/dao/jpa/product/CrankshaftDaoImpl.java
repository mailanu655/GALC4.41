...
public class CrankshaftDaoImpl extends DiecastDaoImpl<Crankshaft> implements CrankshaftDao {

	private static final String RELEASE_PRODUCT_HOLD_WITH_CHECK = "update galadm.crankshaft_tbx set hold_status = 0  where crankshaft_id = ?1 and (select count(*) from galadm.gal147tbx where product_id = ?1 and (release_flag != 1 or release_flag is null)) = 0";
    ...
    private static final String FIND_ALL_BY_QSR_ID = "select p from Crankshaft p, HoldResult h where h.id.productId =  p.crankshaftId and h.qsrId = :qsrId";
    private static final String FIND_ALL_BY_PROCESS_POINT_ID_AND_TIME = "select p from Crankshaft p, CrankshaftHistory h where h.id.processPointId = :processPointId and h.id.crankshaftId = p.crankshaftId and h.id.actualTimestamp >= :startTime and h.id.actualTimestamp <= :endTime";    

    private static final String FIND_VALID_PRODUCT_FOR_PROCESS_POINT = "SELECT A.* FROM CRANKSHAFT_TBX A WHERE A.CRANKSHAFT_ID = ?1 AND A.TRACKING_STATUS IN (" +
	...
	public void updateEngineSerialNumber(String productId, String esn) {
		update(Parameters.with("engineSerialNumber", esn),Parameters.with("crankshaftId", productId));
	}
	...
	@Transactional
	public void updateTrackingAttributes(Crankshaft product) {
		update(Parameters.with("lastPassingProcessPointId", product.getLastPassingProcessPointId())
				...
				Parameters.with("crankshaftId", product.getProductId()));
	}

	@Transactional
	public void updateDefectStatus(String productId, DefectStatus status) {
		update(Parameters.with("defectStatus", status.getId()), Parameters.with("crankshaftId", productId));
	}

	@Transactional
	public void updateHoldStatus(String productId, int status) {
		update(Parameters.with("holdStatus", status), Parameters.with("crankshaftId", productId));
	}
	...
	public int removeDunnage(String productId) {
		return update(Parameters.with("dunnage", null), Parameters.with("crankshaftId", productId));
	}

	@Transactional
	public Crankshaft save(Crankshaft crankshaft, InstalledPart installedPart) {
		getInstalledPartDao().save(installedPart);
		return save(crankshaft);
	}

	public List<Crankshaft> findAllByEngineSerialNumber(String engineId) {
	...
	public int updateDunnage(String productId,String dunnageId) {
		return update(Parameters.with("dunnage", dunnageId), Parameters.with("crankshaftId", productId));
	}
	...
	public void updateNextTracking(String productId, String nextTrackingStatus){
		update(Parameters.with("trackingStatus", nextTrackingStatus),
				Parameters.with("crankshaftId", productId));
	}
	...
	public boolean isProductTrackingStatusValidForProcessPoint(String productId, String processPointId) {
		...
		Crankshaft result = findFirstByNativeQuery(FIND_VALID_PRODUCT_FOR_PROCESS_POINT, params);
		return result != null;
	}
