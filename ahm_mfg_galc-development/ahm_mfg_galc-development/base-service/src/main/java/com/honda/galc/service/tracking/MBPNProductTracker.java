 ...
 * @since Dec 21, 2012
 */
public class MBPNProductTracker extends ProductTrackerBase<MbpnProduct>{
	//@Autowired
	...
	@Override
	protected ProductResult createProductHistory(MbpnProduct product, ProcessPoint processPoint, 
			DailyDepartmentSchedule schedule, Timestamp actualTimestamp, String associateNo, String approverNo, String deviceId) {
	...
	boolean isProductHistoryExist(MbpnProduct product, ProcessPoint processPoint) {
		return getProductResultDao().isProductProcessed(product.getProductId(), 
