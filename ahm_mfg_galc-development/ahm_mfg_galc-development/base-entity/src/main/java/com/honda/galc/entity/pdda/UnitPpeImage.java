 ...
 */
@Entity
@Table(name="PVUPI1", schema="VIOS")
public class UnitPpeImage extends AuditEntry implements Serializable {
	
	private static final long serialVersionUID = 1L;

	@EmbeddedId
	private UnitPpeImageId id;
	...
	@Column(name="PPE_USAGE", length=500)
	private String ppeUsage;

    public UnitPpeImage() {}

	public UnitPpeImageId getId() {
		return this.id;
	}

	public void setId(UnitPpeImageId id) {
		this.id = id;
	}
	...
	public boolean equals(Object obj) {
		...
		if (getClass() != obj.getClass())
			return false;
		UnitPpeImage other = (UnitPpeImage) obj;
		if (deptCode == null) {
	...
	public String toString() {
		return toString(getId().getMaintenanceId(), getId().getImageSeqNo(), getId().getPpeId(), getDeptCode(), getImage(), 
				getPlantLocCode(), getPotentialHazard(), getImageTimestamp(), getPpeRequired(), getImageTimestamp(),
				getPpeUsage());
	}
}