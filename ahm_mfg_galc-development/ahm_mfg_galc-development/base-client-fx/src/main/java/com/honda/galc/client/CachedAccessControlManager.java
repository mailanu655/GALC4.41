...
public class CachedAccessControlManager extends PersistentCache implements IAccessControlManager{
	...
	public String mapProxCardNumber(Long cardNumber) {
	   	ProxCard card = ServiceFactory.getDao(ProxCardDao.class).findByKey(cardNumber);
	...
	public ScreenAccessLevel getHighestAccessLevel(String screenId) {
		...
		if (accessControlList != null && accessControlList.size() > 0) {
			for(AccessControlEntry entry : accessControlList) {
				if(entry.getId().getScreenId().equals(screenId)) {
