...
public class TorqueSender {
	...
	public static int tighteningId = 0;
	static {
	...
	public void send(VirtualTorqueDevice virtualTorqueDevice, DataContainer dc){
		virtualTorqueDevice.sendTorque(createDefaultTorque(dc));
	}
	
	private Torque createDefaultTorque(DataContainer dc) {
		Torque torque = new Torque();
		torque.setId(tighteningId);
		torque.setFields(prepareTorqueFields(dc));
		
		return torque;
	}

	private ArrayList<OPMessageField> prepareTorqueFields(DataContainer dc) {
		...
		fields.add(createOPMessageField("TIGHTENING_ID",String.format("%1$010d", tighteningId++)));
		return  fields;
	}
