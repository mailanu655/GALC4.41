...
public class ViosPartConfigView extends AbstractViosTabbedView<ViosPartConfigModel, ViosPartConfigController> {
	...
	private ObjectTablePane<MCViosMasterOperationPart> partDetailsTablePane;
	private ObjectTablePane<MCViosMasterOperationPartChecker> checkerDetailsTable;
	...
	public ViosPartConfigView(TabbedMainWindow mainWindow) {
		super(ViewId.DATA_COLLECTION_VIEW, mainWindow);
	}

	@Override
	public String getScreenName() {
		return "VIOS Part Config";
	}
	...
	public void reload(String filter) {
		if(platform != null) {
			List<MCViosMasterOperationPart> masterOpPartList = ServiceFactory.getDao(MCViosMasterOperationPartDao.class).findAllFilteredPart(platform.getGeneratedId(), filter);
			partDetailsTablePane.setData(masterOpPartList);
		}
	}
	...
	public Node getCenterBody() {
		...
		partDetailsBox.setPadding(new Insets(10));
		...
		partDetailsTablePane = createPartDetailsTable();

		partDetailsBox.getChildren().addAll(filterBox, partDetailsTablePane);
		...
		TitledPane partTitledPane = createTitiledPane("Part Details", partDetailsBox);
		TitledPane checkerDetailsTitledPane = createTitiledPane("Part Checker Details", checkerBox);
	...
	private ObjectTablePane<MCViosMasterOperationPart> createPartDetailsTable() {

		ColumnMappingList columnMappingList = 
				ColumnMappingList.with("Unit Number", "id.unitNo")
				.put("Part No", "id.partNo")
				.put("Part Type", "id.partTypeAsString")
				.put("Part Mask", "partMask");
		Double[] columnWidth = new Double[] {
				0.16, 0.16, 0.16, 0.10
		}; 
		ObjectTablePane<MCViosMasterOperationPart> subTable = new ObjectTablePane<MCViosMasterOperationPart>(columnMappingList,columnWidth,true);

		subTable.setConstrainedResize(true);
		return subTable;
	}
	...
	public ObjectTablePane<MCViosMasterOperationPart> getPartDetailsTablePane() {
		return partDetailsTablePane;
	}

	public ObjectTablePane<MCViosMasterOperationPartChecker> getCheckerDetailsTable() {
