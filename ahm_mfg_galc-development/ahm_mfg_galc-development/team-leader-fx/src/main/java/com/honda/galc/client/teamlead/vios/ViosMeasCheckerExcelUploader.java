...
public class ViosMeasCheckerExcelUploader extends ViosExcelUploader<MCViosMasterOperationMeasurementChecker> {

	private String viosPlatform;
	public ViosMeasCheckerExcelUploader(String viosPlatform, String userId) {
	...
	@Override
	public String doUpload(List<MCViosMasterOperationMeasurementChecker> entityList) {
		StringBuffer infoLog = new StringBuffer();
		int recordSaved = 0;
		int recordSkipped = 0;
		Set<String> operationSet = new HashSet<String>();
		List<MCViosMasterOperationMeasurementChecker> validatedEntityList = new ArrayList<MCViosMasterOperationMeasurementChecker>();
		for (MCViosMasterOperationMeasurementChecker entity : entityList) {
			try {
		...
		//delete all checkers records from master table and Revision Table
		try {
			ServiceFactory.getService(ViosMaintenanceService.class).deleteAllMeasChecker(viosPlatform, operationSet);
		} catch (Exception e) {
			recordSkipped++;
		}
		for (MCViosMasterOperationMeasurementChecker entity : validatedEntityList) {
			try {
				uploadEntity(entity);
				recordSaved++;
			} catch (Exception e) {
