...
public class ViosMeasConfigView extends AbstractViosTabbedView<ViosMeasConfigModel, ViosMeasConfigController> {
	...
	private ObjectTablePane<MCViosMasterOperationMeasurement> measDetailsTablePane;
	private ObjectTablePane<MCViosMasterOperationMeasurementChecker> checkerDetailsTable;
	...
	public ViosMeasConfigView(TabbedMainWindow mainWindow) {
		super(ViewId.DATA_COLLECTION_VIEW, mainWindow);
	}

	@Override
	public String getScreenName() {
		return "VIOS Meas Config";
	}
	...
	public void reload(String filter) {
		if(platform != null) {
			List<MCViosMasterOperationMeasurement> masterOpMeasList = ServiceFactory.getDao(MCViosMasterOperationMeasurementDao.class).findAllFilteredMeas(platform.getGeneratedId(), filter);
			measDetailsTablePane.setData(masterOpMeasList);
		}
	}
	...
	public Node getCenterBody() {
		...
		measDetailsBox.setPadding(new Insets(10));
		...
		measDetailsTablePane = createMeasDetailsTable();

		measDetailsBox.getChildren().addAll(filterBox, measDetailsTablePane);
		...
		TitledPane partTitledPane = createTitiledPane("Measurement Details", measDetailsBox);
		TitledPane checkerDetailsTitledPane = createTitiledPane("Measurement Checker Details", checkerBox);
	...
	private ObjectTablePane<MCViosMasterOperationMeasurement> createMeasDetailsTable() {

		ColumnMappingList columnMappingList = 
				ColumnMappingList.with("Unit Number", "id.unitNo")
				.put("Part Type", "id.partTypeAsString")
				.put("Meas Seq", "id.measurementSeqNum")
				.put("Device ID", "deviceId")
				.put("Device Msg", "deviceMsg")
				.put("Min Limit", "minLimit")
				.put("Max Limit", "maxLimit")
				.put("Max Attempts", "maxAttempts");
		Double[] columnWidth = new Double[] {
				0.08,  0.5, 0.08, 0.10, 0.12, 0.06, 0.06, 0.5
		}; 
		ObjectTablePane<MCViosMasterOperationMeasurement> subTable = new ObjectTablePane<MCViosMasterOperationMeasurement>(columnMappingList,columnWidth,true);
	...
	private void addVMCComboBoxListener(String value) {
		...
		if(value != null) {
			...
			measDetailsTablePane.getTable().getItems().clear();
	...
	public ObjectTablePane<MCViosMasterOperationMeasurement> getMeasDetailsTablePane() {
		return measDetailsTablePane;
	}

	public ObjectTablePane<MCViosMasterOperationMeasurementChecker> getCheckerDetailsTable() {
