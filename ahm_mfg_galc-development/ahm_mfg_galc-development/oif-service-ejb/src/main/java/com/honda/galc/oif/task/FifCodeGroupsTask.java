...
import com.honda.galc.oif.dto.FifCodeGroupsDTO;
...
/**
 * 
 * <h3>FifCodeGroupsTask.java</h3> <h3>Class description</h3> <h4>Description</h4>
 * <p>
 * FifCodeGroupsTask.java description
 * </p>
 * 
...
public class FifCodeGroupsTask extends OifTask<Object> implements
	...
	public FifCodeGroupsTask(String name) {
		super(name);
	}

	public void execute(Object[] args) {
		try {
			processFifCodeGroups();
	 ...
	 */
	private void processFifCodeGroups() throws ParseException {
		logger.info("start to process CodeGroups");
		refreshProperties();
		...
		String fifCodeGro = getProperty(OIFConstants.PARSE_LINE_DEFS);
		OIFSimpleParsingHelper<FifCodeGroupsDTO> parseHelper = new OIFSimpleParsingHelper<FifCodeGroupsDTO>(
				FifCodeGroupsDTO.class, fifCodeGro, logger);
		parseHelper.getParsingInfo();

		// Process file(s) and update FIF Code Groups data
		for (int count = 0; count < aReceivedFileList.length; count++) {
			...
			// Process files(s)
			List<FifCodeGroupsDTO> fifCodeGroupsDTOList = new ArrayList<FifCodeGroupsDTO>();
			for (String receivedRecord : receivedRecords) {
				FifCodeGroupsDTO fifCodeGroupsDTO = new FifCodeGroupsDTO();
				parseHelper.parseData(fifCodeGroupsDTO, receivedRecord);
				if (fifCodeGroupsDTO.getBaseType() == null
						|| fifCodeGroupsDTO.getDevSeqCd() == null
						|| fifCodeGroupsDTO.getEfctBegDt() == null
						|| fifCodeGroupsDTO.getFifType() == null
						|| fifCodeGroupsDTO.getGroupCd() == null
						|| fifCodeGroupsDTO.getModelCd() == null
						|| fifCodeGroupsDTO.getModelYear() == null
						|| fifCodeGroupsDTO.getPlantCd() == null) {
					...
					fifCodeGroupsDTOList.add(fifCodeGroupsDTO);
				}
			}

			// Update or Insert data
			for (FifCodeGroupsDTO fifCodeGroupsDTO : fifCodeGroupsDTOList) {
				FifCodeGroups fifCodeGroups = fifCodeGroupsDTO
						.deriveFifCodeGroups();
				ServiceFactory.getDao(FifCodeGroupsDao.class).save(
						fifCodeGroups);
			}
			logger.info(" FIF Code Groups saved; file processed: "
					+ receivedFile);
