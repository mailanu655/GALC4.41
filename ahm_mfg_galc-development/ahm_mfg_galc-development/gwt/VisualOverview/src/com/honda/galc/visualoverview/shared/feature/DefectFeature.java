...
public class DefectFeature extends Feature {
	...
	public void draw(Map<String, List<PrintAttributeFormat>> attributeMap, Layer refLayer, HandlerManager eventBus, Vector layer) {
		...
		if(!getAttributes().isEmpty())
    		style = new FeatureStyle(this, getAttributes());
		else
			style = new FeatureStyle(this, new ArrayList<PrintAttributeFormat>()); 
		for(FeaturePoints featurePoint : getFeaturePoints())
		{			
			Pixel pixel = new Pixel((int)featurePoint.getXCoordinate(), (int)featurePoint.getYCoordinate());
			LonLat lonLat = new LonLat(defectTransformX(featurePoint.getXCoordinate(), refLayer),defectTransformY(featurePoint.getYCoordinate(), refLayer));
	...
	private double defectTransformX(double transform, Layer refLayer)
	{
		double result = 0;
		try
		{
			result =  transform * (refLayer.getBoundaryRight() - refLayer.getBoundaryLeft())/500 + refLayer.getBoundaryLeft();
	...
	private double defectTransformY(double transform, Layer refLayer)
	{
		double result = 0;
		try
		{
			result =  transform * (refLayer.getBoundaryLower() - refLayer.getBoundaryUpper())/500 + refLayer.getBoundaryUpper();
