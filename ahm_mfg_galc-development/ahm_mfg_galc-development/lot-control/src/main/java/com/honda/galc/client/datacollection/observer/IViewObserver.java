 ...
 */
@ObserverInterface
public interface IViewObserver extends IViewManager {
	...
	void completeProductId(ProcessRefresh state);
	...
	@ProcessPartState(actions = { Action.OK_WAIT})
	void partSnOkButWait(final ProcessPart state);
	@ProcessPartState(actions = { Action.BYPASS})
	void receivedBypass(final ProcessPart state);
	@ProcessPartState(actions = { Action.AUTO})
	void receivedAuto(final ProcessPart state);
