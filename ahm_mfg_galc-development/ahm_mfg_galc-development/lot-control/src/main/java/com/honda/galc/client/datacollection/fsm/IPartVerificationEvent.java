...
public interface IPartVerificationEvent {
	...
	void partSnOk(InstalledPart part);
	void partsSnOk(List<InstalledPart> parts, InstalledPart part);
	void partSnMissing(InstalledPart part);
	void partSnNg(InstalledPart part, String msgId, String userMsg);
	void reject();
	void partNoAction();
	void repairPartSelected();
	void partSnOkButWait(InstalledPart partBean);
	void receivedBypass(InstalledPart partBean);
	void receivedAuto(InstalledPart partBean);
}