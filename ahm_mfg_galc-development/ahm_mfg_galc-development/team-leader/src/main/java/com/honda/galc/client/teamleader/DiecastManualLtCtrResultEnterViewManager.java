...
public class DiecastManualLtCtrResultEnterViewManager extends ManualLtCtrResultEnterViewManagerBase {
	...
	public DiecastManualLtCtrResultEnterViewManager(MainWindow mainWin,
	...
	private List<BlockBuildResult> getBlockBuildResultData() {
		BlockBuildResult result = new BlockBuildResult();
		BlockBuildResultId id = new BlockBuildResultId(view.getProductPanel().getProductIdField().getText(),
	...
	private List<HeadBuildResult> getHeadBuildResultData() {
		HeadBuildResult result = new HeadBuildResult();
		HeadBuildResultId id = new HeadBuildResultId(view.getProductPanel().getProductIdField().getText(),
				getCurrentLotControlRule().getPartNameString());
		result.setId(id);
		result.setInstalledPartStatus(InstalledPartStatus.OK);
		result.setActualTimestamp(new Timestamp(System.currentTimeMillis()));
	...
	private List<ConrodBuildResult> getConrodBuildResultData() {
		ConrodBuildResult result = new ConrodBuildResult(view.getProductPanel().getProductIdField().getText(), getCurrentLotControlRule().getPartNameString());
		result.setInstalledPartStatus(InstalledPartStatus.OK);
	...
	private List<CrankshaftBuildResult> getCrankshaftBuildResultData() {
		CrankshaftBuildResult result = new CrankshaftBuildResult(view.getProductPanel().getProductIdField().getText(), getCurrentLotControlRule().getPartNameString());
		result.setInstalledPartStatus(InstalledPartStatus.OK);
	...
	private void getInputResultValue(ProductBuildResult result) {
		if (view.getDataPanel().getPartSnField().isVisible())
			result.setResultValue(view.getDataPanel().getPartSnField().getText());
	}
