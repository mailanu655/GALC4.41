...
public class ManualLotCtrRepairUtil {
	...
	public static String EnginePartName = "EnginePartName";
	...
	public static boolean isEngine(ManualLotControlRepairPropertyBean property, PartResult partResult) {
		if(property.getEngineInstallPropertyMap() == null || property.getEngineInstallPropertyMap().size() ==0)
			return false;
		
		return (property.getEngineInstallPropertyMap().get(EnginePartName).equals(partResult.getPartName())
				&& property.getEngineInstallPropertyMap().get(ProcessPointId).equals(partResult.getProcessPointId()));
	}
