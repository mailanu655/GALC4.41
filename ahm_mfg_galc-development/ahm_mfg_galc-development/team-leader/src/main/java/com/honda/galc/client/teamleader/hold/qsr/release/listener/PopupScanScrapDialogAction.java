...
import com.honda.galc.client.teamleader.hold.qsr.release.scrap.ScanScrapDialog;
 ...
 * <p>
 * <code>PopupScanScrapDialogAction</code> is ... .
 * </p>
 ...
 */
public class PopupScanScrapDialogAction extends BaseListener<ReleasePanel> implements ActionListener {

	public PopupScanScrapDialogAction(ReleasePanel panel) {
		super(panel);
	}
	...
	public void executeActionPerformed(ActionEvent e) {
		...
		if (holdResults.size() == 0) {
			message.append("There are no Holds to be released/scrapped");
			JOptionPane.showMessageDialog(getView(), message, "Invalid Input", JOptionPane.WARNING_MESSAGE);
			return;
		}
		...
		ScanScrapDialog dialog = new ScanScrapDialog(getView(), "Scrap Products", qsrHoldResults, holdResults, getView().getCache());
