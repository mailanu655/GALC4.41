...
public class ClientMonitor extends Thread {
	...
	public void run() {
		init();
		long interval = getHeartbeatInterval();
		while (isActive()) {
			try {
				WebStartClient client = getAppContext().getWebStartClient();
				if (client != null) {
					handleShutDownRequest(client);
					sendHeartBeat(interval, client);
				} else {
					getLogger().debug(String.format("Client information not found. Could not update heartbeat for Asset %s (%s)", 
	...
	/**
	 * update wesbtartclient in a thread safe method
	 * 
	 * @param client
	 */
	...
	private ApplicationContext getAppContext() {
		return ClientMain.getInstance().getApplicationContext();
	}
	...
	private boolean isHeadless() {
		return ClientMain.getInstance().getApplicationContext().getArguments().isHeadless();
	}
