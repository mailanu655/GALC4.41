...
/**
 * 
 * <h3>PreProductionLotSeqUtils</h3>
...
public class PreProductionLotSeqUtils extends PreProductionLotUtils{
	...
	private List<PreProductionLot> getUnProcessedRowsBeforeSelectedRow(int selectedRow) {
		...
		if(selectedRow > 0){
			...
			for(MultiValueObject<PreProductionLot> lot : scheduleMainPanel.getUpcomingLotPanel().getItems().subList(0, (firstSelectedRow)))
	...
	public List<PreProductionLot> moveTop(int selectedRowNum) {
		List<MultiValueObject<PreProductionLot>> lotsList = getSelectedProductionLots(0, selectedRowNum);
		List<Double> originalSeqNumList=new ArrayList<Double>();
		for(int i=0;i<=selectedRowNum;i++)
		{
			originalSeqNumList.add(lotsList.get(i).getKeyObject().getSequence());
		}
		List<PreProductionLot> changedLots = new ArrayList<PreProductionLot>();			    
		MultiValueObject<PreProductionLot> selectedProdLot = lotsList.get(selectedRowNum );
		lotsList.remove(selectedProdLot);
		lotsList.add(0,selectedProdLot);
		for(int i=0;i<=selectedRowNum;i++)
		{
			PreProductionLot lot=lotsList.get(i).getKeyObject();
			lot.setSequence(originalSeqNumList.get(i));
			changedLots.add(lot);
		}
...
class MvoPreProductionLotComparator implements Comparator<MultiValueObject<PreProductionLot>>{
	public int compare(MultiValueObject<PreProductionLot> o1, MultiValueObject<PreProductionLot> o2) {
		return (int)(o1.getKeyObject().getSequence() - o2.getKeyObject().getSequence());
	}
