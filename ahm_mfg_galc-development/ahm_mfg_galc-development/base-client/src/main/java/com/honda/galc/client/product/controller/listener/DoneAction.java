package com.honda.galc.client.product.controller.listener;

import java.awt.Component;
import java.awt.event.ActionEvent;

import javax.swing.JButton;

import com.honda.galc.client.product.process.view.ProcessView;
import com.honda.galc.client.product.view.ProductPanel;

/**
 * <h3> Class description</h3>
 * <h4> Description </h4>
 * <p>
 * <code>DoneAction</code> is ... .
 * </p>
 * <h4>Usage and Example</h4>
 * <h4>Special Notes</h4>
 * 
 * <h4>Change History</h4>
 * <Table border="1" Cellpadding="3" Cellspacing="0" width="100%">
 * <TR bgcolor="#EEEEFF" Class="TableSubHeadingColor">
 * <TH>Update by</TH>
 * <TH>Update date</TH>
 * <TH>Version</TH>
 * <TH>Mark of Update</TH>
 * <TH>Reason</TH>
 * </TR>
 * <TR>
 * <TD>&nbsp;</TD>
 * <TD>&nbsp;</TD>
 * <TD>0.1</TD>
 * <TD>(none)</TD>
 * <TD>Initial Realse</TD>
 * </TR>
 * </TABLE>
 * 
 * @see
 * @ver 0.1
 * @author Karol Wozniak
 */
public class DoneAction extends BaseListener<ProductPanel> {

	public DoneAction(ProductPanel view) {
		super(view);
	}

	@Override
	protected void executeActionPerformed(ActionEvent ae) {
		
		Component comp = getView().getSelectedTab();
		if (comp instanceof ProcessView) {
			ProcessView pv = (ProcessView) comp;
			JButton button = pv.getDoneButton();
			if (button != null && button.isEnabled()) {
				button.doClick();
				return;
			}
		}		
		
		getView().getController().finishProduct();
	}
}
