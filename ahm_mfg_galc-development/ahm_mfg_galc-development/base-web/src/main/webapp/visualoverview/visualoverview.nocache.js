function visualoverview(){
  ...
  $stats && $stats({moduleName:'visualoverview', sessionId:$sessionId_0, subSystem:'startup', evtGroup:'bootstrap', millis:(new Date).getTime(), type:'begin'});
  ...
  function maybeStartModule(){
    if (scriptsDone && loadDone) {
      var iframe = $doc_0.getElementById('visualoverview');
      ...
      visualoverview = null;
      frameWnd.gwtOnLoad(onLoadErrorFunc, 'visualoverview', base, softPermutationId);
      $stats && $stats({moduleName:'visualoverview', sessionId:$sessionId_0, subSystem:'startup', evtGroup:'moduleStartup', millis:(new Date).getTime(), type:'end'});
    }
  }
  ...
  function computeScriptBase(){
    ...
    function tryNocacheJsTag(){
      ...
      for (var i = 0; i < scriptTags.length; ++i) {
        if (scriptTags[i].src.indexOf('visualoverview.nocache.js') != -1) {
    ...
    function tryMarkerScript(){
      ...
      if (typeof isBodyLoaded == 'undefined' || !isBodyLoaded()) {
        var markerId = '__gwt_marker_visualoverview';
        var markerScript;
  ...
  function processMetas(){
    ...
    for (var i = 0, n = metas.length; i < n; ++i) {
      ...
      if (name_0) {
        name_0 = name_0.replace('visualoverview::', '');
        if (name_0.indexOf('::') >= 0) {
          continue;
        }
  ...
  function maybeInjectFrame(){
    if (!frameInjected) {
      ...
      iframe.id = 'visualoverview';
      iframe.style.cssText = 'position:absolute;width:0;height:0;border:none';
      iframe.tabIndex = -1;
      $doc_0.body.appendChild(iframe);
      $stats && $stats({moduleName:'visualoverview', sessionId:$sessionId_0, subSystem:'startup', evtGroup:'moduleStartup', millis:(new Date).getTime(), type:'moduleRequested'});
  ...
  visualoverview.onScriptLoad = function(){
    if (frameInjected) {
      loadDone = true;
      maybeStartModule();
    }
  ...
  ;
  visualoverview.onInjectionDone = function(){
    scriptsDone = true;
    $stats && $stats({moduleName:'visualoverview', sessionId:$sessionId_0, subSystem:'startup', evtGroup:'loadExternalRefs', millis:(new Date).getTime(), type:'end'});
    maybeStartModule();
  }
  ;
  processMetas();
  ...
  var initialHtml;
  if (isHostedMode()) {
    if ($wnd_0.external && ($wnd_0.external.initModule && $wnd_0.external.initModule('visualoverview'))) {
      $wnd_0.location.reload();
      return;
    }
    initialHtml = 'hosted.html?visualoverview';
    strongName = '';
  }
  $stats && $stats({moduleName:'visualoverview', sessionId:$sessionId_0, subSystem:'startup', evtGroup:'bootstrap', millis:(new Date).getTime(), type:'selectingPermutation'});
  if (!isHostedMode()) {
    try {
      unflattenKeylistIntoAnswers(['ie6'], '01915C0E7F0E064DFECF78BAEEDE943F');
      unflattenKeylistIntoAnswers(['safari'], '2629D4E773EB33888BCBC29C2058BD71');
      unflattenKeylistIntoAnswers(['gecko1_8'], '2AE2BF7FDEE38D6CD742F5C982D33746');
      unflattenKeylistIntoAnswers(['ie8'], '7D81D81A950B68C24D39778118BB410D');
      unflattenKeylistIntoAnswers(['ie9'], '7E458A0A4BF470B91CECCB137B6D834E');
      unflattenKeylistIntoAnswers(['opera'], '8B08B12790AFFE8F85EA5CB7EA311F76');
  ...
  , 50);
  $stats && $stats({moduleName:'visualoverview', sessionId:$sessionId_0, subSystem:'startup', evtGroup:'bootstrap', millis:(new Date).getTime(), type:'end'});
  $stats && $stats({moduleName:'visualoverview', sessionId:$sessionId_0, subSystem:'startup', evtGroup:'loadExternalRefs', millis:(new Date).getTime(), type:'begin'});
  ...
  if (!__gwt_scriptsLoaded['OpenLayers.js']) {
    __gwt_scriptsLoaded['OpenLayers.js'] = true;
    document.write('<script language="javascript" src="' + base + 'OpenLayers.js"><\/script>');
  }
  $doc_0.write('<script defer="defer">visualoverview.onInjectionDone(\'visualoverview\')<\/script>');
}
