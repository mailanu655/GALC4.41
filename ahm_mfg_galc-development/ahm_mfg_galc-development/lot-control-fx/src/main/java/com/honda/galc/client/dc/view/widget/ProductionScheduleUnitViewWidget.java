...
public class ProductionScheduleUnitViewWidget extends AbstractWidget {

	private static final long serialVersionUID = 1L;

	public ProductionScheduleUnitViewWidget(ProductController productController) {
		super(ViewId.PRODUCTION_SCHEDULE_UNITVIEWWIDGET, productController);
	}
	...
	protected void init() {
		...
		String height = property.getProductionScheduleUnitHeight();
		String fontsize = property.getProductionScheduleUnitFontSize();
		String rows = property.getProductionScheduleUnitRows();
		...
		String url ="http://"+ViewControlUtil.getHostURL(hosturl)+"/BaseWeb/ProductionScheduleUnitViewWidget.html?product-id="+getProductController().getModel().getProductId()+"&height="+height+"&width="+width+"&font-size="+fontsize+"&number-of-records="+rows;
		engine.load(url);
		this.setCenter(view);
	}
