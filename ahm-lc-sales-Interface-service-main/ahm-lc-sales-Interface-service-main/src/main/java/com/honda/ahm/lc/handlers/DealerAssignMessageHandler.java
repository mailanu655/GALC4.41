...
public class DealerAssignMessageHandler implements IStatusMessageHandler {
    ...
    public List<String> handle(StatusMessage statusMessage, StatusEnum status) {
        ...
        String asgnDate = DateTimeFormatter.ofPattern("yyMMdd").format(localDateTime);
        String asgnTime = DateTimeFormatter.ofPattern("hhmmss").format(localDateTime);
        ...
        if (shippingStatus.getStatus() > StatusEnum.DLR_ASGN.getStatus()) {
            logger.warn("Ignoring request. ShippingStatus is greater than {}", StatusEnum.DLR_ASGN.getType());
            errorMessages.add("Ignoring request. ShippingStatus for VIN "+ statusVehicle.getVin()+" is greater than "+ StatusEnum.DLR_ASGN.getType());
            return errorMessages;
        }
        ...
        errorMessages.addAll(frameShipConfirmationService.processFrameShipConfirmation(
                ...
                propertyUtil.getProcessPoint(StatusEnum.DLR_ASGN.getType()),
                asgnDate,
                asgnTime, StatusEnum.DLR_ASGN
                ));

        logger.info("Tracking - "+status.getType()+" Status Message for VIN {}", statusVehicle.getVin());
        shippingStatusService.trackProduct(galcUrl, propertyUtil.getProcessPoint(StatusEnum.DLR_ASGN.getType()), statusVehicle.getVin());
