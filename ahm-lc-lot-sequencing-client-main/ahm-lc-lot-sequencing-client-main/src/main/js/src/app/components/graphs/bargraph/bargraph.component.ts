...
export class BarChartComponent {
  ...
  createChart(usageDetails: any) {
    this.logService.info(`
      ...
      userDetails.labels ${usageDetails.labels},
      usageDetails.dataSets ${usageDetails.dataSets}`, 
      [{ method: 'createChart' }]);
  ...
  selectFromDate(fromDate) {
    this.logService.info('select from date ', [{ method: 'selectFromDate' }]);
    this.frmDatePick = fromDate;
    this.getApplicationsUsageDetailsByDate(fromDate, this.toDatePick);
  }

  selectToDate(toDate) {
    this.logService.info('select to date ', [{ method: 'selectToDate' }]);
